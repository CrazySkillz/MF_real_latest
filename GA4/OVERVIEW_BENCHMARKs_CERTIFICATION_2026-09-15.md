...
**Status: CLEAN-CERTIFIED / PRODUCTION_READY for the GA4 Benchmarks section only at deployed revision `47180dcf96586fa4fff9a83e7a097e33e42c0721`, for the exact campaign/property/source and dependency boundary below. The original behavior certificate at `236afff993e60c5f9eaf75c42bca8b31b52f601d` remains historical supporting evidence; the current-revision delta and affected shared report consumer were revalidated below.**

The original certification was completed on 2026-09-16 after the forward duplicate-history fix was deployed, concurrently verified, existing duplicate history was cleaned within its proven boundary, and the full authenticated post-cleanup lifecycle passed. Current-deployed revalidation is recorded after the historical evidence below.
...
## Current-Deployed Revalidation — 2026-09-16

- Exact deployed `/api/health` returned `200` and revision `47180dcf96586fa4fff9a83e7a097e33e42c0721` before and after validation. The delta from `236afff9` changed no Benchmark CRUD, persistence, recompute job, scheduler, metric identity/formula, threshold, or alert resolver. Shared GA4 Campaign Breakdown, Ad Comparison, and report-assembly code did change. The import-to-date breakdown route now rejects missing property/campaign scope, and its populated Overview rows require a valid session key event rate even though Benchmark formulas do not consume that rate; this is an availability precondition of the manifested financial fallback, recorded in the dependency manifest. The affected Benchmark browser/server report consumers and financial fail-closed paths were rechecked. Commit `f7afeb2b` itself changed only the Ad Comparison status sentence in `GA4/README.md`, outside the Benchmark dependency boundary.
- Focused Benchmark and affected shared-consumer packet: **18 files, 185/185 tests passed**; targeted financial/browser fallback packet: **5 files, 68/68 tests passed**. The existing `server/ga4-benchmark-regression.test.ts` certification assertions passed in a post-edit rerun (**16/16**), including the historical machine-record boundary; the repository has no separate `check:ga4-benchmark-certification` command, so no nonexistent gate is claimed. `npm run check` and `npm run build` each passed once.
- One final `npm run test:current-version` execution: **2,041 tests; 2,000 passed; 41 explicitly deferred unrelated failures; zero blocking current-version failures**. Deferred failures are visible, not counted as passes, and do not exercise the manifested Benchmark boundary. This suite ran with unrelated pre-existing dirty documentation/test worktree edits; those edits were not used as proof of deployed Benchmark behavior.
- Authenticated read-only deployed audit: two campaign inventories, four active GA4 Benchmarks, zero failures or application mutations; the selected live-property campaign had two exact provider/persisted/scheduler/UI matches, ready completed-day traffic, card/tracker/Insights/alert parity, browser Benchmark PDF parity, and no-property fail-closed behavior in the other campaign.
- Authenticated deployed lifecycle: one campaign-scoped manual refresh succeeded; temporary zero-current create, target/current edit, USD unit and status formatting, alert deduplication/resolution, same-owner other-client isolation, foreign-owner read/edit/delete denial, Executive Summary parity for both existing rows, delete/child cleanup, and final definition restoration passed. Independent postflight found zero active exact-definition duplicate groups, zero same-date/scope history duplicate groups, and no temporary concurrency-validation rows.
- A current **manual-only** Benchmark report snapshot and server PDF passed exact identity, current value, target, and status parity for both rows; parsed PDF was 7,980 bytes. The report configuration hash was restored exactly. This validation retained one normal immutable manual snapshot and sent **no email**. An older scheduled snapshot need not match today's current values; current scheduled delivery, inbox receipt, and a natural timer firing were not re-proven or claimed.
- Negative zero, unavailable, stale, blocked, insufficient, malformed-source, and fail-closed paths retain deterministic focused coverage; only ready zero and the populated USD/live-property state were exercised on the deployed fixture. No other currency, future provider window, source mix, whole-Reports behavior, KPI, Ad Comparison, or Overview certification is implied. No application code, other section, historical machine certificate, or master ledger was changed by this revalidation.
