...
Bounded scheduler evidence is reused from exact deployed SHA `85f5233ebfc298afc35f4c24e0930c1a66fbd07c`: the natural timer fired at `2026-08-14T20:35:00.001Z`, both Benchmarks updated with zero skipped or failed, and all `22` underlying daily rows were scheduler-written with no later application repair. The process-wide run failed for 17 excluded obsolete campaigns, so no global scheduler-health claim is made. Later commits `950307b6`, `1a93d8d8`, and `7366051f` did not change the scheduler or Benchmark recompute path; this is bounded path evidence, not an exact-current natural-run claim.
...
- Final read-only health at `2026-08-03T09:33:27Z` returned exact SHA `b42c51e9ebcc12d74851cc640c86038513a57828`. Report scheduler metrics recorded `totalSent = 2`, `totalFailed = 0`, and `successRate = 100.00%`. At that historical checkpoint, the separate GA4 daily scheduler remained healthy but idle and Current Commit 14 had not yet been completed. Current Commit 14 was later completed on exact runtime SHA `650ce59c4b0d14a21a198e8a2effd0c3a6d1fccd`.
...
The historical `650ce59c` boundary has no retroactive evidence gap. Exact deployed-candidate read-only validation is complete for implementation `3d762260` within deployed evidence revision `d6656e11`, and validation-only wrapper `c6487555` is user-confirmed in production. The separately authorized final combined GA4 certification remains intentionally outstanding. Reports-owned PDF/artifact findings remain recorded for the separate Reports audit and are not part of this Benchmark release-candidate decision.
...
- Historical checkpoint: the documentation/machine-record commit was evidence-only, and the then-planned final descendant comparison still preceded any status change. That comparison and the later natural-run gate are now complete at the controlling certified boundary above.
...
- Historical checkpoint findings for Current Commit 12: Critical `0`; Major `0`; Minor `2`, both fixed as validation guards. No applicable failure was deferred. Production certification remained `UNVERIFIED` at that checkpoint because later production-only evidence was still pending; those Benchmark gates are now complete.
...
- `GET /api/health` returned `200`, `nodeEnv = production`, and exact SHA `9ed32290b32773af543b7a927ef5e197c4e3b761`. `GET /health/scheduler` returned `200`; the GA4 daily scheduler was started, timer-scheduled, idle, and configured for `03:00 UTC`. At that checkpoint, natural-run evidence remained Current Commit 14; it was later completed on exact runtime SHA `650ce59c4b0d14a21a198e8a2effd0c3a6d1fccd`.
...
- Completed closure: after Current Commit 14 passed, the canonical and machine-readable Benchmark status was set to `PRODUCTION_READY`, the exact certified runtime SHA was recorded, zero remaining Benchmark Critical/Major findings were recorded, and the evidence-only boundary was committed, pushed, deployed, and verified.
...
This stop condition was satisfied at exact runtime SHA `650ce59c4b0d14a21a198e8a2effd0c3a6d1fccd` and deployed evidence commit `375b13728c6ce0c08fbe580ecf65e4b60ed21de1`. Reopen only for a changed dependency boundary, contradictory evidence, or a new in-scope defect.

### Historical July 2026 Status And Evidence

Historical July 31, 2026 record: Commit 18 changed the shared campaign Benchmark current-value failure contract. Failed financial reads, disconnected/missing GA4, and missing selected source IDs returned unavailable so existing refresh loops preserved last-known values instead of writing misleading zero. At that checkpoint the bounded implementation had passed focused tests, TypeScript, and a production build but had not yet been committed, deployed, or externally validated. That former gap is closed by the controlling August 15 release-candidate evidence above; this paragraph is not a current status statement.

At the July 1, 2026 historical checkpoint, after a strict then-current-code re-trace, focused Benchmark validation, and controlled deployed provider/UI/email/scheduler/token-failure evidence, GA4 Benchmarks were **clean-certified for that recorded GA4 Benchmarks section boundary**.

No blockers remained open for that historical GA4 Benchmarks certification boundary. Do not extend that claim to later code changes, future GA4 properties/windows, future alert emails, a real unsimulated Google token revocation event, timer-fired executions beyond the exact recorded run, or future platforms without fresh evidence.

Status split:

- historical July GA4 Benchmark section certification: passed for the traced UI/API/storage/scheduler/alert/notification/report paths covered by that recorded boundary, the focused Benchmark tests, and the controlled deployed validations recorded here
- historical July Current Commit blockers: none for that recorded packet; the later Benchmark Current Commits 8-14 are now complete, and former Commit 15 remains Reports-only history
...
- At that historical checkpoint, GA4 Benchmarks passed validation for the recorded production-ready clean-certification boundary documented in this section.
...
- Do not claim timer-fired executions beyond the exact `2026-08-12` recorded run, future GA4 provider windows/outages/delayed attribution, future Benchmark alert email deliveries, a real unsimulated Google revoked-token event, future source mixes, or future platform readiness without fresh evidence for that exact claim.
...
- historical documentation update: Commit 0 rewrote this file into the strict KPI-style certification structure without changing runtime behavior
- historical validation-support update: Commit 2 added a GA4 Benchmark provider validation endpoint and regression coverage; deployed validation found a live provider auth blocker and an apparent stored-current mismatch that required stricter window tracing
- historical deployed Commit 2 evidence: Render validation on June 30, 2026 for campaign 8aa735ee-c02f-41e2-bb1f-7c3f43bb9458, property 542352127, and requested provider window 2026-06-19 through 2026-06-29 first returned provider.status = live_provider_error with 401 UNAUTHENTICATED; after Commit 3 and Commit 4 fixes, final Commit 2 validation on July 1, 2026 for requested provider window 2026-06-19 through 2026-06-30 and current-value window 2026-06-24 through 2026-06-30 returned `provider.status = live_provider_success`, `currentValueProvider.status = live_provider_success`, non-null provider totals, and exact stored-vs-scheduler deltas of `0` for both controlled Benchmark rows
- historical Commit 3 deployed validation: Commit 3 deployed validation passed with `provider.status = live_provider_success` and non-null provider totals for the same campaign/property/filter/requested window; the validation-only `simulateRefreshFailure=1` branch also passed on July 1, 2026 with `success = true`, `simulation.refreshFailure = true`, `provider.status = live_provider_refresh_failed`, `provider.totals = null`, and an explicit no-token-metadata-changed error
- historical Commit 4 deployed validation: validation-window alignment was implemented, pushed, deployed, and revalidated; `sourceWindows.currentValue` was `2026-06-24` through `2026-06-29`, `currentValueProvider.totals` was non-null, stored Benchmark Revenue `12376.38` equalled `schedulerCandidateCurrentValue` `12376.38`, and `storedVsSchedulerDelta = 0`; the `storedVsUiDelta = 0.01` was a one-cent provider-versus-persisted rounding difference, not proven Benchmark damage
- historical Commit 4 Follow-Up local fix: scheduler/report-preflight guards tracked updated Benchmark row IDs, failed closed for Benchmark-section reports whose selected rows were not recomputed, and prevented duplicate Benchmark history rows for the same Benchmark/date even when an older report date was reprocessed after newer history existed
...
- historical July production-readiness queue: complete for that packet; the later Benchmark Current Commits 8-14 are now complete
- not proven and not claimed: timer-fired executions beyond the exact `2026-08-12` recorded run, future GA4 provider availability outside the recorded windows, future GA4 processing delays, future alert email delivery, a real unsimulated Google token revocation event, future source mixes, and future platform Benchmark readiness

The historical July safe answer was:

`At the July 2026 checkpoint, GA4 Benchmarks were clean-certified for that recorded section boundary, with controlled deployed UI/email/provider/manual-scheduler/token-failure validation recorded. Do not extend this to later code, future GA4 properties/windows, future sends, real unsimulated token revocation, daily timer-fired evidence, or future platforms without fresh evidence.`
...
- completed fixes and controlled deployed evidence supported the historical July clean-certified GA4 Benchmarks decision for its exact documented boundary
- any clean-certified answer remains valid only while the complete value inventory, downstream propagation matrix, lifecycle matrix, negative cases, report consumers, alert/notification paths, cleanup boundary, and test/deployed evidence remain covered by current evidence
...
| Revenue | selected scoped GA4 native financial revenue + active GA4-context imported revenue | `ga4Revenue + importedRevenue`; Pipeline Proxy excluded; compatible campaign currency required | Native source uses the saved GA4 initial-import date through the latest completed reporting day; imported source-backed aggregation uses all available mapped records | Benchmark cards, ROAS, ROI, tracker, alerts, notifications, reports | Proven locally through Benchmark/financial/regression evidence; controlled deployed provider validation passed for the recorded window; future provider windows require fresh evidence |
...
| Scheduler refresh | GA4 daily scheduler refreshes campaign facts, recomputes only successfully refreshed campaign IDs through `runGA4DailyKPIAndBenchmarkJobs`, keeps any provider failure fatal in the process-wide status, and suppresses the unsafe global alert sweep on failure; source refresh paths run recompute before alert checks where immediate correctness is promised | Proven locally and by the exact `2026-08-12` natural-run evidence; future runs/configurations require fresh evidence |
...
| GA4 native metric refresh | GA4 daily/on-demand refresh writes selected campaign/property facts and then recomputes Benchmarks for successfully refreshed campaign IDs | Proven locally; controlled deployed provider/manual scheduler validation and the exact natural timer run passed for the recorded campaign/window; future provider windows and future timer runs require fresh evidence |
...
| Scheduler lifecycle | GA4 daily and source-refresh paths update persisted Benchmark values before alert checks; the daily path recomputes successfully refreshed campaign IDs while preserving any provider failure in the process-wide status | Proven locally, by the controlled manual trigger, and by the exact `2026-08-12` natural run; future runs require fresh evidence |
...
| Scheduler/runtime observability | `server/ga4-scheduler-observability-regression.test.ts`, `server/ga4-daily-scheduler-regression.test.ts`, focused partial-failure regression, deployed health, and exact natural-run evidence | `/health/scheduler` exposes GA4 daily scheduler status; successfully refreshed campaigns continue to recompute; provider failures remain fatal process-wide and suppress the unsafe global alert sweep; the exact natural run recorded zero in-scope campaign/KPI/Benchmark failed IDs | Exact `2026-08-12` timer evidence passed for the certified boundary; do not generalize to future runs or global all-campaign health |
| Auto-refresh and scheduler recompute | `server/ga4-auto-refresh-regression.test.ts`, `server/campaign-scheduler-current-value-regression.test.ts`, `server/ga4-kpi-financial-window-regression.test.ts`, and focused partial-failure regression | GA4 refresh/scheduler paths recompute successfully refreshed campaign IDs before covered consumers; scheduler updates Benchmark current values and does not insert duplicate same-date Benchmark history when older report dates are reprocessed | Manual validation and the exact natural run passed; future runs/configurations require fresh evidence |
...
The deployed validation endpoint correctly exposed live provider totals, persisted daily totals, financial inputs, and stored Benchmark values side by side. However, it used the manually requested provider window as the source for `schedulerCandidateCurrentValue` and `uiCandidateCurrentValue`. That overcompared stored Benchmark current values against a window the actual scheduler does not use. `runGA4DailyKPIAndBenchmarkJobs` now computes persisted GA4 Benchmark current values from the saved GA4 initial-import date through the selected complete end date, plus all available mapped imported financial records. Therefore the apparent `12376.38` versus `21922.96` mismatch was a validation-window defect, not proven damaged Benchmark data.
...
- Confirm `sourceWindows.currentValue` starts at the saved GA4 initial-import date and ends at `2026-06-29`.
...
- Deployed manual scheduler-validation trigger passed on July 1, 2026 for campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`: `success = true`, `beforeManualRuns = 1`, `afterManualRuns = 2`, `lastRunTrigger = manual`, `lastRunStatus = success`, and `lastError = null`. This proved the deployed campaign-scoped GA4 daily refresh plus KPI/Benchmark recompute pipeline could run on demand through the guarded validation trigger. It did not prove the timer at that historical checkpoint; the exact natural-run gate was later completed on runtime SHA `650ce59c4b0d14a21a198e8a2effd0c3a6d1fccd`.

Implementation status:

Implemented and validated for the scheduler/report-preflight code path, observability support path, and controlled deployed manual scheduler-validation path. Deployed manual snapshot/report-preflight validation already passed for the controlled GA4 Benchmark report. The observability support exposes GA4 daily scheduler state through `/health/scheduler`, makes report scheduler health metrics update on each check, adds a guarded read-only report send-event endpoint, and adds a campaign-access-guarded manual GA4 daily scheduler validation trigger. Deployed evidence on July 1, 2026 proved the report scheduler checker was running, a controlled GA4 Benchmark report send event was created, the GA4 daily scheduler timer was scheduled, and the guarded manual trigger successfully ran the deployed campaign-scoped GA4 daily refresh plus KPI/Benchmark recompute path. At that historical checkpoint, the timer itself was not separately proven; the exact natural-run gate was later completed on runtime SHA `650ce59c4b0d14a21a198e8a2effd0c3a6d1fccd`.
...
| Natural daily timer run | Exact revision-bound production evidence | Yes, for the recorded run only | The `2026-08-12T23:00:00.001Z` run on runtime SHA `650ce59c4b0d14a21a198e8a2effd0c3a6d1fccd` recomputed the active Benchmark campaign and both active Benchmark rows with zero in-scope failed IDs; 17 excluded obsolete campaigns kept the process-wide status failed | Any future run, configuration, provider window, or global all-campaign scheduler-health claim |
