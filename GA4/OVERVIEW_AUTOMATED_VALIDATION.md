...
For strictly read-only production investigations, do not run `overviewPack(...)`, `commit16Pack(...)`, `/ga4-daily`, provider-validation, or scheduler run-now routes: those paths can refresh OAuth tokens, persist daily rows, or run recomputation. Use authenticated guarded inventory GETs, database `SELECT` transactions, and public health endpoints only. The `2026-07-30` Current Commit 8 investigation and completed `2026-08-01` final-pack run followed this boundary. The user reviewed the four visible retained Spend sources totaling $2,698.75 and explicitly approved keeping all four, so their presence is not a damage finding. A later user-controlled post-publish Google Sheets reconnect and one no-click mapped-value update supplied bounded deployed continuity evidence without changing the read-only character of the inventory packet or authorizing cleanup. The deterministic/read-only pack passed; the later `2026-08-10` exact timer-fired target run and provider-usable connection evidence closed the remaining scheduled-run and OAuth durability gates for the certified boundary.
