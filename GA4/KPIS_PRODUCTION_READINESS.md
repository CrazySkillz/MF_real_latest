...
**Result: UNVERIFIED for exact reviewed/deployed SHA 3e65bbb4033a2843462f0122a5ff5f015a69fc64.** Current Commits 0–13 remain implemented. Commit 14 passed target-data inventory, the deployed six-state browser matrix, natural scheduler evidence, GA4 KPI alert delivery, and the current-version regression boundary. Commit 14A deployed successfully, but its one authorized exact-campaign recompute and post-recompute parity check failed. Commit 14B remains pending and must not begin until Commit 14A correction closes.
...
- exact-campaign Commit 14A execution on deployed SHA `3e65bbb4`: campaign hash `fc734ddaf728`, property `542352127`, Europe/Amsterdam completed day `2026-08-04`; one manual pipeline completed successfully at `2026-08-05T13:15:01.359Z`, alerts were suppressed, and no other campaign was touched. The post-run 30-day rows all had non-null exact `engagedSessions`, proving the migration and forward persistence path deployed
- validation safety caveat: the subsequent browser card check blocked non-GET requests, but the deployed `GET /api/campaigns/:id/ga4-daily` route can itself perform missing-date provider backfill and upsert. Production timestamps confirm that the exact target campaign's daily rows were refreshed again during the supposed read-only browser evidence capture. No other campaign, connection, recipient, schedule, alert, notification, or configuration was touched, but this browser evidence is not represented as read-only
...
1. **Provider/current-value parity — Commit 14A validation failed August 5, 2026:** deployed SHA `3e65bbb4033a2843462f0122a5ff5f015a69fc64` persisted non-null exact daily `engagedSessions` for all five populated rows in the completed 30-day window. Those rows contain 552 sessions and 377 engaged sessions (`68.30%`), and persisted KPI, progress, KPI API, alert resolver, and deployed KPI card all consistently show `68.30%`. The refresh-disabled live aggregate response has the same 553 users, 552 sessions, 930 pageviews, 68 conversions, and 11694.33 GA4 revenue, but its Engagement Rate is `68.47826086956522%`, representing 378 engaged sessions. Therefore the missing-column defect is fixed, but aggregate and date-dimension GA4 query shapes remain inconsistent by one engaged session. The read-only comparison endpoint also exposes a separate evidence defect by selecting the latest daily row's `69.37%` instead of the weighted daily total. The direct provider and database comparisons disabled token refresh/persistence and used a read-only transaction. However, the browser card evidence reached the mutation-capable `GET /ga4-daily` backfill path and refreshed the same target campaign's daily rows, so the overall post-recompute packet was not strictly read-only and cannot close the gate.
...
14A. **Correction required; do not rerun production yet.** First add a refresh-disabled diagnostic that exposes the exact aggregate `engagedSessions` numerator and the date-dimension numerator for the identical property, campaign filter, reporting timezone, start date, and completed end date. Correct the validation endpoint to use the weighted daily total rather than the latest row rate, and make the next browser validation mutation-proof by intercepting the daily response or using a dedicated read-only endpoint that cannot backfill. Then bind persisted recompute to one proven authoritative numerator without inventing or distributing a missing daily session, preserve valid-zero and legacy-null handling, and add a real-path mismatch regression. Only after that focused fix deploys may one newly authorized exact-campaign recompute repeat the same cross-consumer comparison. Do not broaden campaign, owner, property, filter, window, source, token, alert, report, or production-data scope.
