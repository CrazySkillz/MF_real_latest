...
- The chronological Overview queue is documented as Current Commits 11–18 in `GA4/OVERVIEW_PRODUCTION_READINESS.md`. Commit 16's bounded deployed 30-day saved-window correction is closed; OAuth seven-day durability remains unproven. Commit 17 deployed as `36676deb`, and the user confirmed existing Total Revenue, Total Spend, and source lists were unchanged, closing its bounded forward implementation while provider-cycle rollback and retained-source disposition remain open. Commit 18's bounded fail-closed downstream implementation is locally complete and validated but is not yet committed, deployed, externally validated, or clean-certified; named external and scheduler gates remain unproven.

- GA4 Overview is not production-ready or clean-certified. Current Commits 1-7 are closed for their documented bounded packets. Current Commit 8's deployed 30-day coverage/activity and UI-stability checks passed; live 90-day and current timer-fired evidence remain unproven. Current Commit 9's owner/campaign/source-scoped read-only inventory is complete: it found four actively growing orphan LinkedIn/Meta spend groups caused by redundant scheduler writes. Commit `57036ebc` deployed the forward-only fix without changing canonical platform metrics or visible source-backed totals. The immediate post-deploy inventory proved no growth from the final pre-deploy 325,538-row boundary; first four-hour scheduled-cycle proof remains required. Existing orphan rows and cached-spend drift remain unchanged pending that proof and separately authorized exact cleanup. Current Commit 10 deployed as `ec265895` and is closed for its bounded code/browser packet: `GA4 single` / `ga4_mock` Total Spend matched between GA4 Overview and Performance Summary, and Total Revenue matched between GA4 Overview and Budget & Financial Analysis → ROI & ROAS. Scheduled artifacts, historical Trend, live multi-source, and valid-zero/negative production fixtures remain unproven. The durable source of truth is `GA4/OVERVIEW_PRODUCTION_READINESS.md`.
...
- GA4 KPI historical certification evidence remains recorded, but Commit 18 changes the shared current-value fail-closed path. That affected path is reopened and unproven until the Commit 18 code is deployed and validated; use `GA4/KPIS_PRODUCTION_READINESS.md` for the exact boundary.
- GA4 Benchmark historical certification evidence remains recorded, but Commit 18 changes the same shared current-value fail-closed path. That affected path is reopened and unproven until the Commit 18 code is deployed and validated; use `GA4/BENCHMARKS_PRODUCTION_READINESS.md` for the exact boundary.
...
- Google Sheets spend readiness hold: the recurring disconnect root cause was confirmed as Google OAuth `External + Testing`; Publishing status is now `In production`. The post-publish GA4 reconnect completed and immediate UI validation passed on `2026-07-30`, but automatic renewal and more-than-seven-day durability remain unproven. The final Google Sheets reconnect, `mumus.app` public legal pages/domain ownership, Google branding/data-access verification, and the deployed automatic-mutation packet remain open. See `GA4/OVERVIEW_SPEND_PRODUCTION_READINESS.md`.
...
- `GA4/OVERVIEW_PRODUCTION_READINESS.md`
  Canonical whole-tab Overview production-readiness source of truth. Current status: not production-ready; Commits 1–17 are closed only for their documented bounded packets. Commit 17's provider-cycle rollback and retained-source disposition remain external/open. Commit 18 is locally implemented and validated but remains uncommitted, undeployed, and externally unproven; live 60/90-day, OAuth-durability, and named scheduler/external gates also remain. It also contains the future-platform template and readiness checklist for Meta, Google Ads, LinkedIn, and other sources.
- `GA4/KPIS.md`
...
- `GA4/OVERVIEW_PRODUCTION_READINESS.md` states the current not-production-ready Overview status, records the deployed Commit 2 validation, and provides the reusable future-platform readiness template
...
- live Overview Summary cards must import metrics from the selected GA4 property and saved campaign values as one coherent configured-lookback source: persisted selected-campaign daily facts when available, then same-window selected-campaign breakdown totals; campaign-to-date totals are reserved for the explicitly labeled financial section
...
- live same-window breakdown totals can feed the visible Overview cards when persisted daily rows are absent, so a live property with current UTM traffic does not render zero top-line metrics while populated tables exist
