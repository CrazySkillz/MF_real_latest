...
| Daily metrics path | `/api/campaigns/:id/ga4-daily` to persisted daily rows and optional scoped backfill. | Daily rows feed Summary fallback hierarchy without mirroring GA4 native revenue into imported revenue records; stale-but-nonempty persisted rows are now freshness-flagged and trigger due missing-day backfill attempts. | Proven locally and deployed-stale-warning validated for the affected campaign/property; live provider backfill remains blocked by a deployed HTTP `500` provider fetch failure. |
...
| GA4 daily stale-row freshness guard | `npm test -- server/ga4-reporting-day-cutoff-regression.test.ts`; `npm run check`; deployed diagnostic packet from `2026-07-03T07:48:10.650Z` identified stale persisted rows through `2026-06-30` while `dataThroughDate` was `2026-07-02`; deployed recheck on `2026-07-03T08:44:38.952Z` returned `refreshIsStale: true` and the UI displayed the stale daily-history warning. | The `/ga4-daily` route now treats stale-but-nonempty persisted rows as stale when due completed dates are missing, attempts a provider backfill for due missing daily rows, and preserves existing stored rows with `providerRefreshWarning` if the provider refresh fails. | Closed for deployed stale detection/warning behavior on the affected campaign/property; not closed for fresh GA4 backfill because `/ga4-timeseries` returned `AUTO_REFRESH_NEEDED`, persisted rows still ended at `2026-06-30`, and Render GA4 daily env had to be corrected separately. |
| GA4 timeseries reconnect response guard | `npm test -- server/ga4-reporting-day-cutoff-regression.test.ts`; `npm run check`; deployed console packet on `2026-07-03` showed `/ga4-timeseries` returned HTTP `500` with `error: AUTO_REFRESH_NEEDED` and no reconnect flag. | The `/ga4-timeseries` route now maps `AUTO_REFRESH_NEEDED` and `TOKEN_EXPIRED` to HTTP `401` with `requiresReauthorization: true` and a reconnect message, matching the GA4 daily route's auth-error semantics. | Local fix only until pushed/deployed and rechecked; does not refresh GA4 data or repair credentials. The required product action remains reconnecting GA4 for the affected campaign/property. |
...
| Current Commit 2e.1: GA4 daily stale-row freshness guard | Completed for deployed stale detection/warning behavior; live provider backfill remains unresolved. | `/api/campaigns/:id/ga4-daily` persisted daily rows that feed Overview/Insights trends. | Treat due missing completed dates as stale even when older stored rows exist; attempt a provider backfill for due missing dates; if provider refresh fails, return existing rows with explicit `providerRefreshWarning` and `refreshIsStale: true` instead of silently claiming freshness. | No for local code-scope certification; no for the deployed stale-warning behavior; yes for fresh-row restoration until GA4 is reauthorized and a successful provider/scheduler fetch writes rows beyond `2026-06-30`. |
| Current Commit 2e.2: GA4 timeseries reconnect response guard | Implemented locally after deployed console packet; awaiting deploy/revalidation. | `/api/campaigns/:id/ga4-timeseries` live provider path used for diagnosing/backfilling GA4 daily freshness. | Return HTTP `401`, `requiresReauthorization: true`, and a reconnect message for `AUTO_REFRESH_NEEDED` and `TOKEN_EXPIRED` instead of a generic HTTP `500`. | No for local code-scope certification after tests; yes for deployed response-shape closure until Render deploy plus console recheck. Does not remove the need to reconnect GA4. |
| Current Commit 2f: Google Sheets spend campaign/property portability | Outstanding deployed validation gate, not a confirmed code bug. | A second disposable campaign/GA4 property context with Google Sheets spend. | Repeat the Google Sheets spend lifecycle on a second campaign/property context with controlled amounts and source IDs: add/import, edit/update, run-now refresh/reprocess, and delete/deactivate, or explicitly record any action not exposed. | No for local code-scope certification; yes for multi-campaign/property certification. |
...
3. Next queued validation before other source families, if the goal is broader Google Sheets spend certification: Current Commit 2f for a second campaign/property context and Current Commit 2g for alternate Google Sheets tabs/mappings. If strict normal wall-clock scheduled-hour proof is required, capture that natural timer run before 2f.
...
- Normal wall-clock daily scheduler execution remains unproven if required beyond the startup-fired scheduler packet. The GA4 daily stale-row freshness fix is deployed-validated only for stale detection/warning behavior; fresh-row restoration remains blocked by `AUTO_REFRESH_NEEDED` until the affected GA4 connection is reauthorized and a provider/scheduler fetch writes rows beyond `2026-06-30`. The `/ga4-timeseries` reconnect response guard remains local until deployed and rechecked. Current Commit 2f other campaign/property validation, Current Commit 2g other Google Sheets tabs/mappings, Current Commit 2h report/email propagation, Current Commit 3 production source-damage inventory, and other source families remain unproven until their own evidence packets are captured.
