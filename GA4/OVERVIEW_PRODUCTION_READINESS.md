...
| Current Commit 2d: Google Sheets spend refresh validation trigger | Implemented, deployed, and validated for the recorded run-now source path. | One active Google Sheets spend source in one authorized campaign. | Add a campaign/source-scoped validation route that reuses the scheduler Google Sheets spend reprocess function for `/api/campaigns/:id/spend-sources/:sourceId/google-sheets-refresh/run-now`; do not run the full daily scheduler, unrelated providers, reports, emails, alerts, or unrelated campaigns. | No for local code-scope certification; no for the recorded Google Sheets spend run-now refresh/reprocess path; yes for daily scheduler timer execution and delete/deactivate. |
...
2. Next source family/action: validate Google Sheets spend delete/deactivate for source `8f67b03f-a00b-434f-b81f-db1b2b951595`.
...
5. For the next packet, delete/deactivate only the existing Google Sheets spend source, then capture before/after endpoint evidence and UI confirmation; do not add a replacement spend source in the same evidence packet.
...
- Google Sheets revenue delete/deactivate is closed only for the validated deployed campaign/source path recorded above. Current Commit 2c deployed latency timing is user-confirmed closed without a numeric SLA. Google Sheets spend add/import and edit/update are closed only for the validated deployed campaign/source path recorded above. Google Sheets spend run-now refresh/reprocess is closed only for the validated deployed campaign/source path recorded above. Google Sheets spend delete/deactivate remains unproven until it has deployed before/after endpoint evidence plus UI parity.
