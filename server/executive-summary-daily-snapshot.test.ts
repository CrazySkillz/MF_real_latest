...
const performanceSummary = (endDate: string, revenue: number) => ({
  ...
  totals: {
    ...
    cvr: { value: 12.89, available: true, sources: ["conversions", "sessions"] },
    cpa: { value: 17.76, available: true, sources: ["spend", "conversions"] },
    roas: { value: 18.92, available: true, sources: ["revenue", "spend"] },
    roi: { value: 1791.77, available: true, sources: ["revenue", "spend"] },
  },
});
...
describe("Executive Summary daily snapshot", () => {
  it("copies the exact authoritative values and source contract", () => {
    const snapshot = build("2026-08-25", 51072.99);
    expect(snapshot.reportingDate).toBe("2026-08-25");
    expect(snapshot.currency).toBe("USD");
    expect(snapshot.ga4PropertyId).toBe("542352127");
    expect(snapshot.totals.revenue).toEqual({ value: 51072.99, available: true, sources: ["ga4", "revenue_csv"] });
    expect(snapshot.totals.roi.value).toBe(1791.77);
  ...
  it("classifies only an exact seven-day compatible comparison", () => {
    const result = evaluateExecutiveSummaryTrajectory(
      row(build("2026-08-25", 51072.99)),
      row(build("2026-08-18", 44000)),
    );
    expect(result.available).toBe(true);
    expect(result.trajectory).toBe("accelerating");
    expect(result.trendPercentage).toBeCloseTo(16.075, 3);
  });

  it("fails closed for missing or incompatible history", () => {
    const current = build("2026-08-25", 51072.99);
  ...
  it("does not invent a trajectory when revenue history is unavailable", () => {
    ...
    expect(evaluateExecutiveSummaryTrajectory(row(build("2026-08-25", 51072.99)), row(previous)))
