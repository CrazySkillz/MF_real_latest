...
describe("Instagram Connected Platforms regression guard", () => {
  ...
  it("renders Instagram analytics Overview from the selected daily metrics endpoint only", () => {
    ...
    expect(page).toContain('label: "Total Revenue"');
    expect(page).toContain('hasInstagramAttributedRevenue ? formatCurrency(instagramAttributedRevenue) : "Not connected"');
    expect(page).toContain('label: "ROAS"');
    expect(page).toContain('label: "ROI"');
    expect(page).toContain('label: "Profit"');
    expect(page).toContain("Connect attributed revenue");
    expect(page).toContain("Imported Instagram attributed revenue");
  ...
  it("resolves Instagram Overview revenue only from Instagram-scoped revenue sources", () => {
    const modal = readFileSync(join(process.cwd(), "client", "src", "components", "AddRevenueWizardModal.tsx"), "utf-8");
    ...
    expect(page).toContain("/api/campaigns/${campaignId}/revenue-sources/${encodeURIComponent(sourceId)}");
    expect(page).toContain('label: "Total Revenue"');
    expect(page).toContain('hasInstagramAttributedRevenue ? formatCurrency(instagramAttributedRevenue) : "Not connected"');
    expect(page).toContain('label: "ROAS"');
    expect(page).toContain('label: "ROI"');
    expect(page).toContain('label: "Profit"');
    expect(page).not.toContain("resolveInstagramRevenue");
  });
});
