...
**Current status: UNVERIFIED.** Deployed runtime `340da6d5831f77fb88d125a55aaec78c189c4c3d` passed exact-current authenticated persisted-data/API/UI/source/PDF parity and production-data integrity for the recorded configuration; direct database checks ran inside a read-only transaction. The 34-file / 433-test focused packet, TypeScript, and production build also pass. Only the exact-current natural `22:00 UTC` daily-timer observation remains open. The exact controlling status and exclusions remain in `GA4/OVERVIEW_PRODUCTION_READINESS.md`.
...
| Campaign Breakdown | Fixed import-to-date saved-campaign traffic rows plus a separate exact campaign-to-date native GA4 revenue query, followed by exact mapped imported campaign revenue | traffic: initial-import boundary through latest completed day; native revenue: campaign start through that completed day; matched imports: source-to-date | Current candidate is locally regression-covered but not deployed. Prior output used `$43,668.50` fixed-window native revenue; the corrected native target is the `$65,362.20` GA4 Revenue card, then `$16,799.99` exact mapped imports, for an expected displayed total of `$82,162.19` on the recorded data. |
