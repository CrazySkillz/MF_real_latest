...
Whole-Overview Commit 17 is locally implemented. Google Sheets Revenue daily reprocess and Google Ads/Meta Spend daily reprocess now replace last-good records inside the existing exact campaign/source/type/context transaction boundary; individual Spend deletion now deactivates the source and deletes only its same-campaign records in one transaction. Provider reads, saved campaign selection, calculations, cadence, recompute order, response shapes, and production records are unchanged. Deployment and real scheduler/provider-cycle rollback evidence remain unproven.
