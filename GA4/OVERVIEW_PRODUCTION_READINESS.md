...
| Current Commit 2g: Google Sheets mapping variant automated pack | Implemented as validation tooling; deployed fixture evidence pending until the runner is deployed and executed against chosen fixture sources. | Configured Google Sheets revenue/spend tab and mapping-shape evidence for already-created fixture sources; not another basic lifecycle retest. | Use `GA4OverviewValidation.googleSheetsVariantPack(...)` to validate source IDs, expected amounts, active state, duplicate active Google Sheets mapping signatures, and persisted mapping metadata for configured fixture variants. After deploy, run it only against the selected fixture variants and pair the output with UI parity for the affected card/source modal. | No for local code-scope certification; yes only for configured tab/mapping breadth claims until deployed output and UI confirmation are captured. |
...
| Current Commit 3: Production source-damage inventory | Implemented locally as a campaign-access-guarded read-only inventory route plus browser-runner command; production output still must be captured after deploy. | Production/staging inventory for orphan, duplicate, inactive, or platform-context-drifted revenue/spend records outside the already documented synthetic-GA4-revenue cleanup boundary. | Run `GA4OverviewValidation.sourceDamageInventory({ campaignId })` in a logged-in deployed browser session. If `overallPass: false`, document exact returned source/record IDs before proposing any cleanup migration. | No for local code-scope certification; yes for production database-health certification until deployed inventory output is recorded. |
...
6. Run Current Commit 3 with `GA4OverviewValidation.sourceDamageInventory({ campaignId })` after deploy before any cleanup claim; do not delete or rewrite rows until the exact damaged-data boundary is documented.
