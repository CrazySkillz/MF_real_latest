...
Executive Summary KPI Progress should render current values, progress percentages, and statuses from live GA4/connected-source aggregate values when a campaign KPI maps to an available page-level `performanceSummary.totals` metric, such as users, sessions, conversions, revenue, ROI, ROAS, CTR, or CVR. Saved KPI progress/current values are fallback only when the KPI cannot be mapped to an available aggregate metric.
