...
Production-readiness status lives in `GA4/OVERVIEW_PRODUCTION_READINESS.md`. Current status: **UNVERIFIED**. Current Commit 20 restored initial-import-plus-accumulation, but its first deployment used OAuth `connectedAt` for retained rows and included three pre-window days. The local corrective follow-up uses the verified `2026-07-02` cutover only for connections that predate `importStartDate`; new connections retain their exact property-selection boundary. Deployment/parity and the remaining external gates are still open.
...
Remaining Commit 8 read-only evidence on `2026-07-30`: the validated account has no suitable live 90-day fixture. Its three active, primary, non-empty 90-day connections use property `yesop`, which the application explicitly treats as simulated; `Summer splash` is a separate empty-Property-ID placeholder and correctly fails closed. `/health/scheduler` showed the current process healthy and scheduled for `2026-07-31T03:00:00.000Z`, but `totalScheduledRuns` was `0`. No provider, token-refresh, scheduler-run, reconnect, campaign, connection, or stored-data mutation was triggered. Live 90-day and current timer-fired evidence remain unproven.
...
Current queue status on `2026-08-01`: Commit 16's bounded deployed 30-day saved-window correction is closed; OAuth seven-day durability remains unproven. Commit 17 deployed as `36676deb`, and the user confirmed existing Total Revenue, Total Spend, and source lists were unchanged; its bounded forward implementation and retained-source disposition are closed, with rollback behavior regression-covered and no unsafe production failure injection required. Commit 18's fail-closed downstream implementation and deterministic downstream packet are closed. Its corrective scheduler-backed Summary deployed as `e857c15d`; on existing campaign `GA4 single` / `ga4_mock`, the rendered 30-day Summary and exact `/ga4-daily` response both returned 866 Sessions, 867 daily-summed Users, 110 Conversions, 68.4% Engagement Rate, and 12.7% Conversion Rate with `refreshIsStale: false`. Formal clean certification remains pending current-release timer-fired GA4 scheduler validation and OAuth durability; GA4 processing finality is not claimed.

Current Commit 19 runtime `ba2e4329` deployed. Both GA4 setup surfaces expose only 30 days; both persistence APIs reject new missing/60/90-day configuration before provider or storage mutation; and retained non-30 connections fail closed without record mutation. The user confirmed `Last 30 completed days` and normally loaded metrics on existing `GA4 single` / `ga4_mock`; the bounded packet is closed. Unsupported-write rejection is automated/code-path proven and was not production-injected. The user approved keeping all four visible retained Spend sources. The post-publish Google Sheets reconnect and one no-click mapped-value update were user-confirmed on `2026-08-01`; that automatic-update gate passes. One normal timer-fired GA4 daily scheduler run on the current deployed release and OAuth durability on `2026-08-07` or later remain open.
...
- GA4 KPI/Benchmark financial values must use the same HubSpot imported revenue and selected GA4 native financial source included in Overview `Total Revenue`, with Pipeline Proxy excluded. HubSpot Revenue is **UNVERIFIED on the current revision**; `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md` is canonical and H10d is historical only.
- Shopify imported revenue follows the same GA4 source-backed financial model. Its current readiness status is **UNVERIFIED on the current revision**; use `GA4/OVERVIEW_REVENUE_SHOPIFY_PRODUCTION_READINESS.md` as the controlling source. The 2026-07-15 packet is historical, and dormant OAuth/non-GA4 sources remain excluded.
