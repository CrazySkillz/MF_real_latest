...
Trend Analysis renders one comprehensive executive view instead of separate tabs. The view combines the Campaign-to-Date Performance Summary, Campaign Performance Trend, Efficiency Trends, conditional Anomaly Detection, Website Engagement & Conversion Summary, conditional paid/source detail, and evidence-gated Executive Recommendations without duplicating the same values across panels. Its browser selector defaults to `7 days`; the `7/14/30/90-day` options control the chart window and exact cumulative comparison date, but do not replace authoritative cumulative current totals with rolling-window totals. The shared Trend PDF uses its separate fixed 30-day calendar contract.
...
- Executive Summary no longer displays Campaign Grade, Health Score, Campaign Story, duplicate Platform Performance, or the full Risk Assessment card. Compatible 7-day trajectory and the top Risk Level badge remain visible; retained backend response fields preserve API compatibility only.
