...
const readPersistenceFingerprint = async (client: any, campaignId: string) => {
  const result = await client.query(`
    SELECT
      ...
      (SELECT md5(COALESCE(jsonb_agg(CASE WHEN x.source_type = 'google_sheets' AND x.mapping_config IS NOT NULL THEN (to_jsonb(x) - 'mapping_config') || jsonb_build_object('mapping_config', (mapping_config::jsonb - 'lastSyncedAt')::text) ELSE to_jsonb(x) END ORDER BY x.id)::text, '[]')) FROM (SELECT * FROM spend_sources WHERE campaign_id = $1) x) AS spend_sources,
      (SELECT md5(COALESCE(jsonb_agg(to_jsonb(x) - 'id' - 'created_at' ORDER BY x.spend_source_id, x.date, x.source_type, x.sub_campaign_urn, x.spend, x.currency)::text, '[]')) FROM (SELECT * FROM spend_records WHERE campaign_id = $1) x) AS spend_records,
