...
- Executive Summary's corrected sections should be understood as follows: funnel and key metric cards come from page-level `performanceSummary.totals`; KPI Progress and Benchmark Comparison use campaign records for rows/targets and page-level `performanceSummary.totals` for current values; Risk Assessment uses the fixed six risk inputs; Strategic Recommendations use source-capability gating so paid-media guidance requires paid-media main sources plus available spend, revenue, ROI, and ROAS inputs. GA4-only web/outcome recommendations should also state whether KPI or Benchmark targets exist for CVR, revenue, and conversions before judging quality.
