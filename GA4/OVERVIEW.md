...
- Campaign Breakdown row `Sessions`, `Users`, and `Conversions` retain the fixed import-to-date GA4 values and, for exact UTM reconstruction, sum the same daily provider grain persisted by Summary so their row totals reconcile without scaling or allocation; native `Revenue` uses the separate campaign-start-to-latest-completed-day GA4 query and is not scaled or allocated
