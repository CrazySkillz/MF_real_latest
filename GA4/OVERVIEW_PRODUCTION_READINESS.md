...
Next active HubSpot step: capture deployed HubSpot email delivery evidence for the configured GA4 Overview Report, or proceed to Current Commit 4.15 HubSpot other-campaign portability if email delivery is intentionally deferred. Deployed email proof requires provider acceptance plus actual inbox receipt or provider delivery-event evidence; provider API acceptance alone is not delivery proof.
