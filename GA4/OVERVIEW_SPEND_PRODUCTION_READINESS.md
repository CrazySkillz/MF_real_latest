...
- **Google Sheets spend has current deployed continuity evidence but is not independently clean-certified for its full future provider/scheduler lifecycle.** On `2026-07-11`, the repaired manual run-now path returned `200/success:true` and visibly ingested a new value for source `618e5e12-0f3f-44a2-837a-d2677ad95f64`. On `2026-08-01`, the user completed the post-publish reconnect, changed the mapped sheet, left Overview open without manually refreshing, and confirmed the displayed value updated automatically. This closes stale reconnect and basic no-click propagation for the exact whole-Overview boundary. The chat did not record the exact changed amount, source-count parity, generalized cadence, or a failure-injection result, so those broader claims remain excluded.
...
- Current Commit 12's historical chooser packet is committed as `23a4fdc0`. Whole-Overview Commit 5 (`5da5f41c`) later hid Google Sheets temporarily. Current Commit 21 restored Google Sheets beside Google Ads and Upload CSV through the existing campaign-scoped atomic process path, and that chooser code is deployed in runtime `8ba694060411a2a05663a4915652767e4e3ba713`; retained-source reconnect and one automatic-update observation remain bounded evidence.
