...
- Google Sheets/Upload CSV revenue readiness: Current Commit 10 has bounded user-confirmed deployed normal CSV add/repeated-edit/delete evidence for `$150` / `$1,250` / `$150`, no duplicate source, Revenue baseline restoration, and unchanged Spend. Current Commit 12 retraced the CSV lifecycle, passed 44/44 bounded local tests, passed runner syntax and TypeScript checks, and refreshed the read-only target inventory with no active/reconciliation damage. Upload CSV Revenue is not clean-certified because no deployed invalid-file, complete runner/source-ID/endpoint-parity, or full derived-card packet was supplied. Google Sheets Revenue remains on hold and is not clean-certified. Use `GA4/OVERVIEW_REVENUE_PRODUCTION_READINESS.md` for the exact boundary.
