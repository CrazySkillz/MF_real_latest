...
**UNVERIFIED for current production certification at user-confirmed deployed runtime `82fc3a7887d14e370e29a27ae4349333bacc2f58`.**

Current candidate evidence captured on `2026-08-13` proves:
...
- deployed `82fc3a78` labels those two windows explicitly; the regression-first wording guard failed before the runtime edit and passed in the 40/40 focused UI regression file afterward
...
The sole remaining external gate is one natural `22:00 UTC` target-persistence observation under the current scheduler configuration. Until that passes and is recorded, this candidate must not be called release-candidate ready or production-ready.
