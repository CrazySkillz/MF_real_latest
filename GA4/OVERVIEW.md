...
- connected-source financial totals are not a Summary fallback because they have a different source-combination contract
...
- every initial-import-to-latest-completed-day native GA4 financial request must verify that the response metadata currency equals the requested campaign currency; missing or mismatched currency fails closed
...
- persisted daily and configured-lookback breakdown revenue have no historical currency provenance, so they remain native-only continuity fallbacks and must not be combined with imported revenue when the verified imported-window provider value is unavailable
...
- the `Total Revenue` Revenue Sources modal should not repeat a HubSpot provider or mapped-campaign subtitle beneath `HubSpot (Deals)`; HubSpot campaign mappings remain attribution metadata, while Shopify retains its mapped campaign label on the item row
...
- GA4 KPI/Benchmark financial values must use the same HubSpot imported revenue and selected GA4 native financial source included in Overview `Total Revenue`, with Pipeline Proxy excluded. The HubSpot contribution path is clean-certified only for the five exact active GA4 sources and exercised configurations recorded in `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md`; whole-tab readiness remains separately controlled.
...
- GA4-native financial revenue and CPA conversions use the first complete source in this fixed order: initial-import-to-latest-completed-day provider totals, compatible persisted daily totals, then the configured-lookback breakdown only when both earlier sources are absent; values are never selected by maximum revenue
- valid zero and negative imported-window native values remain authoritative; a provider response without both revenue and conversions is treated as empty and falls through to the next complete candidate
...
- every active exact GA4 Salesforce or HubSpot mapping with selected values is reprocessed through its stable source ID every five minutes by default, whether Pipeline Proxy is enabled or disabled; a saved stage is required only for a pipeline-enabled source. An open Overview checks saved Pipeline results every minute, and the same atomic provider refresh moves a newly won Opportunity or deal out of Pipeline Proxy and into confirmed Total Revenue and source provenance
...
- the visible subtitle states `Traffic metrics are cumulative from the initial GA4 import; Revenue combines native GA4 revenue from the imported data window with exact campaign-mapped imported revenue.`
- `Campaign Breakdown` revenue starts with exact initial-import-to-latest-completed-day GA4 revenue attributed to each saved GA4 campaign row; the native row sum must reconcile to Campaign Breakdown's own provider total before rendering. The currently certified Campaign2 result also independently reconciles to the protected GA4 Revenue card, but that cross-endpoint comparison is deployed evidence rather than a UI availability dependency
- Campaign Breakdown row `Sessions`, `Users`, `Conversions`, and native `Revenue` retain the fixed import-to-date GA4 window and are not scaled or allocated
...
- every exact mapped imported record is included, and the amount is added only by exact saved campaign mapping
...
`Landing Pages` is a cumulative view from the fixed initial-import boundary through the latest completed day for the GA4 property and campaign selection configured for this app campaign.
...
`Conversion Events` follows the same cumulative fixed-boundary rule as `Landing Pages`.
