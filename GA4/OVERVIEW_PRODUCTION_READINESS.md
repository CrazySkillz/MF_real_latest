...
**Current section status: UNVERIFIED.** The multi-campaign daily UTM aggregation path has changed since the recorded Campaign Breakdown certificate and is unverified until the affected scheduler, Summary, KPI/Benchmark propagation, Campaign Breakdown, reports, and production Campaign3 evidence are rerun. Conversion Events is **PRODUCTION_READY only for the exact boundary** in [`OVERVIEW_CONVERSION_EVENTS_CERTIFICATION_2026-09-15.md`](./OVERVIEW_CONVERSION_EVENTS_CERTIFICATION_2026-09-15.md): application runtime `176d02ef00a65091917136f1ed2bd8c7cc817fcb`, deployed with documentation-only revision `8dd0856bc2b63a65f56e90f45f8ec562b881f6fc`. The complete Overview remains fail-closed while Landing Pages populated-row production accuracy, Revenue, Spend, and the natural scheduler gate retain their separate validation status.

2026-09-15 Conversion Events certification: the exact saved campaign/property scope, saved initial-import boundary through the latest completed reporting-timezone day, complete provider pagination, ordered exact attribution fallback, positive-conversion-only output, four UI/PDF columns, access isolation, failure/last-good behavior, refresh behavior, and API/UI/browser-PDF parity passed for the recorded production fixture. The authenticated result was `purchase` with 295 Conversions, 295 Event Count, and 295 Users. Scheduled/server PDF mapping and failure behavior passed deterministic tests; an actual scheduled delivery was not run or claimed. Older 193-conversion observations below remain historical evidence for their own revisions.

2026-09-07 historical Campaign Breakdown revenue correction: deployed output combined `$43,668.50` native revenue from `2026-07-02` through `2026-09-06` with `$16,799.99` exact mapped imports, producing `$60,468.49`, while another financial path queried `2026-06-24` through `2026-09-06` and returned `$65,362.20`. Runtime `08d7abe5` aligned those observed paths and authenticated production validation confirmed 1,505 Sessions, 1,507 Users, and 193 Conversions; native revenue of `$20,315.40`, `$26,935.20`, and `$18,111.60`; exact mapped imports of `$99.99`, `$16,100.00`, and `$600.00`; and displayed revenue of `$20,415.39`, `$43,035.20`, and `$18,711.60`, totaling `$82,162.19`. This dated evidence does not define the current boundary: current native Revenue always starts at the saved GA4 initial-import date, and campaign metadata dates are never substituted.
...
- separately labeled connected-source financial values
...
| Current Campaign Breakdown audit | **Passed** | Runtime `08d7abe5` passed authenticated API/UI/browser-PDF value parity, access isolation, and read-only production-data integrity for Campaign Breakdown. The full current-version suite passed 1,727 tests with zero blocking failures. |
| Current Conversion Events audit | **Passed for the exact certified boundary** | Runtime implementation `176d02ef` passed the complete section-specific gate recorded in `OVERVIEW_CONVERSION_EVENTS_CERTIFICATION_2026-09-15.md`; the documentation-only deployed head is `8dd0856b`. This does not certify the whole Overview or Reports lifecycle. |
| Remaining Overview revalidation | **Pending** | Landing Pages populated-row production accuracy, Revenue, Spend, and the natural `22:00 UTC` scheduler run retain separate validation gates; section-specific evidence does not certify them. |
