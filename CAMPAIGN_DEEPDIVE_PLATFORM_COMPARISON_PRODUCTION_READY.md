...
- Yesop/mock GA4 source rows mirror the current GA4 contract: traffic uses the saved initial-import window, native financial Revenue uses the ordered saved-import-window source selection, and imported Revenue/Spend use every available mapped record. Values are never selected merely because they are larger.
...
- Commit 2 follow-up, as currently superseded: the GA4 mock/test aggregate now follows the same saved-import traffic and native financial boundaries as GA4 Overview, with all mapped imported Revenue/Spend included. It does not select an incompatible source merely because its value is larger; users remain non-additive across daily rows.
