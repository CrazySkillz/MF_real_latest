...
**GA4 Overview is not production-ready and is not clean-certified at deployed commit `747192ff`.**
...
| --- | --- | --- | --- |
| Sessions | configured-lookback persisted daily totals; same-window breakdown response when daily rows are absent | selected connection's 30/60/90 completed-day lookback | Commit 16 proves the deployed connection response supplies the saved 30-day window for the validated campaign; 60/90-day variants and provider-value parity remain open. Commit 3 distinguishes failed fallback requests from successful zero/empty responses locally. |
...
### Current Commit 17 — Retained-source lifecycle and production-data disposition — smallest forward fix locally implemented; external evidence/disposition open
...
- Remaining boundary: the forward code is locally implemented, not deployed or provider-cycle validated. The eight retained sources are not silently certified or cleaned. Commit 17 cannot be fully closed until the implemented paths receive bounded deployed evidence and each retained source is proven or separately dispositioned; any cleanup follows read-only dry run, exact owner/campaign/source boundary, explicit authorization, reviewed apply, and post-apply inventory.
