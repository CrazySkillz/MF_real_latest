...
- **Google Sheets spend has current deployed continuity evidence but is not independently clean-certified for its full provider/scheduler lifecycle.** On `2026-07-11`, the repaired manual run-now path returned `200/success:true` and visibly ingested a new value for source `618e5e12-0f3f-44a2-837a-d2677ad95f64`. On `2026-08-01`, the user completed the post-publish reconnect, changed the mapped sheet, left Overview open without manually refreshing, and confirmed the displayed value updated automatically. This closes the stale reconnect and basic no-click propagation blockers for the whole-Overview non-scheduler decision. The chat did not record the exact changed amount, source-count parity, or a failure-injection result, and credential durability remains pending.
...
- Current Commit 12's historical chooser packet is committed as `23a4fdc0`. It was later superseded by whole-Overview Commit 5 (`5da5f41c`): the current new-source Spend chooser exposes only Google Ads and Upload CSV. Google Sheets remains available only for exact existing-source continuity; that retained path now has post-publish reconnect and one automatic-update observation, while broader source-family re-enablement remains deferred.
...
Do not call the complete Google Sheets spend provider/scheduler lifecycle clean-certified until the exact remaining source-family evidence below is closed. This narrower source-family boundary does not revoke the whole-Overview 30-day non-scheduler pass recorded in `GA4/OVERVIEW_PRODUCTION_READINESS.md`.
...
| Current Commit 8b: Automatic Google Sheets spend propagation | **Committed, pushed, deployed; bounded no-click update observed.** | Replace the once-daily-only Google Sheets spend freshness gap without increasing any other source-family cadence. | Commit `e4c3de5a` adds a 1-minute source-only timer and 15-second Overview polling. One deployed no-click mapped-value update was user-confirmed on `2026-08-01`; exact timing/delta and failure evidence remain outside that observation. | The bounded Google Sheets automatic-update gate is closed; whole-Overview certification still requires the current-release GA4 daily scheduled-run gate and OAuth durability. |
| Current Commit 8c: Production Google OAuth publishing and verification | **Publishing and both post-publish reconnects complete; durability remains open.** | Eliminate Testing-mode seven-day token expiry for Google Sheets and GA4. | GA4 reconnect completed `2026-07-30`; Google Sheets reconnect completed `2026-08-01`. Capture more-than-seven-day durability on `2026-08-07` or later. Broader Google verification remains a separate external scope. | Yes only for the remaining durability gate. |
| Current Commit 9: Additional Google Sheets tab/mapping variants | **Optional and deferred for later with the Google Sheets hold.** | Unlisted tabs, headers, dates, filters, and mappings. | Run one bounded packet per mapping family after the user resumes Google Sheets work. | Only blocks broader mapping claims. |
...
GA4 Overview Google Sheets spend has deployed current-value, retained-source, post-publish reconnect, and one no-click automatic-update observation. Its full provider/scheduler source-family certification still excludes exact timing/delta/failure and more-than-seven-day durability claims. The bounded Google Sheets automatic-update gate is closed; the separate whole-Overview current-release GA4 daily scheduled-run gate remains open. Upload CSV retains its bounded deployed dated-Alpha lifecycle packet; new GA4 CSV sources are dated-only by automated UI/API guards. Existing undated sources, unusual/unlisted CSV shapes, deployed forged-request behavior, and additional inventories remain unproven. Google Ads and all other source families remain excluded from this component certification.
