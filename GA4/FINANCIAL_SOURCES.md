...
- Shopify OAuth must not infer or guess its callback URL from request host, browser origin, or Render proxy headers. Shopify requires the OAuth `redirect_uri` to exactly match the callback URL whitelisted in the Shopify app. Production Shopify OAuth should resolve from the deployed app base URL first (`APP_BASE_URL`, then `RENDER_EXTERNAL_URL`) using `/api/auth/shopify/callback`; Shopify-specific values (`SHOPIFY_REDIRECT_URI`, `SHOPIFY_APP_BASE_URL`) are fallback only when no app base is configured.
