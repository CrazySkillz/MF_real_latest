...
- Before Strategic Recommendations are considered production-ready, Executive Summary's already-corrected sections should be understood as follows: funnel and key metric cards come from page-level `performanceSummary.totals`; KPI Progress and Benchmark Comparison use campaign records for rows/targets and page-level `performanceSummary.totals` for current values; Risk Assessment uses the fixed six risk inputs; Strategic Recommendations still require source-capability gating.
