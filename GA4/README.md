...
- Certification integrity rule: GA4 readiness is valid only for the exact certified SHA/configuration/dependency boundary. A relevant upstream or downstream change automatically invalidates affected tabs. The GA4 KPI record is `GA4/certifications/ga4-kpis.json` and its checker is `npm run check:ga4-kpi-certification`. Current Commits 1-4 implement the fail-closed gate, dynamic cross-consumer guard, metric identity contract, and authoritative traffic/rate window contract while the record remains `UNVERIFIED`; Current Commits 5-10 and external validation remain open. Copied formulas and source-text guards are structural evidence only.
...
- GA4 KPI whole-tab status is not production-ready and remains `UNVERIFIED`; `GA4/KPIS_PRODUCTION_READINESS.md` is controlling. Current Commit 4 locally aligns persisted traffic/count/rate and notification inputs to 30 completed campaign-timezone reporting days with weighted Engagement Rate, but Current Commits 5-10 and all listed external gates remain open.
