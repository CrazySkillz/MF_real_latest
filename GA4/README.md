...
- GA4 Overview is not production-ready or clean-certified. Current Commits 1-7 are closed for their documented bounded packets. On `2026-07-30`, the user deleted only the reviewed `Summer splash` `$400` Manual Spend source; Total Spend became `$14,045.83`, and the post-delete owner-scoped inventory confirmed only the unchanged Google Ads source remains for that campaign. Current Commit 8 is next. The durable source of truth is `GA4/OVERVIEW_PRODUCTION_READINESS.md`.
