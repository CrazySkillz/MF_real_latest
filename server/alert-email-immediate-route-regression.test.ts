...
describe("immediate alert email route durability regression guard", () => {
  it("schedules GA4 create processing before return while updates retain durable immediate attempts", () => {
  ...
  it("keeps immediate send eligibility gates before any sendable audit claim", () => {
    ...
    for (const immediateSource of [kpiImmediate, benchmarkImmediate]) {
      expectBefore(immediateSource, "if (!campaignName) return false;", "const retryLease = retryClaim ? await this.claimDueAlertEmailRetry(retryClaim) : null;");
      expectBefore(immediateSource, "if (!Number.isFinite(currentValue) || !Number.isFinite(thresholdValue)) return false;", "const retryLease = retryClaim ? await this.claimDueAlertEmailRetry(retryClaim) : null;");
      expectBefore(immediateSource, "if (!this.shouldSendAlert(currentValue, thresholdValue, condition)) return false;", "const retryLease = retryClaim ? await this.claimDueAlertEmailRetry(retryClaim) : null;");
      expectBefore(immediateSource, "if (recipients.length === 0) return false;", "const retryLease = retryClaim ? await this.claimDueAlertEmailRetry(retryClaim) : null;");
      expectBefore(immediateSource, "const retryLease = retryClaim ? await this.claimDueAlertEmailRetry(retryClaim) : null;", "const emailSent = await emailService.sendAlertEmail(recipients, {");
    }
  });
