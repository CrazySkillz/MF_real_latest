...
describe("Google Ads revenue Google Sheets flow", () => {
  it("admits Google Ads for Google Sheets revenue routes through the shared platform validator", () => {
    ...
    expect(routes).toContain('const zSheetsRevenuePlatformContext = z.enum(["ga4", "linkedin", "meta", "google_ads", "instagram"]);');
  ...
  it("uses Google Ads revenue purpose and keeps imports revenue-only", () => {
    ...
    expect(processRoute).toContain("platformContext === 'google_ads' ? 'google_ads_revenue' : platformContext === 'instagram' ? 'instagram_revenue' : 'revenue'");
  ...
  it("passes Google Ads context, purpose, source identity, campaign scope, and date mapping from the shared wizard", () => {
    ...
    expect(sheetsAuth).toContain("purpose?: 'spend' | 'revenue' | 'general' | 'linkedin_revenue' | 'meta_revenue' | 'google_ads_revenue' | 'instagram_revenue';");
