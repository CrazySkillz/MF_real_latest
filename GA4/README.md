...
- GA4 Insights Trends history gating is mode-specific: `Daily` needs 2 daily rows; `7d` and `30d` require complete calendar windows for chart values and two complete adjacent windows for comparison; `Monthly` can show one calendar month, marked partial when incomplete, but needs two complete adjacent months for comparison. Pre-creation imported history is excluded from Trends only.
