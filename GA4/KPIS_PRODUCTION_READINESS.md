...
- Commit 10B partially evidenced the deployed state matrix on revision `bc0d0f73d2980ba16df71ae382546aa3d2974c09`: 23 GA4 KPI parent rows were inventoried read-only. Nineteen belong to existing campaigns and include unavailable and last-good candidates, mixed sources, timezones/windows, thresholds, and linked notification state. Four legacy parent rows reference one deleted campaign; current campaign access/list paths fail closed and their notification candidates are hidden, but the persisted orphan-parent boundary remains unresolved. Gate 2 also remains pending because production has no authoritative valid-zero, blocked, insufficient-data, or safely observable failure fixture, and no authenticated mutation-proof browser capture was available; the user previously confirmed only the browser-PDF happy path
...
- the Current Commit 10 dry-run found 0 cleanup candidates and applied 0 changes. Commit 10A now closes target-data Gate 1 for exhaustive inventory and classification: its 17 skips are 10 financial KPI rows plus 7 immutable duplicate-email audit rows. Three simulated-property rows are deterministically verified, three propertyless rows are proven unavailable rather than assigned invented values, and four live-property rows are fully inventoried. Their current provider totals remain pending under the separate provider/token gate
...
10. **Full validation and re-certification — performed; certification remains withdrawn.** Local GA4 packets, TypeScript, build, deployed numeric migration, target dry-run, and read-only orphan inventory were completed. Commit 10A closes only the target-data inventory gate by exhaustively classifying all 17 skips. The certification record remains `UNVERIFIED` because repository CI is not clean and the deployed state, timer, provider/token, and report/browser gates are not fully evidenced.
...
Status: consolidated Gates 2-6 validation performed; certification remains withdrawn (`UNVERIFIED`). Gate 2 passed. Gates 3-5 remain pending only at the exact external boundaries recorded below; Gate 6's GA4 evidence passed while repository-wide CI remains red from out-of-scope failures.
...
- proven: revision `80a659b819f64868ff766490348e0e5cb93bef76` is deployed in production; the three widened columns and current row bounds are present on the target database; all 10 financial KPIs have exact owner/campaign/property-or-absence/filter/window/source/persisted-input/provider-credential/stored-state classification; 3 deterministic simulated-property rows exactly match; 3 propertyless rows resolve unavailable; the 4 documented orphan parents were removed by the exact guarded transaction with notification audit retained; the complete mutation-proof deployed browser matrix and browser PDFs pass; and 2 live-provider Revenue rows match campaign-to-date resolved values under refresh-disabled success/failure validation
- unproven: timer-fired exact updated/skipped/failed KPI IDs before the configured `2026-08-03T03:00:00Z` run; authenticated refresh-disabled live totals for KPI hashes `2f27f064cd32` and `befd1e90d209`; production notification retry after an injected failure; current direct snapshot/test-send/manual-send/scheduled-report freshness parity for a qualifying selected-KPI fixture; and SHA-bound alert/report delivery. Historical audit rows, persisted last-good values, or provider acceptance are not inferred as current proof

Chronological smallest-safe validation-closure queue (current evidence status):

1. **Target-data Gate 1 — passed for exhaustive inventory and classification in Commit 10A.** All 17 skips are fully classified, all 10 financial KPI rows have exact boundaries and current evidence states, and the 3 propertyless rows are valid unavailable-state fixtures; no user configuration is required to prove that inventory result. This gate does not certify the 4 current live-provider totals, which remain in step 4's provider/token gate.
2. **Deployed state-matrix Gate 2 — passed on deployed `80a659b819f64868ff766490348e0e5cb93bef76`.** Exact dry-run detection identified only the four documented non-null-campaign orphan GA4 parents. The user-authorized count-and-identity-guarded transaction deleted exactly those four parents, retained all 35 resolved notification audit rows, and the post-check found zero orphan GA4 parents. Mutation-blocked browser automation then passed valid-zero, unavailable, stale/last-good, blocked, insufficient-data, and failed-list states across cards, breach indicators, tracker scoring, KPI Insights, bell/Notifications, and browser PDF. Every application request was intercepted; non-GET requests were blocked and none occurred.
3. **Timer scheduler Gate 3 — pending until the already-configured natural run.** Revision `58d6542d46d1e99d68e67740ab8065672a508b13` is deployed. Health at `2026-08-02T10:51:38Z` showed `runOnStartup=false`, `totalRuns=0`, `lastRunStatus=idle`, next run `2026-08-03T03:00:00Z`, and the new `lastRecomputeRecordedAt`/`lastRecomputeEvidence` fields present and correctly null before any run. No manual trigger was used. The scheduler logs raw processed/skipped/failed campaign and KPI IDs privately and now retains SHA-256-prefixed ID arrays in public scheduler health without exposing raw IDs or changing scheduler behavior; 34/34 focused scheduler/auto-refresh/certification tests, the checker, TypeScript, and the production build pass. This still cannot prove a timer run before the configured time.
4. **Provider Gate 4 — partially passed and pending for two inaccessible campaigns.** Refresh-disabled, non-persisting provider success and simulated-failure checks passed for KPI hashes `7ed1d5c4d10f` and `ff24c717b842`: exact property/filter/Europe-Amsterdam scope, campaign-to-date provider window `2026-06-24..2026-08-01`, imported-revenue and spend source IDs, and resolved Revenue `$51,405.93` matched stored values; failure returned no provider totals and attempted no token refresh. A fresh read-only production/Clerk check proves KPI hashes `2f27f064cd32` and `befd1e90d209` share one persisted owner ID that Clerk returns `404` for; this is stronger than the previous "no active session" wording, and a temporary revocable owner session cannot be created. Ownership was not changed. The next natural scheduler result can prove the owner-independent production recompute outcome for those exact row hashes; if it skips/fails them, a separate reauthorization or ownership/data-lifecycle decision is still required and must not be inferred.
5. **Report Gate 5 — pending because no qualifying existing or historical fixture exists.** The refreshed read-only inventory found nine GA4 reports. The only custom report whose KPI section is enabled has zero selected KPI IDs, no recipients, no schedule, no snapshots, and no send/delivery events. Recipient-configured scheduled GA4 reports are Overview or Benchmark reports, not KPI reports. Therefore direct snapshot/test-send/manual-send/scheduled-send parity against the same selected KPI IDs and confirmed delivery cannot be proven from existing evidence. Closing this gate requires explicit authorization for one temporary exact-scope KPI report using an already configured recipient, the four controlled output/send paths, confirmed provider/inbox delivery, and guarded cleanup; no such mutation or send has occurred.
6. **Final regression Gate 6 — GA4-local evidence passed; repository CI remains red.** The exact results are recorded in the consolidated evidence below. Because Gates 3-5 remain incomplete, status stays `UNVERIFIED`.
...
| --- | --- | --- | --- | --- |
| `2f27f064cd32` | CPA | owner present; primary live property; UTC; no campaign filter; no configured financial sources | traffic `2026-07-03..2026-08-01`; financial sources `1900-01-01..2026-08-02` | no persisted native rows, revenue, or spend; encrypted-only credentials recorded expired `2026-01-10`; stored `6.01` was last updated `2026-01-08` and is classified last-good/unverified, not current; provider total remains in the provider/token gate |
...
| `befd1e90d209` | Revenue | owner present; primary live property; UTC; campaign filter; HubSpot revenue and manual spend source | same captured windows | no persisted native rows; `$12,000` imported revenue; spend source has no records; encrypted-only credentials recorded expired `2026-01-19`; stored `$2,000` was last updated `2026-01-13` and is classified last-good/unverified; provider total remains in the provider/token gate |
| `7ed1d5c4d10f` | Revenue | owner present; primary live property; Europe/Amsterdam; campaign filter; CSV/HubSpot/Shopify revenue and CSV/Sheets spend | same captured windows | 16 persisted native rows through `2026-07-12` were last rewritten `2026-08-02T03:46Z`; `$33,400` imported revenue and `$2,699.75` spend exist; stored `$51,405.93` was updated `2026-08-02T06:03Z`; the encrypted access credential recorded expiry at `06:52Z`, so the stored value is bounded last-good evidence and the current provider total remains in the provider/token gate |
| `ff24c717b842` | Revenue | same live campaign/property/source boundary; its only calculation metadata is alert-email schedule | same captured windows | same persisted/source inputs and stored `$51,405.93` update; current provider total remains unverified under the provider/token gate |
...
All 10 rows are standard recognized metrics with `applyTo=all`; there is no specific-campaign override or hidden source-ID calculation mapping. Commit 10A corrects the queue's root classification error: target-data inventory completeness and live-provider behavior are separate external gates. `PRODUCTION_READINESS.md` permits inventory paths to be explicitly classified as not locally verifiable; it does not permit those paths to be counted as provider success. Gate 1 therefore passes because every target row and absence is exhaustively classified. The four current live-provider totals remain pending under `provider_and_token`. The three propertyless rows are retained as proven unavailable-state rows and require no user configuration for this inventory result. This does not weaken the overall certification gate, which remains `UNVERIFIED`.
...
Gate 2 status at the time of Commit 10B was **pending**. The consolidated closure evidence immediately below supersedes that bounded result and records Gate 2 as passed. Overall KPI status remains **UNVERIFIED** because Gates 3-5 remain incomplete.
