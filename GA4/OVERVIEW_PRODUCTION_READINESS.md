...
**Current section status: UNVERIFIED.** The current candidate changes Campaign Breakdown native revenue from the fixed import-to-date traffic window to an exact per-campaign campaign-start-to-latest-completed-day query, requires its native row sum to reconcile to the GA4 Revenue card, and then adds only exact campaign-mapped imported revenue. Traffic metrics remain on the fixed import-to-date boundary. Focused local validation passes; exact deployment and authenticated API/UI/browser-PDF parity have not yet been completed.

2026-09-07 Campaign Breakdown revenue correction: prior deployed output combined `$43,668.50` native revenue from `2026-07-02` through `2026-09-06` with `$16,799.99` exact mapped imports, producing `$60,468.49`, while the GA4 Revenue card used the saved campaign start `2026-06-24` through `2026-09-06` and returned `$65,362.20`. The candidate retains Summary-aligned traffic from `2026-07-02`, queries native row revenue separately from `2026-06-24`, and then adds the mapped imports. For the recorded data, the expected displayed table total is `$82,162.19`; this exact production result remains unproven until deployment validation.
