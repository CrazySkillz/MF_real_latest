...
9. Current Commit 8: Shopify portability and provider-boundary variants. Pending after the core lifecycle: other campaigns/mappings and a >250-order provider fixture if one becomes available.
...
- Current Commit 6a is locally regression-covered for Shopify downstream value/content: scheduled GA4 PDF text, GA4 KPI/Benchmark current rows, and `/api/notifications` alert metadata now have Shopify-specific automated proof. Deployed report/PDF inspection, deployed KPI/Benchmark row packet, deployed notification UI row packet, provider email acceptance, and delivered-email/inbox receipt remain unproven and must not be inferred from either Current Commit 6 cache/refetch evidence or local-only 6a automation.
