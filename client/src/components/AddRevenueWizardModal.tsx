...
export function AddRevenueWizardModal(props: {
  ...
  const invalidateAfterRevenueChange = () => {
    ...
    if (platformContext === 'google_sheets') {
      ...
      void queryClient.invalidateQueries({ queryKey: [`/api/campaigns/${campaignId}/revenue-totals?platformContext=google_sheets&dateRange=all`], exact: false });
    ...
    if (platformContext === 'custom_integration') {
      void queryClient.invalidateQueries({ queryKey: [`/api/campaigns/${campaignId}/revenue-totals?platformContext=custom_integration`], exact: false });
      void queryClient.invalidateQueries({ queryKey: [`/api/campaigns/${campaignId}/revenue-totals?platformContext=custom_integration&dateRange=all`], exact: false });
      void queryClient.invalidateQueries({ queryKey: ["/api/campaigns", campaignId, "revenue-sources", "custom_integration"], exact: false });
      void queryClient.invalidateQueries({ queryKey: ["/api/custom-integration", campaignId], exact: false });
      void queryClient.invalidateQueries({ queryKey: ["/api/custom-integration", campaignId, "metrics"], exact: false });
      void queryClient.invalidateQueries({ queryKey: [`/api/platforms/custom-integration/kpis?campaignId=${campaignId}`], exact: false });
      void queryClient.invalidateQueries({ queryKey: ["/api/platforms/custom-integration/kpis", campaignId], exact: false });
      void queryClient.invalidateQueries({ queryKey: ["/api/platforms/custom-integration/benchmarks", campaignId], exact: false });
    ...
    if (platformContext === 'linkedin') {
      ...
      void queryClient.refetchQueries({ queryKey: [`/api/campaigns/${campaignId}/revenue-totals?platformContext=google_sheets&dateRange=all`], exact: false });
    ...
    } else if (platformContext === 'custom_integration') {
      void queryClient.refetchQueries({ queryKey: [`/api/campaigns/${campaignId}/revenue-totals?platformContext=custom_integration&dateRange=all`], exact: false });
      void queryClient.refetchQueries({ queryKey: ["/api/campaigns", campaignId, "revenue-sources", "custom_integration"], exact: false });
      void queryClient.refetchQueries({ queryKey: ["/api/custom-integration", campaignId, "metrics"], exact: false });
      void queryClient.refetchQueries({ queryKey: [`/api/platforms/custom-integration/kpis?campaignId=${campaignId}`], exact: false });
    } else {
  ...
  const sheetsPurpose = platformContext === 'linkedin' ? 'linkedin_revenue' : platformContext === 'meta' ? 'meta_revenue' : platformContext === 'google_ads' ? 'google_ads_revenue' : platformContext === 'instagram' ? 'instagram_revenue' : platformContext === 'tiktok' ? 'tiktok_revenue' : platformContext === 'google_sheets' ? 'google_sheets_revenue' : platformContext === 'custom_integration' ? 'custom_integration_revenue' : 'revenue';
  ...
  const [revenueCampaignMappings, setRevenueCampaignMappings] = useState<PlatformCampaignMapping[]>([]);
  ...
  const handleCrmDisconnect = async (platform: "hubspot" | "salesforce" | "shopify") => {
    ...
    try {
      if (platform === 'hubspot' && platformContext === 'ga4') {
        ...
        setCrmOAuth(prev => ({ ...prev, hubspot: false }));
        setCrmStatus(prev => ({ ...prev, hubspot: false }));
        setCrmHasSource(prev => ({ ...prev, hubspot: false }));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        } else if (campaignPlatform === 'instagram') {
          const resp = await fetch(`/api/instagram/${campaignId}/campaigns`, { credentials: "include" });
          const json = await resp.json().catch(() => ({}));
          if (!cancelled && Array.isArray(json?.campaigns)) {
            const selectedIds = new Set(Array.isArray(json?.selectedCampaignIds) ? json.selectedCampaignIds.map((id: any) => String(id)) : []);
            setPlatformCampaigns(json.campaigns
        ...
        } else if (campaignPlatform === 'tiktok') {
          const resp = await fetch(`/api/tiktok/${campaignId}/campaigns`, { credentials: "include" });
          const json = await resp.json().catch(() => ({}));
          if (!cancelled && Array.isArray(json?.campaigns)) {
            const selectedIds = new Set(Array.isArray(json?.selectedCampaignIds) ? json.selectedCampaignIds.map((id: any) => String(id)) : []);
            setPlatformCampaigns(json.campaigns
    ...
    return () => { cancelled = true; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, step, manualPlatform, platformContext, campaignId]);
  ...
  const platformCampaignOptions = useMemo(() => {
    ...
    for (const mapping of revenueCampaignMappings) {
  ...
  const updateRevenueCampaignMapping = (crmValue: string, campaignIdValue: string) => {
    ...
    setRevenueCampaignMappings((prev) => {
      ...
      return [
        ...
        {
          crmValue: value,
          linkedinCampaignUrn: campaignIdValue,
          linkedinCampaignName: campaign?.name || campaignIdValue,
        },
      ];
    });
  };
          ...
          {selectedValues.map((value) => {
            ...
            return (
              <div key={value} className="grid gap-2 md:grid-cols-[minmax(0,1fr)_minmax(220px,320px)] md:items-center">
                <div className="truncate text-sm text-foreground/80">{value}</div>
                <Select
                  value={current?.linkedinCampaignUrn || SELECT_NONE}
                  onValueChange={(next) => updateRevenueCampaignMapping(value, next)}
  ...
  const hasMeaningfulCsvRevenueEditChanges = useMemo(() => {
    ...
    const normalizeMappings = (items: any[]) => JSON.stringify((Array.isArray(items) ? items : [])
      .map((item: any) => ({
        crmValue: String(item?.crmValue || "").trim(),
        linkedinCampaignUrn: String(item?.linkedinCampaignUrn || "").trim(),
      }))
      .filter((item: any) => item.crmValue && item.linkedinCampaignUrn)
      .sort((a: any, b: any) => a.crmValue.localeCompare(b.crmValue)));
  ...
  const handleSheetsProcess = async () => {
    ...
    try {
      ...
      const valueSource: 'revenue' = 'revenue';
      const campaignMappings = mappedRevenueCampaignsForValues(sheetsCampaignValues);
      const mapping = {
