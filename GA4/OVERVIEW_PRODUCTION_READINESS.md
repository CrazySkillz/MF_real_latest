...
| Current Commit 2f Google Sheets spend second-campaign add/import evidence | Deployed second-campaign cleanup snapshot captured on `2026-07-03T11:30:06.922Z`; after-add snapshot captured on `2026-07-03T11:42:32.717Z`; endpoint status packet captured on `2026-07-03T11:45:16.366Z`; user UI validation confirmed `$507.70` on `2026-07-03`. | Campaign `61bf28cb-74b0-4beb-9afe-fd02f2f285c6` / property `498536418` was cleaned to zero Google Sheets spend sources, then one Google Sheets spend source was imported with expected spend `$507.70`; `exactlyOneGoogleSheetsSpendSourceAdded`, `spendIncreasedByExpected`, and `revenueUnchanged` passed; revenue/spend endpoints returned HTTP `200`; the visible GA4 Overview spend card/source modal matched `$507.70`. | Closed only for this second-campaign Google Sheets spend add/import UI/source-modal financial path. The compact after packet showed `spendToDate: 0` while spend breakdown/UI showed `$507.70`, and `ga4ToDate` returned HTTP `500` with GA4 `INVALID_ARGUMENT`; therefore broader Current Commit 2f campaign/property certification remains blocked until Current Commit 2f.1 is deployed/validated and the remaining lifecycle actions are captured. |
| Current Commit 2f.1 GA4 to-date no-completed-window guard | `npm test -- server/ga4-reporting-day-cutoff-regression.test.ts`; `npm run check`. | `/api/campaigns/:id/ga4-to-date` now returns scoped zero native GA4 totals with `noCompletedWindow: true` when the selected campaign's start/created date is later than the latest completed GA4 reporting day, instead of calling GA4 Data API with an inverted date window. | Locally regression-covered only until deployed. Must be deployed and rechecked against campaign `61bf28cb-74b0-4beb-9afe-fd02f2f285c6` / property `498536418` before continuing broader Current Commit 2f certification. |
...
| Current Commit 2f: Google Sheets spend campaign/property portability | Partially validated for the second-campaign add/import UI/source-modal financial path; broader 2f is paused until Current Commit 2f.1 is deployed and validated. | A second disposable campaign/GA4 property context with Google Sheets spend. | Repeat the Google Sheets spend lifecycle on a second campaign/property context with controlled amounts and source IDs: add/import, edit/update, run-now refresh/reprocess, and delete/deactivate, or explicitly record any action not exposed. The add/import packet captured `$507.70` UI/source-modal parity, but the native `ga4ToDate` provider path failed and must be fixed before continuing broader 2f certification. | No for local code-scope certification; yes for multi-campaign/property certification until the second campaign/property native GA4 guard and remaining lifecycle evidence packets are captured. |
| Current Commit 2f.1: GA4 to-date no-completed-window guard | Implemented and regression-covered locally; pending deploy validation. | `/api/campaigns/:id/ga4-to-date` for a selected campaign/property when the campaign start/created date is later than the latest completed GA4 reporting day. | Return scoped zero native GA4 totals with `noCompletedWindow: true` before calling the provider when `startDateUsed > endDateUsed`; preserve auth, campaign access, property scoping, imported revenue/spend endpoints, scheduler behavior, and source behavior. | No for local code-scope certification after tests; yes for broader second-campaign/property Overview certification until deployed against campaign `61bf28cb-74b0-4beb-9afe-fd02f2f285c6` / property `498536418`. |
...
- Certification boundary: this closes only the visible second-campaign Google Sheets spend add/import UI/source-modal financial path. It does not close second-campaign native GA4 provider health, spend-to-date exact-value parity from the compact packet, edit/update, run-now refresh/reprocess, delete/deactivate, alternate tabs/mappings, report/email propagation, normal scheduler proof, or other source families.
...
- Deployed validation still required: after this fix is committed, pushed, and deployed, rerun the second-campaign endpoint packet and confirm `/ga4-to-date` returns HTTP `200` with `noCompletedWindow: true` or valid native totals, revenue/spend endpoints remain HTTP `200`, and the UI still shows the `$507.70` Google Sheets spend correctly.
...
| Google Sheets spend | `Total Spend`, spend sources modal, Profit, ROAS, ROI, CPA, report payload values after the change | Closed for deployed campaign/source add/import of `8f67b03f-a00b-434f-b81f-db1b2b951595` after endpoint plus UI evidence (`$240`); second-campaign/property add/import UI/source-modal path user-confirmed for `$507.70` on campaign `61bf28cb-74b0-4beb-9afe-fd02f2f285c6` / property `498536418`, with native `ga4ToDate` blocked separately by Current Commit 2f.1 | Closed for deployed campaign/source edit/update of `8f67b03f-a00b-434f-b81f-db1b2b951595` after endpoint plus UI evidence (`$420.20`); second-campaign edit/update remains unproven | Closed for deployed campaign/source run-now reprocess of `8f67b03f-a00b-434f-b81f-db1b2b951595` after endpoint plus UI evidence (`$198.75`); startup-fired scheduler execution closed separately for source `618e5e12-0f3f-44a2-837a-d2677ad95f64` after endpoint plus UI evidence (`$678.95`); second-campaign refresh/reprocess remains unproven | Closed for deployed campaign/source delete/deactivate of `8f67b03f-a00b-434f-b81f-db1b2b951595` after endpoint plus UI evidence (`$198.75` removed); second-campaign delete/deactivate remains unproven | Closed only for the validated deployed Google Sheets spend campaign/source lifecycle, the recorded startup-fired scheduler execution path, and the second-campaign add/import UI/source-modal evidence. Does not certify second-campaign native GA4 provider health until 2f.1 deploy validation, second-campaign edit/refresh/delete, normal wall-clock scheduled-hour execution if separately required, reports/emails, or other source families. |
