...
- all three cumulative Campaign Breakdown rows reconcile exactly to Summary traffic/conversions before exact campaign-matched source-to-date revenue is added
...
| GA4 native financial revenue | ordered complete-source selection: campaign-to-date provider, persisted daily where available to the caller, then configured-lookback breakdown only when earlier candidates are absent | campaign-to-date with explicitly ordered continuity fallbacks | Current authenticated value is `$65,362.20 USD`; requested/observed currency matches. |
| Financial conversions | Taken from the same object selected for native financial revenue | same ordered financial candidate | Current exact value is 292 and is paired with the selected campaign-to-date native financial response. |
...
| Total Revenue | selected campaign-to-date GA4 native revenue + source-backed imported revenue to date | campaign lifetime/to date | Exact current result is `$82,162.19 USD`; additive source list, source identities, currency, and browser-PDF parity passed. |
...
| Profit | Total Revenue - Total Spend | same available financial inputs | Current authenticated result is `$79,462.44`; valid zero revenue produces the negative-spend loss, while missing input remains unavailable. |
| ROAS | Total Revenue / Total Spend | same available financial inputs | Current authenticated result is `30.43x`. Valid zero revenue with positive spend renders `0.00x`; missing or zero spend blocks the ratio. |
| ROI | (Total Revenue - Total Spend) / Total Spend | same available financial inputs | Current authenticated result is `2943.3%`. Valid zero revenue with positive spend renders `-100%`; missing or zero spend blocks the ratio. |
| CPA | Total Spend / financial conversions | spend plus conversions from the selected native financial candidate | Current authenticated result is `$9.25`; missing or non-positive conversions remain unavailable. |
...
Runtime `08d7abe535c9ea4f57f57d8cbdbb072fba64052e` separates Campaign Breakdown traffic and native-revenue windows without allocating or scaling values. Traffic remains fixed at `2026-07-02` through `2026-09-06`. Because the campaign has no explicit start date, native revenue uses the same `2026-06-24` campaign creation-date fallback as the GA4 Revenue card, through `2026-09-06`. The native row sum reconciles to `$65,362.20`; exact saved mappings add `$16,799.99`; the displayed rows reconcile to `$82,162.19`. The authenticated read-only audit confirmed the API, rendered table, and downloaded browser PDF, plus unauthenticated/cross-owner denial and zero orphan or duplicate external-revenue keys.
