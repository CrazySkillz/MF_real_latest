...
- Shopify OAuth must not infer or guess its callback URL from request host, browser origin, Render service URL, or generic app base URL. Shopify requires the OAuth `redirect_uri` to exactly match the callback URL whitelisted in the Shopify app. Production Shopify OAuth must use `SHOPIFY_REDIRECT_URI` or `SHOPIFY_APP_BASE_URL`; otherwise it should fail clearly before opening Shopify.
