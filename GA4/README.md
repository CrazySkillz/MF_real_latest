...
Executive Summary Risk Assessment should show the configured inputs it checked. For GA4-only campaigns, this means available ROI/ROAS when revenue and spend are available, no paid-platform concentration risk unless a main paid-media platform is connected, compatible 7-day trajectory only after enough aggregate snapshot history exists, aggregate-backed KPI and Benchmark misses when those campaign-level records map to available GA4/connected-source metrics, and GA4 data freshness warnings. Visible ROI/ROAS checked-input values and KPI/Benchmark risk counts should use the same page-level `performanceSummary` aggregate as the Executive Summary metric cards, KPI Progress, and Benchmark Comparison. Non-actionable boundary rows should stay out of the executive UI: paid-platform concentration should not show for GA4-only campaigns, and budget pacing remains in Budget & Financial Analysis until Executive Summary has a shared pacing input.
