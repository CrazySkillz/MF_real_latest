...
- The concise current Overview decision and active gates live in `GA4/OVERVIEW_PRODUCTION_READINESS.md`; detailed evidence lives in `GA4/OVERVIEW_PRODUCTION_READINESS_EVIDENCE.md`; chronological Current Commits 0–20 live in `GA4/OVERVIEW_PRODUCTION_READINESS_HISTORY.md`. Current status is **UNVERIFIED**: Current Commit 20 corrects the escaped rolling-window interpretation of the 30-day setup value. The intended contract is a fixed initial 30-completed-day import followed by completed-day accumulation. Deployment and parity validation remain open; OAuth durability remains due on or after `2026-08-07`.

- The Overview tab is not clean-certified or production-ready for live users. Current Commit 20 must be deployed and validated, then the current-release scheduled-run and OAuth durability gates must pass. No production cleanup is authorized.
...
- `GA4/OVERVIEW_PRODUCTION_READINESS.md`
  Concise canonical current-status index. Current status: **UNVERIFIED** while Current Commit 20 restores the initial-import-plus-accumulation contract and awaits deployed parity evidence.
