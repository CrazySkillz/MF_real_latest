...
11. Current Commit 4.7d: HubSpot clean-certification update. With 4.7c closed as not applicable for deployed user-facing refresh/reprocess, update this file only after the 4.7c local guard is committed/pushed and mark HubSpot clean-certified for the exact proven interactive scope: campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`, GA4 Overview confirmed `Total Revenue only (no Pipeline card)`, add/import, edit/update, delete/deactivate, before/after inventory, and provenance. Do not certify Pipeline Proxy, other campaigns, alternate mappings, background scheduler execution, Reports, KPI/Benchmark propagation, or provider mutation propagation unless separately proven.
12. Current Commit 4.8: optional HubSpot scheduler/provider propagation proof. If strict background provider-refresh certification is required, change a controlled HubSpot deal value/date in the connected account, let the existing scheduler path run or trigger the existing scheduler pipeline through an already-approved operational path, then capture before/after snapshot, inventory, and provenance evidence. Required checks: same active HubSpot source ID, no duplicate source, revenue changes to the expected provider value, records remain scoped to GA4, no unrelated source changes, and Pipeline Proxy remains outside the evidence unless explicitly selected. Do not add a new HubSpot source-scoped run-now route unless a separate root cause proves it is necessary.
...
- Shopify, Salesforce, legacy Manual revenue, LinkedIn Ads, Meta Ads, Google Ads, and legacy Manual spend remain provider/user validation work until each relevant family has before/after evidence. HubSpot has deployed Total Revenue add/edit/delete endpoint and data-health evidence plus deployed provenance evidence for campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`; 4.7c closes deployed user-facing refresh/reprocess as not applicable once the static guard is committed/pushed. HubSpot remains not clean-certified until the final 4.7d certification update is made for the exact proven scope.
