...
Before Commit 4, the corrected Executive Summary sections are source-aligned for GA4/current connected-source validation: the narrative, funnel, key metric cards, KPI Progress current values, Benchmark Comparison current values, and Risk Assessment risk inputs use page-level `performanceSummary` where current connected-source values are needed. Strategic Recommendations remain the next production-readiness target because recommendation copy and paid-media guidance still need source-capability gating.
