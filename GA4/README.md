...
- Certification integrity rule: GA4 readiness is valid only for the exact certified SHA/configuration/dependency boundary. A relevant upstream or downstream change automatically invalidates affected tabs. The GA4 KPI record is `GA4/certifications/ga4-kpis.json` and its checker is `npm run check:ga4-kpi-certification`. Current Commits 1-8 implement the fail-closed gate, dynamic cross-consumer guard, metric identity contract, authoritative traffic/rate window contract, bounded financial source/failure contract, alert/notification eligibility, UI/browser-consumer states, and exact recompute/report proof while the record remains `UNVERIFIED`; Current Commits 9-10 and external validation remain open. Copied formulas and source-text guards are structural evidence only.
