...
Current controlling Google Sheets Revenue decision: **Google Sheets Revenue is clean-certified and production-ready for the bounded forward/active-source lifecycle and its direct GA4 Overview source-list, breakdown, and imported-total surfaces on deployed revision `f8061d135a85fbe2c4c11433fffb3f80dedceae8`.** The claim includes the documented supported amount/date formats, tabs with at most 50,000 allocated rows, pre-mutation last-good retention, successful source response semantics after a post-commit derived-recompute failure, rejection of simultaneous identical GA4 add requests on the currently observed single-runtime Render topology, and the campaign-scoped atomic removal of all GA4 Google Sheets Revenue sources plus their unused sheet connections. It excludes horizontally scaled/multi-worker deployments, duplicate retries after the first request has completed, imports above 50,000 allocated rows, ambiguous locale values/dates, deployed Profit/ROAS/ROI/CPA parity, report/email artifacts, other source families, and the globally clean historical-database claim described below. The prior whole-Overview and Upload CSV Revenue decisions retain only their separately recorded boundaries.

**V1 completion: Google Sheets Revenue is clean-certified for the documented, currently deployed single-runtime scope. Required steps remaining: 0.** The V2 improvements below are explicitly outside this completed V1 certification and do not block it.
...
- **Google Sheets Revenue is clean-certified for the bounded forward/active-source lifecycle and direct GA4 Overview source-list, breakdown, and imported-total surfaces on deployed revision `f8061d135a85fbe2c4c11433fffb3f80dedceae8`.** Foreground add/edit/delete, aggregate Google Sheets disconnect, exact-source provider refresh, automatic refresh, OAuth renewal, direct source-list/totals propagation, refresh/edit overlap protection, simultaneous-identical-add protection, pre-mutation failure retention, post-commit derived-recompute response safety, large-sheet rejection, and exact tab identity are covered by the evidence below. The certification does not extend to excluded formats, consumers, or deployment topologies.
...
- Current Commit 7 has user-confirmed deployed UI validation for the normal exact-source deletion flow. Revision `f8061d13` additionally has user-confirmed deployed validation for aggregate Google Sheets Revenue disconnect. Both rollback boundaries remain deterministically transaction-tested because the normal UI intentionally has no unsafe database-failure injection control; neither is represented as an observed production failure.
...
### V2 Improvements (Non-Blocking For V1)

- add persistent idempotency for completed-request retries and duplicate submissions across multiple workers;
- add distributed source-operation and OAuth-refresh coordination before enabling horizontal scaling;
- support imports above 50,000 allocated rows with a separately bounded and certified large-sheet design;
- add explicitly selected locale-aware amount and date formats without guessing ambiguous values;
- certify exact deployed Profit/ROAS/ROI/CPA parity and any Google Sheets revenue included in reports, PDFs, or email artifacts;
- assess and, only with separate destructive authorization, transactionally clean the 67 records retained on three inactive historical sources.
...
- foreground and scheduler processing accepted numeric prefixes from selected revenue cells, so values such as `1.234,56`, `1 234.56`, or `100 USD` could be persisted as a different amount instead of failing.

The localized fix makes GA4 scheduler dates use the foreground normalizer, fails currency mismatch before mutation, renews refresh-token-only connections, preserves token rotation, rechecks the exact prior mapping inside the source-and-record transaction, refreshes campaign-value totals in that transaction, and validates selected GA4 Google Sheets amounts before permissive parsing. A forced optimistic-conflict test proves old records remain untouched when the source changed; a scheduler regression proves unsupported selected amount text returns before replacement and retains last-good records.
...
Locally proven in this addendum: exact campaign/source/context guards, deterministic row/date validation, campaign-currency recheck, atomic source/record replacement and delete, optimistic stale-write rejection, stable scheduler source ID, token-only renewal wiring, active-source totals/breakdown/source-list reads, exact numeric Google Sheets propagation through Total Revenue, Profit, ROAS, and ROI with CPA unchanged, and static propagation into Ad Comparison metadata, KPI/Benchmark jobs, alerts, notifications through alert recompute, outcome/executive consumers, and scheduled report inputs. The focused packet, TypeScript check, and production build passed; the exact commands/results are retained in the working-session handoff until commit is requested.

The proactive Google Sheets OAuth token scheduler now applies the same 15-second provider timeout used by the other Google Sheets refresh paths. This closes the unbounded-hang defect in the deployed code; timeout behavior is locally guarded, while a deliberately stalled live Google endpoint and cross-worker refresh serialization remain unproven.

Deployed validation on exact runtime `ce3eaeafc98e9f8813c3fb476a9bcf68eb870a3d` used one user-created active GA4 Google Sheets Revenue source. Two exact-source provider refreshes returned `200` with stable source ID, two records, `$30,300` source amount, `$33,756.90` imported total, four active revenue sources, and one active Google Sheets source. After one selected provider cell was changed to text `1.234,56`, the natural one-minute pass left the prior source fingerprint and `lastSyncedAt` unchanged; the campaign/source-scoped run-now route returned the expected `502`, and a read-only database comparison proved the source plus records were unchanged. After the user restored the cell, run-now returned `200`, the rendered Revenue Sources modal showed `Google Sheets revenue $30,300.00`, and a later natural timer advanced `lastSyncedAt` to `2026-09-11T20:55:00.624Z` without amount, record-count, source-count, or duplicate drift. This is a bounded fixture packet, not general Google Sheets production-readiness certification.

The user then deleted that exact disposable source through the deployed Overview UI. Read-only reconciliation at `2026-09-11T22:26:33.970Z` proved source `cb3fe74a-579c-44c0-a8b0-d241756504b4` inactive with zero linked records, imported revenue and breakdown both changed from `$33,756.90` to `$3,456.90`, active imported sources changed from four to three, active Google Sheets sources changed from one to zero, and the deleted ID was absent from both the active source list and breakdown. The rendered Revenue Sources control changed from `Sources (5)` to `Sources (4)`, and the modal contained neither the deleted source label nor `$30,300.00`. The complete Google Sheets inventory changed from 69 to 67 records and retained only the same three pre-existing inactive-source groups containing 67 records; no orphan, cross-campaign, wrong-type, currency, attribution-total, date, amount, duplicate, or sync-timestamp finding was introduced.
...
Commit `b987d980` closes the foreground post-commit response defect without changing storage, formulas, other providers, or the public response shape. The GA4 Google Sheets source-and-record transaction remains authoritative; if the subsequent derived-value recompute rejects, the route logs that secondary failure and returns the existing success response for the committed source instead of returning a misleading `500` that could invite an additive retry. The six-file focused packet passed 52/52, TypeScript passed, and the production build passed. Render then reported exact runtime `b987d980ec36c8b07c6c3448e7f43bd65e058359`; `/api/health` and `/health/scheduler` returned `200`, with all schedulers started and no reported scheduler error. No unsafe production recompute-failure injector was added or used.

Commit `b50c2f6a` adds a GA4 Google Sheets add-only in-flight fingerprint guard. On the currently certified single runtime, a simultaneous identical add receives `409` before any provider read or source mutation; the guard is always released after success or failure. Different mappings remain additive, while edit, refresh, other platform contexts, formulas, storage, and response shapes are unchanged. The seven-file focused packet passed 67/67, TypeScript passed, and the production build passed. Render reported exact runtime `b50c2f6a0e9a9cc35adc146885145697f83fc2d4`, and `/api/health` returned `200`. This is not persistent idempotency across workers or after the first request completes.

Commit `47c79a46` fixes the edit-preview flash observed when Google returned quota status `429`. The root cause was an edit-mode failure branch that moved to the chooser and immediately auto-advanced back to mapping, creating an automatic retry loop. The localized UI fix keeps the saved mapping unchanged, stops the loop, preserves the reconnect-only chooser transition, exposes one deliberate `Retry preview` action, and replaces the raw provider payload with a short quota message. The eight-file focused packet passed 111/111, TypeScript passed, and the production build passed. Render reported exact runtime `47c79a46aea87bef54111bf2909130d5499aa96c`, and `/api/health` returned `200`. Exact fixed visual behavior on that runtime has not yet been manually observed; this UI-only confirmation is recorded separately from revenue-value certification.

Revision `f8061d13` brings the Add Revenue Source Google Sheets card and the aggregate destructive path into alignment with the additive multi-sheet implementation. When at least one active GA4 Google Sheets Revenue source exists, the card and inert `Connected` label no longer navigate; only `Add another sheet` opens the append chooser, and the separate trash control opens an explicit confirmation. The confirmed action calls the new campaign-access-guarded GA4 Google Sheets disconnect route. `disconnectGa4GoogleSheetsRevenue(...)` runs source deactivation, exact campaign/source record deletion, and unused connection deactivation/cache clearing in one database transaction; it preserves connection IDs still referenced by another active revenue or spend source, preserves other campaigns and platform contexts, verifies affected-row counts, and promotes a remaining active primary connection when required. Derived-value recomputation remains post-commit and does not change an existing public response shape.

Local validation for `f8061d13` passed `server/google-sheets-ga4-disconnect-transaction.test.ts` plus `server/ga4-ui-regression.test.ts` with 50/50 tests, the focused GA4 Google Sheets disconnect assertion in `server/source-safety-regression.test.ts`, `npm run check`, `npm run build`, and `git diff --check`. The transaction suite forces source, record, and connection failures and proves full rollback; it also proves preservation of a sheet connection shared with an active spend source and isolation from another platform/campaign. The complete source-safety file still has ten pre-existing Instagram/Google Ads static-assertion failures; the new Google Sheets assertion passed and those unrelated failures are not represented as a clean full-suite result.

After deployment, the user confirmed the current Google Sheets card presentation and completed the confirmation-driven trash flow on the test campaign. The observed result removed all Google Sheets Revenue sources, updated Total Revenue, and left the other revenue sources unchanged. This is user-confirmed deployed normal-path evidence for revision `f8061d13`; no source IDs, raw before/after amounts, read-only database reconciliation, or deployed failure injection were supplied, so those are not claimed.
...
- one deployed overlapping exact-source refresh pair safely produced one conflict and one success without record or total drift. Simultaneous identical foreground adds are now rejected by the current single-runtime in-flight guard before mutation. Persistent duplicate retries and OAuth token refreshes are not serialized across workers; those paths require recertification before horizontal scaling;
- a fast derived-recompute failure after a successful source transaction is now contained only in the GA4 Google Sheets foreground path: committed source data remains authoritative and the existing success response is returned. This is structurally regression-guarded and deployed at the exact healthy runtime above; no unsafe production failure injection is claimed;
...
- live provider reads, mapping edit/restore, repeated refresh, invalid-provider-value last-good retention, startup renewal, forced-`401` renewal, already-open changed-value convergence, same-runtime overlapping refresh, greater-than-50,000-row rejection, exact trailing-space tab identity, exact-source deletion, and the current aggregate Google Sheets Revenue disconnect normal path passed within their recorded evidence boundaries. Genuine multi-worker refresh and generated report/email artifacts remain unproven and excluded.

This audit is complete as an evidence inventory. Google Sheets Revenue is clean-certified and production-ready for the bounded forward/active-source lifecycle and direct GA4 Overview source-list, breakdown, and imported-total surfaces on deployed revision `f8061d135a85fbe2c4c11433fffb3f80dedceae8`. The claim must always retain its supported-format, maximum-row, current single-runtime, failure-mode, and consumer exclusions. The read-only inventory at `2026-09-12T10:04:41.952Z` still reports `pass: false` solely because 67 records remain attached to three known inactive historical sources in other campaigns. They do not feed active totals and no new finding was introduced, but they mean the production database as a whole is **not** clean-certified; no destructive cleanup is implied or authorized by the forward-path certification.
...
10. The connected Google Sheets card exposes append separately from aggregate removal. Confirmed aggregate removal uses a campaign-access-guarded route and one storage transaction to deactivate all active GA4/null-context Google Sheets Revenue sources, delete only their campaign/source records, deactivate and clear only their unshared revenue sheet connections, and preserve connections referenced by another active revenue or spend source. The route then recomputes campaign-derived values and the UI invalidates/refetches the existing revenue consumers.
...
| Aggregate Google Sheets disconnect failure | Source, record, and unshared-connection changes roll back together | Dedicated GA4 Google Sheets transaction; shared revenue/spend connections excluded from deactivation | Five focused transaction tests pass locally; normal deployed UI path user-confirmed; unsafe production failure injection not required |
...
- `server/google-sheets-revenue-downstream-propagation.test.ts`: the current addendum dynamically proves one exact filtered Google Sheets delta is selected once even when aggregate and attribution-detail totals coexist, then reaches Total Revenue, Profit, ROAS, and ROI while CPA inputs remain unchanged. Static guards connect the same source to foreground and scheduler replacement, active-source totals/breakdown/source-list reads, mutation invalidation, and active-Sheets-only bounded Overview polling. This is local automation, not deployed numeric or browser evidence.
- `server/google-sheets-revenue-amount-validation.test.ts` and `server/ga4-auto-refresh-regression.test.ts`: the current addendum proves the Google-Sheets-specific amount boundary accepts canonical plain/US values, rejects ambiguous locale and partial numeric text only for selected rows, runs before foreground/scheduler replacement, and dynamically retains last-good scheduler data on rejection. Shared CSV parsing and non-GA4 Google Sheets contexts are unchanged. This is local automation, not deployed provider evidence.
- `server/google-sheets-revenue-damaged-data-inventory.test.ts`: the current addendum dynamically covers exact local persistence plus mapping, connection ownership/status, durable credential presence, currency, campaign-value total, sync, record-link, record-grain, date, and amount findings. The executable inspector uses an explicit read-only transaction and proposes no cleanup. Provider state and OAuth usability remain outside its scope.
- `server/google-sheets-ga4-disconnect-transaction.test.ts`: revision `f8061d13` dynamically proves aggregate GA4/null-context source removal, exact record cleanup, unused revenue-connection cleanup, preservation of another platform/campaign and spend-shared connections, plus rollback on forced source, record, and connection failures. This is mocked local transaction evidence; the user-confirmed deployed flow covers the normal path only.
...
| 14 | Google Sheets post-commit response safety | Google-Sheets-only guard implemented; 52/52 focused tests, TypeScript, production build, exact-revision deployment, and health/scheduler smoke checks completed |
| 15 | Concurrent identical Google Sheets add protection | GA4 add-only single-runtime guard implemented; 67/67 focused tests, TypeScript, production build, exact-revision deployment, and health smoke check completed |
| 16 | Stable edit preview after provider quota failure | Retry loop removed, saved mapping retained, reconnect path preserved, raw `429` hidden, 111/111 focused tests, TypeScript, production build, exact-revision deployment, and health smoke check completed; deployed visual observation remains separately unrecorded |
| 17 | Add Revenue Source multi-sheet controls and atomic aggregate disconnect | Card and `Connected` label are inert; `Add another sheet` remains additive; trash uses explicit confirmation and a campaign-scoped transaction that preserves shared connections. Focused 50/50 tests, the targeted source-safety guard, TypeScript, build, diff check, deployment, and user-confirmed normal deletion/totals/unrelated-source validation completed |
...
- The GA4 Google Sheets foreground route returns its unchanged success contract after the atomic source transaction even when the subsequent derived-value recompute rejects; a structural regression proves the catch remains between commit and response.
- Simultaneous identical GA4 Google Sheets add requests are fingerprinted before provider access; only the first proceeds on the current single runtime, and cleanup runs in `finally`. Different mappings and all edit requests retain existing behavior.
- A Google Sheets edit-preview provider failure no longer bounces between chooser and mapping or retries automatically; the user remains on a stable mapping failure state and can retry deliberately without changing the saved source.
- Aggregate GA4 Google Sheets Revenue disconnect is campaign/context scoped and atomic across active source deactivation, exact record deletion, and unused connection cleanup. Connections referenced by another active revenue or spend source are retained; forced source, record, and connection failures roll back all transaction changes locally.
...
- Persistent duplicate retries after the first request completes and cross-worker duplicate adds; the current in-flight guard is intentionally limited to simultaneous identical adds on one runtime.
...
Upload CSV Revenue is clean-certified for its separately validated documented scope. Google Sheets Revenue is clean-certified and production-ready for the bounded forward/active-source lifecycle and direct GA4 Overview source-list, breakdown, and imported-total surfaces on deployed runtime `47c79a46aea87bef54111bf2909130d5499aa96c`: campaign-scoped add/edit/delete, exact source/list/totals propagation, scheduler refresh, OAuth startup and forced-`401` renewal, supported currency/date/amount validation, pre-mutation failure retention, post-commit derived-recompute response safety, same-runtime refresh/edit overlap protection, simultaneous-identical-add protection, full-width chunking through 50,000 allocated rows, fail-closed rejection above that boundary, and exact tab names including trailing spaces are evidenced. Horizontal scaling/multi-worker behavior, persistent duplicate retries after the first request completes, ambiguous locale formats/dates, imports above 50,000 allocated rows, exact deployed derived-card parity, report/email artifacts, other source families, future revisions/data, and a globally clean historical database are not claimed. The known 67 records on three inactive historical sources remain excluded from active totals and explicitly prevent a whole-database clean claim.
