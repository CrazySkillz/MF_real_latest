...
- `GA4/OVERVIEW_PRODUCTION_READINESS.md`
  Canonical whole-tab Overview production-readiness source of truth. Current status: not production-ready; Commits 1–17 are closed only for their documented bounded packets. Commit 17's provider-cycle rollback and retained-source disposition remain external/open. Commit 18's corrective scheduler-backed Summary implementation is locally validated but remains undeployed and externally unproven; live 60/90-day, OAuth-durability, and named scheduler/external gates also remain. It also contains the future-platform template and readiness checklist for Meta, Google Ads, LinkedIn, and other sources.
- `GA4/KPIS.md`
