...
| Ad Comparison | GA4-native 30-completed-day campaign rows plus separate source-to-date imported provenance | live client props and guarded GA4/revenue APIs | Behavior contract aligned; current machine status is `UNVERIFIED` for the committed-hash guard repair and fresh revision check. Reports-owned output is outside the tab boundary. |
