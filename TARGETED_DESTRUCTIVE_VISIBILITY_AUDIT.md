...
- Legacy route/schema/storage cleanup: complete for the targeted destructive/visibility scope. Legacy report routes, ownerless campaign compatibility, connected-data-source helpers, Google Sheets connection helpers, and transfer routes have been inventoried and guarded where retained. No legacy code was removed because no route was proven both unreachable and production-data independent.
...
- Legacy/stale routes and schema cleanup: complete for the targeted destructive/visibility scope. LinkedIn and Meta/Google Ads report route reachability, ownerless campaign behavior, connected-data-source helpers, Google Sheets connection helpers, and retained transfer routes are regression-inventoried. No code removal is approved without a separate production dependency proof.

## Authoritative Outstanding Queue

No remaining targeted destructive/visibility blockers are open in this tracker.

Optional future cleanup, not a blocker for this targeted audit:

- Remove or rewrite stale routes only after a separate production dependency proof confirms they are unreachable and not required by scheduler compatibility, existing production data, or ownerless legacy campaign migration.
