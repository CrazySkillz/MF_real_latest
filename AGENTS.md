...
- when a scheduled report is proven to reference a missing campaign, disable only that report's schedule instead of deleting report/audit history or continuing to retry it forever, including the already-skipped send-event path
- campaign-specific KPI refresh jobs must not substitute aggregate metrics when the selected platform campaign/source value is missing; skip that KPI instead of writing misleading current values
