...
- [x] Completed locally: Shopify revenue OAuth now builds its callback URL from `SHOPIFY_REDIRECT_URI` first, then browser/request/forwarded public host values, with Shopify/app base URLs only as fallbacks; the connect response also returns the exact redirect URI sent to Shopify so whitelist mismatches are verifiable.
