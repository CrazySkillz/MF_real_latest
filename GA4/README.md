...
- The chronological non-scheduler Overview remaining-work queue is documented as Current Commits 11–18 in `GA4/OVERVIEW_PRODUCTION_READINESS.md`. Commit 11 is implemented and locally validated; deployment and UI validation remain pending, and Commits 12–18 have not started. The queue separately tracks Pipeline Proxy scope, financial availability, ordered-source validation parity, regression/document alignment, deployed edge-state evidence, live windows/OAuth durability, retained-source/data disposition, and final downstream certification. Explicit scheduler exclusions remain unproven.
...
- `GA4/OVERVIEW_PRODUCTION_READINESS.md`
  Canonical whole-tab Overview production-readiness source of truth. Current status: not production-ready; Commit 2 is deployed and bounded UI validation passed, while later source, failure-state, freshness, cleanup, and downstream gates remain. It also contains the future-platform template and readiness checklist for Meta, Google Ads, LinkedIn, and other sources.
- `GA4/KPIS.md`
...
- GA4 Overview financial cards use one selected scoped GA4 financial source for native revenue and CPA conversions in fixed order: campaign-to-date provider totals, persisted daily totals where available to the caller, then configured-lookback breakdown only when earlier complete candidates are absent. Valid zero and negative values remain authoritative; selection is never based on maximum revenue. Browser and scheduled/server GA4 Overview output follow the same contract. Commit 2 deployed UI smoke validation passed for one configured campaign/window, while full 30/60/90 provider evidence and later Overview blockers remain open; `GA4/OVERVIEW_PRODUCTION_READINESS.md` is canonical.
