...
| Current Commit 2e: Google Sheets spend daily scheduler timer execution | Outstanding deployed validation gate, not a confirmed code bug. | Daily external-value auto-refresh timer for Google Sheets spend in the validated campaign/source path. | Prove the scheduled timer path without the run-now route: either wait for the normal daily timer or temporarily set `AUTO_REFRESH_RUN_ON_STARTUP=true`, redeploy/restart once, capture scheduler log timing plus before/after endpoint and UI evidence for the same source behavior, then remove the startup flag and redeploy. | No for local code-scope certification; yes for daily timer-fired scheduler certification. |
...
2. Next queued validation before other source families, if the goal is to close Google Sheets spend caveats first: Current Commit 2e daily scheduler timer execution.
3. After Current Commit 2e, run Current Commit 2f for a second campaign/property context and Current Commit 2g for alternate Google Sheets tabs/mappings if broader Google Sheets spend certification is required before moving on.
