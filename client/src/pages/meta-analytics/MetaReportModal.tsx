...
export function MetaReportModal(props: Props) {
  const {
    isReportModalOpen,
              ...
              <div className="space-y-4 pt-4 border-t">
                ...
                <Accordion type="multiple" className="w-full">
                  ...
                  <AccordionItem value="meta-derived-metrics">
                    ...
                    <AccordionContent>
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        {["ctr", "cpc", "cpm", "cpp", "frequency", "conversionRate", "costPerConversion"].map((metric) => {
                          const labels: Record<string, string> = {
                  ...
                  {summary?.hasRevenueTracking === 1 && (
                    <AccordionItem value="meta-revenue-metrics">
                      ...
                      <AccordionContent>
                        <div className="grid grid-cols-2 gap-3 pt-2">
                          {["roi", "roas", "totalRevenue", "profit", "profitMargin"].map((metric) => {
                            const labels: Record<string, string> = {
                      ...
                      <div className="space-y-3 pt-2">
                        ...
                        <div className="space-y-2">
                          {[
                            ...
                            { key: "byObjective", label: "Breakdown by Objective" },
                            { key: "byPlacement", label: "Breakdown by Placement" },
                          ].map((item) => {
                  ...
                  <AccordionItem value="demographics">
                    ...
                    <AccordionContent>
                      {(() => {
                        const selected = Array.isArray((customReportConfig as any).demographicsSections)
                          ? (customReportConfig as any).demographicsSections
                          : [];
                        ...
                        const toggle = (key: string, enabled: boolean) => {
                          ...
                          setCustomReportConfig({
                            ...customReportConfig,
                            demographicsSections: arr,
                            includeDemographics: arr.length > 0,
                          } as any);
                        };

                        const items: Array<{ key: string; label: string }> = [
                          { key: "age", label: "Age breakdown" },
                          ...
                          <div className="space-y-3 pt-2">
                            ...
                            <div className="space-y-2">
                              {items.map((i) => (
                                <div key={i.key} className="flex items-center space-x-2">
                                  <Checkbox
                                    id={`demographics-${i.key}`}
                                    checked={selected.includes(i.key)}
                                    onCheckedChange={(v) => toggle(i.key, !!v)}
                                    data-testid={`checkbox-demographics-${i.key}`}
                                  />
                                  <Label htmlFor={`demographics-${i.key}`} className="text-sm cursor-pointer">
                                    {i.label}
                                  </Label>
                                </div>
                              ))}
