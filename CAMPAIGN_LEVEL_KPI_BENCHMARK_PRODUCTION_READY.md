...
For GA4 traffic metrics, the connected-platform current value is cumulative from the saved initial-import boundary through the latest completed reporting day. The initial 30/60/90-day selection, where supported, establishes the starting boundary; it must not become a rolling window after setup. Native GA4 financial KPI and Benchmark values use that same fixed boundary, while imported Revenue and Spend use all available mapped records. Saved timeframe, tracking-period, app-creation, and budget-period dates must not replace these authoritative current-value boundaries.
...
- [x] Commit 4: Regression and final validation. Add tests proving connected-platform totals feed campaign-level KPI and Benchmark current values after source updates.
- [x] Commit 4: Regression and final validation. Run targeted tests, `npm run check`, and `npm run build`.
...
## Final Validation Root Cause And Result

Commit 4 did not require another production-path code change. The remaining gap was proof and documentation: the tracker still required a regression showing that changed connected-platform totals feed campaign-level KPI and Benchmark current-value calculations, plus final targeted validation.

Validation completed on 2026-05-18:

- Render validation passed: after source-refresh/scheduler execution, campaign-level KPI and Benchmark values stayed aligned with the refreshed connected totals.
- `npm test -- server/campaign-current-value-calculation-regression.test.ts server/campaign-scheduler-current-value-regression.test.ts server/campaign-alert-current-value-regression.test.ts server/campaign-benchmark-ui-regression.test.ts server/campaign-kpi-ui-regression.test.ts server/ga4-auto-refresh-regression.test.ts server/ga4-benchmark-regression.test.ts server/benchmark-route-isolation-regression.test.ts`
- `npm run check`
- `npm run build`

The first non-escalated build attempt failed before execution with sandbox `spawn EPERM`; the same build passed when rerun with escalation. The production build emitted existing chunk-size warnings only.
