...
### Current Commit 13 — Ordered financial-source validation parity — implemented and locally validated; deployment/runtime validation pending
...
- Local evidence: the focused financial-source parity, Benchmark provider-validation, and GA4 UI packet passed 44/44; the runner syntax check, TypeScript, production build, and `git diff --check` passed. Independent functional cases prove first-candidate zero/negative retention and incomplete-candidate fallthrough. The three directly affected legacy HubSpot runner tests still stop only at their known obsolete `2026-07-12.6` version assertion; all nine such assertions remain explicitly assigned to Commit 14.
- Unproven until deployment: runner `2026-07-30.10` and the Benchmark validation comparison on a deployed configured campaign. No visible Overview calculation, provider request, endpoint response shape, storage/schema, persistence, scheduler, connection, token, or production data changed.

### Current Commit 14 — Regression and documentation alignment — documented, not implemented

- Fix nine obsolete validation-runner version assertions, the pre-Commit-10 unscoped scheduler assertion, and add the remaining exact negative guards for Commits 11–12 while preserving Commit 13 parity guards.
