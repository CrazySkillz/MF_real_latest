...
- GA4 KPI historical certification evidence remains recorded, but the August 2 Current Commit 10 re-certification keeps the status **not production-ready / `UNVERIFIED`**. The deployed Commit 9 migration and local GA4 packets pass; current deployed state, timer, provider/token, and complete browser/report gates remain pending in `GA4/KPIS_PRODUCTION_READINESS.md`.
...
- Certification integrity rule: GA4 readiness is valid only for the exact certified SHA/configuration/dependency boundary. A relevant upstream or downstream change automatically invalidates affected tabs. The GA4 KPI record is `GA4/certifications/ga4-kpis.json` and its checker is `npm run check:ga4-kpi-certification`. Current Commits 1-9 implement the guarded contracts; Current Commit 10 reran the local evidence and inspected the deployed migration read-only, but the record remains `UNVERIFIED` until every external gate is current and SHA-bound. Copied formulas and source-text guards are structural evidence only.
