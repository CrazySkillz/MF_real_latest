...
**CLEAN-CERTIFIED / PRODUCTION_READY for the Executive Financials cards and their “Sources used” provenance at exact Render revision `f4fe2f3e3b47a3b350dd14c8a8d336bc8ff946ca`, within the campaign, property, sources, and value states below.** All required gates for this bounded consumer result passed. The cards now use the exact `financialRevenue`, `financialSpend`, `financialROAS`, and `financialROI` values calculated for Overview Revenue & Financials. Profit uses those same Revenue and Spend values. When the Overview Spend value lacks verified source details, Executive Financials withholds Spend and dependent values instead of presenting its possible false zero. Different reporting cutoffs remain disclosed; ratios across them are not described as same-period comparisons.
...
The deployed native GA4 response covered `2026-09-08` through the last completed Amsterdam day, `2026-09-16`. Spend source-to-date and its breakdown covered `1900-01-01` through `2026-09-16`. The existing imported Revenue endpoint and breakdown covered `1900-01-01` through current UTC date `2026-09-17`. The earlier `e2a8e361` read-only database inventory found zero imported records after `2026-09-16`; that record-level check was not repeated at `f4fe2f3e`. Current exact-deployment source breakdowns still summed to the displayed source totals, and the mixed-cutoff note remained visible.

The Executive Financials footer displays the three actual API cutoff dates whenever they differ and states that Profit, ROAS, and ROI combine those source windows. A controlled $100 current-day imported Revenue response demonstrated this disclosure on the previous `e2a8e361` revision; it was not repeated at `f4fe2f3e`. The current deployed normal state showed the disclosure. These numeric ratios are formulas over the displayed source totals; **they are not represented as same-period performance when the cutoffs differ**.
...
Unverified outside this certificate: real Google Sheets, CSV, CRM, Shopify, or GA4 provider refresh/reconnect and scheduler lifecycles on this revision; campaigns/properties/currencies/source configurations outside the exact inventory; future source data or changed provider/configuration behavior; whole-tab Insights status. The existing whole-Insights certificate is a historical record for `4be16c54c550a45dbf3104313c820ea47b453604` and was not edited or renewed. Its repository checker remains failed for current whole-tab dependency hashes; that failure is **not** counted as a pass or silently treated as an Executive Financials certificate. The separate Overview certificates likewise retain only their documented boundaries and revisions.

`APP_PRODUCTION_READINESS.md` was not updated. This evidence document is the only new certification document for this work.
