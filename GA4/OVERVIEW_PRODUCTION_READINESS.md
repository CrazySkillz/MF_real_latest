...
| Current Commit 2f.2 Google Sheets spend refresh edit-preview metadata guard | `npm test -- server/ga4-auto-refresh-regression.test.ts`; `npm run check`. | `/api/campaigns/:id/spend/sheets/process` now persists fresh `sheetHeaders`, `sheetSampleRows`, and `sheetRowCount` from the sheet rows fetched during process/reprocess. This keeps the edit-mode preview metadata aligned with the same refreshed sheet data used to materialize `spend_records`. | Locally regression-covered only until deployed. Existing stale mapping metadata for source `62772549-88dc-4cc5-bfe6-2e991d518ef5` requires a bounded self-heal by rerunning that source's run-now refresh after deploy, then opening edit mode and confirming the old `$198.75` preview is gone and the updated `$300` row / `$807.70` total context is visible. |
...
| Current Commit 2f: Google Sheets spend campaign/property portability | Partially validated for second-campaign add/import, edit/update, and refresh totals/source identity; refresh edit-mode preview metadata is blocked pending Current Commit 2f.2 deploy validation; delete/deactivate is still unproven. | A second disposable campaign/GA4 property context with Google Sheets spend. | Deploy Current Commit 2f.2, rerun the same source run-now refresh for source `62772549-88dc-4cc5-bfe6-2e991d518ef5`, verify spend remains `$807.70`, then open edit mode and confirm the old `$198.75` preview is gone and the updated `$300` row / `$807.70` context is visible. After that, run delete/deactivate as a separate packet. | No for local code-scope certification; yes for multi-campaign/property certification until 2f.2 deployed self-heal/edit-mode validation and second-campaign delete/deactivate evidence are captured. |
...
| Current Commit 2f.2: Google Sheets spend refresh edit-preview metadata guard | Implemented and regression-covered locally; pending deploy validation. | Google Sheets spend refresh/reprocess source metadata used by edit mode after `/spend/sheets/process`. | Persist fresh sheet preview metadata from the fetched rows during process/reprocess so edit mode no longer shows stale `mappingConfig.sheetSampleRows` after a successful refresh. This does not change spend totals, spend rows, source identity, campaign/property/source scoping, scheduler timing, report/email behavior, or API ownership. | No for local code-scope certification after tests; yes for deployed second-campaign refresh/edit-mode certification until the affected source is reprocessed after deploy and edit mode is confirmed fresh. |
| Current Commit 2g: Google Sheets spend tab/mapping variants | Outstanding deployed validation gate, not a confirmed code bug. | Google Sheets spend tabs and mapping shapes beyond the single validated tab/mapping. | Validate separate controlled packets for a different tab, campaign-filter mapping, date-column mapping, and multi-row total; each packet must prove source identity, totals, no duplicate source, source modal parity, and revenue non-interference. | No for local code-scope certification; yes for tab/mapping breadth certification. |
| Current Commit 2h: Overview financial report/email propagation for source-backed spend | Outstanding downstream validation gate, not a confirmed code bug. | Browser/downloaded report values, scheduled/server report values, and email delivery after source-backed spend changes. | Generate a report after a controlled source-backed spend state and prove report values match Overview endpoints; for scheduled email, prove provider delivery event or inbox receipt plus attachment values. Provider/API acceptance alone is not proof of delivery. | No for local Overview code-scope certification; yes for report/email propagation certification. |
...
- Existing-data boundary: existing stale `mappingConfig.sheetSampleRows` for source `62772549-88dc-4cc5-bfe6-2e991d518ef5` is not fixed merely by deploying code. The bounded self-heal is to rerun the same source's Google Sheets spend run-now refresh after deploy, then open edit mode and confirm the stale `$198.75` preview is gone and the updated `$300` row / `$807.70` context is visible. No direct database cleanup is required if that self-heal passes.
...
3. Current Commit 2f second-campaign add/import is closed only for the visible Google Sheets spend UI/source-modal financial path at `$507.70` and the post-fix endpoint recheck; Current Commit 2f.1 is closed for deployed no-completed-window endpoint behavior; second-campaign edit/update endpoint evidence is closed for `$706.45`; second-campaign refresh totals/source identity partially passed at `$807.70`, but edit-mode preview freshness is pending Current Commit 2f.2 deploy validation. After 2f.2 deploy validation, run delete/deactivate as a separate evidence packet. Current Commit 2g for alternate Google Sheets tabs/mappings remains next after 2f. If strict normal wall-clock scheduled-hour proof is required, capture that natural timer run separately rather than mixing it into 2f.
4. Run Current Commit 2h separately for report/email propagation; do not treat live Overview UI evidence as report/email proof.
5. Run Current Commit 3 as a read-only production source-damage inventory before any cleanup claim; do not delete or rewrite rows until the exact damaged-data boundary is documented.
6. Keep evidence packets separate: do not combine Google Sheets spend with revenue latency validation, another source family, Reports evidence, scheduler evidence, or damaged-data inventory.
7. If any action changes an unrelated source ID, duplicates an active source, loses provenance, changes totals unexpectedly, broadens source scope, or still shows a false failed response after a successful delete, stop Current Commit 2 and add the exact runtime fix as the next Current Commit.
