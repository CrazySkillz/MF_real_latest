...
describe("GA4 reporting timezone contract", () => {
  it("persists a campaign reporting timezone with UTC fallback", () => {
    ...
    expect(routes).toContain('getExpectedDailyRefreshAt');
    expect(routes).toContain('getReportingDateWindow');
    expect(routes).toContain('normalizeReportingTimeZone');
    expect(routes).toContain('resolveGA4DailyFreshness');
    ...
    expect(campaigns).toContain('data-testid="select-create-reporting-time-zone"');
    expect(campaigns).toContain('onValueChange={(value) => form.setValue("reportingTimeZone", value, { shouldDirty: true, shouldValidate: true })}');
