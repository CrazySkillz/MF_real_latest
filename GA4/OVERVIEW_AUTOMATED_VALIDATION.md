...
Current Commit 10 adds scheduler-consumer guards for the shared full GA4 financial selector, campaign-to-date GA4-context revenue/spend reads, Trend GA4-context predicates, valid zero revenue ROAS/ROI, and `performance_summary_aggregate_v2` compatibility. These are local regression proofs only until the change is deployed and the named browser/scheduled artifact comparisons pass.
