...
describe("GA4 UI regression guard", () => {
  ...
  it("shows Google Sheets and CSV revenue with the established source breakdown layout", () => {
    const ga4Metrics = readClient("pages/ga4-metrics.tsx");

    expect(ga4Metrics).toContain('const isGoogleSheets = sourceType === "google_sheets";');
    expect(ga4Metrics).toContain('const isCsv = sourceType === "csv";');
    expect(ga4Metrics).toContain('const hasSingleSourceBreakdown = sourceType === "shopify" || isGoogleSheets || isCsv;');
    expect(ga4Metrics).toContain('const sourceDisplayText = isGoogleSheets ? "Google Sheets" : isCsv ? "CSV" : revenueSourceDisplayLabel(s);');
    expect(ga4Metrics).toContain('const csvFileName = String(s.displayName || cfg?.displayName || "CSV").trim() || "CSV";');
    expect(ga4Metrics).toContain('const sourceDetailText = isGoogleSheets && String(cfg?.sheetName || "").trim() ? String(cfg.sheetName) : isCsv ? csvFileName : sourceTypeText;');
    expect(ga4Metrics).toContain('{hasSingleSourceBreakdown && (');
    expect(ga4Metrics).toContain('aria-label={`Remove ${isGoogleSheets ? "Google Sheets" : isCsv ? "CSV" : "Shopify"} revenue source`}');
  });
  ...
  it("prevents Add and Edit Revenue modal transitions from shifting the page", () => {
    ...
    expect(revenueModal).toContain("<DialogContent data-add-revenue-dialog");
  ...
  it("shows active Google Sheets and CSV revenue status in the Add Revenue picker", () => {
    ...
    expect(revenueModal).toContain('csv: csvSources.length > 0,');
    expect(revenueModal).toContain("importSourceStatus.google_sheets");
    expect(revenueModal).toContain("<span>Connected</span>");
    expect(revenueModal).toContain('<span aria-hidden="true">|</span>');
    expect(revenueModal).toContain("Add another sheet");
    expect(revenueModal).toContain('if (importSourceStatus.google_sheets) return;');
    expect(revenueModal).toContain('title="Disconnect Google Sheets"');
    expect(revenueModal).toContain("/ga4/google-sheets/disconnect");
  ...
  it("uses the fixed initial-import window across Overview live tables", () => {
    const ga4Metrics = readClient("pages/ga4-metrics.tsx");
    ...
    expect(ga4Metrics).toContain('queryKey: ["/api/campaigns", campaignId, "ga4-landing-pages", "import-to-date", selectedGA4PropertyId]');
    expect(ga4Metrics).toContain('queryKey: ["/api/campaigns", campaignId, "ga4-conversion-events", "import-to-date", selectedGA4PropertyId]');
    ...
    expect(conversionRoute).toContain("const ga4DateRange = importToDateWindow?.startDate || explicitStartDate || toGA4LookbackStartDate(dateRange, '90daysAgo');");
    expect(landingRoute).toContain("if (windowMode === 'import-to-date')");
    expect(conversionRoute).toContain("if (windowMode === 'import-to-date')");
    expect(landingRoute).toContain("importToDateWindow?.startDate || explicitStartDate");
    expect(conversionRoute).toContain("importToDateWindow?.startDate || explicitStartDate");
    ...
    expect(landingMethod).toContain("dateRanges: [{ startDate: dateRange, endDate }]");
    expect(conversionMethod).toContain("dateRanges: [{ startDate: dateRange, endDate }]");
  });

  it("labels the cumulative Campaign Breakdown revenue window explicitly", () => {
    const ga4Metrics = readClient("pages/ga4-metrics.tsx");
  ...
  it("discloses the cumulative Campaign Breakdown window in browser-generated GA4 Reports", () => {
    const ga4Metrics = readClient("pages/ga4-metrics.tsx");
  ...
  it("keeps GA4 Insights trend history requirements aligned to selected mode", () => {
    ...
    expect(ga4Metrics).not.toContain("Last scheduler refresh <span");
  ...
  it("keeps the selected GA4 property aligned after Set as Primary", () => {
    ...
    expect(ga4Metrics).toContain('queryKey: ["/api/campaigns", campaignId, "ga4-breakdown", activeTab === "insights" ? dateRange : "import-to-date", selectedGA4PropertyId, activeTab === "insights", insightsValidationReadOnly]');
