...
**Current status: UNVERIFIED.** The production-ready evidence below is historical for runtime boundary `12789c1ebb92dd6a905a9f2f0f877f0bc6a90627`. The current candidate changes all three Overview detail-table windows from rolling lookback to the fixed initial-import boundary through the latest completed day; local regression evidence is recorded below, but deployed authenticated parity is still required. The exact controlling status and exclusions remain in `GA4/OVERVIEW_PRODUCTION_READINESS.md`.
...
- historically, all three Campaign Breakdown rows reconciled exactly from native 30-day GA4 values plus exact campaign-matched source-to-date imports
- that historical rolling-window evidence does not validate the current cumulative-table candidate
...
| Campaign Breakdown | GA4 acquisition rows plus exact mapped imported campaign revenue | selected property/filter; fixed initial-import boundary through latest completed day; matched imports are source-to-date | Current cumulative route/UI/report behavior is locally regression-covered. Historical rolling-window deployed values do not certify this candidate; authenticated deployed parity remains required. |
| Landing Pages | GA4 rows with exact-key same-scope conversion supplementation | selected property/filter and fixed initial-import-to-latest-completed-day window; API limit 50, UI renders 20 | Current cumulative route/service behavior is locally regression-covered. Initial loading, successful empty, last-good background failure, and unavailable states remain distinct; deployed parity remains required. |
| Conversion Events | GA4 event rows with exact event-name supplementation | selected property/filter and fixed initial-import-to-latest-completed-day window; API limit 50, UI renders 25 | Current cumulative route/service behavior is locally regression-covered and does not allocate imported campaign revenue into event rows; deployed parity remains required. |
...
| Breakdown | `/ga4-breakdown?window=import-to-date` -> saved `importStartDate`/reporting-timezone end -> `getAcquisitionBreakdown` -> client aggregation/render | Selected property/filter and fixed cumulative window are locally covered; deployed parity is unverified. |
| Landing Pages | `/ga4-landing-pages?window=import-to-date` -> saved `importStartDate`/reporting-timezone end -> provider report -> exact-key supplement -> client first 20 rows | Exact cumulative bounds and state behavior are locally covered; deployed parity is unverified. |
| Conversion Events | `/ga4-conversion-events?window=import-to-date` -> saved `importStartDate`/reporting-timezone end -> provider report -> exact-event supplement -> client first 25 rows | Exact cumulative bounds and state handling are locally covered; deployed parity is unverified. |
...
### B1a. Overview detail tables incorrectly remained rolling — current candidate locally fixed, deployed validation open

The initial 30-day setting is an import seed, not a permanent display window. Campaign Breakdown, Landing Pages, and Conversion Events continued recalculating their start date from a rolling lookback while Summary already used the fixed saved `importStartDate`. The current candidate routes those three browser and scheduled-report table queries from the saved initial-import boundary through the latest completed campaign-reporting-timezone day. The primary 4-file packet passes 104 tests; the adjacent Insights, Ad Comparison, financial, source-lifecycle, and report-window packet passes 6 files / 45 tests; TypeScript and the production build pass. Authenticated deployed parity remains unverified.
