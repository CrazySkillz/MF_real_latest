...
- the KPI-create latency corrections remove only avoidable waiting: the create route reuses a successful KPI alert sweep already completed by the authoritative campaign recompute, retains the original synchronous fallback when that sweep was not attempted or failed, and preserves Benchmark reconciliation and immediate email ordering. A follow-up runs only independent recompute reads concurrently, closes the successful modal before cache refreshes finish, and prevents a duplicate Notifications refetch. Source precedence, provider fallback, calculation, persistence, alert, and email order remain unchanged. Final local evidence: affected packet 8 files / 100 tests passed; current-version suite 1,376 total / 1,347 passed / exactly 29 visible deferred failures / zero blocking failures; certification checker, TypeScript, and production build passed with 3,466 client modules. The follow-up is undeployed and therefore does not restore certification.
