...
6. Current Commit 4.5: deployed HubSpot provider lifecycle validation. Run the real/sandbox HubSpot before/after packet for add/import, edit/update, refresh/reprocess, and delete/deactivate. Evidence must record campaign ID, HubSpot account, selected deal filters/stages/date field, source IDs, totals, source-modal provenance, and confirm no duplicate source or unrelated source changes. Until this passes, HubSpot revenue remains unproven and not production-ready.
7. Current Commit 4.6: production HubSpot data-health inventory runner. Root cause confirmed locally: the HubSpot certification track had deployed provider lifecycle validation queued, but no HubSpot-specific, repeatable, read-only production-data inventory runner to bracket that validation without using other source-family evidence. Smallest safe local fix: extend the existing campaign-access-guarded GET source-damage inventory endpoint with separate `hubspotInventoryPass`, `hubspotSummary`, and `hubspotFindings`, and expose `GA4OverviewValidation.hubspotInventory({ campaignId })` for before/after runs. The existing broad `overallPass` source-damage semantics are unchanged. Local validation: `node --check client/public/ga4-overview-validation-runner.js`, `npm test -- server/hubspot-revenue-ga4-overview-regression.test.ts server/spend-source-additivity.test.ts`, and `npm run check`. Execution remains pending: run the deployed 4.6 inventory before and after Current Commit 4.5 provider lifecycle validation and record campaign ID, source IDs, finding arrays, and timestamps. Cleanup, if needed, must be a separate Current Commit with exact affected IDs.
8. Current Commit 4.7: HubSpot clean-certification update. Only after 4.5 and 4.6 pass, update this file with the validated provider/data evidence and mark HubSpot clean-certified for the exact proven campaign/provider scope. Do not use Google Sheets, CSV, KPI, Benchmark, Reports, or other source-family evidence as proof.
