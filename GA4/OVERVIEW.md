...
Current Commit 10 local status on `2026-07-30`: the downstream scheduled/manual Campaign DeepDive aggregate now reuses the shared ordered GA4 campaign-to-date financial selection, scopes persisted revenue/spend and Trend financial rows to GA4, preserves valid zero revenue for ROAS/ROI, and writes only `performance_summary_aggregate_v2` compatibility snapshots. Existing 90-day engagement/platform windows and all public contracts remain unchanged. Local tests pass within the recorded scope; the change is not deployed and deployed report/Trend/multi-source proof remains required.
