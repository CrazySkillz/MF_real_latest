...
describe("Google Ads revenue platform context", () => {
  it("adds Google Ads to storage revenue context filtering without routing non-GA4 totals through LinkedIn", () => {
    ...
    expect(storageFile).toContain("export type RevenuePlatformContext = 'ga4' | 'linkedin' | 'meta' | 'google_ads' | 'instagram' | 'tiktok';");
  ...
  it("permits Google Ads on revenue read and manual source endpoints", () => {
    ...
    expect(routesFile).toContain('const zPlatformContext = z.enum(["ga4", "linkedin", "meta", "google_ads", "instagram", "tiktok"]);');
    expect(routesFile).toContain('const zRevenueReadPlatformContext = z.enum(["ga4", "linkedin", "meta", "google_ads", "instagram", "tiktok"]);');
    ...
    expect(routesFile).toContain("sourcePlatformContext === 'google_ads' ? 'google_ads_revenue' : sourcePlatformContext === 'instagram' ? 'instagram_revenue' : sourcePlatformContext === 'tiktok' ? 'tiktok_revenue' : 'revenue'");
