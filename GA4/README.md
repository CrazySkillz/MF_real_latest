...
- The concise current Overview decision and active gates live in `GA4/OVERVIEW_PRODUCTION_READINESS.md`; detailed evidence lives in `GA4/OVERVIEW_PRODUCTION_READINESS_EVIDENCE.md`; chronological Current Commits 0–19 live in `GA4/OVERVIEW_PRODUCTION_READINESS_HISTORY.md`. Commit 18's corrective scheduler-backed Summary deployed as `e857c15d`, and the existing 30-day UI/daily parity packet passed. Current Commit 19 runtime `ba2e4329` deployed; the user confirmed `Last 30 completed days` and normally loaded metrics on existing `GA4 single` / `ga4_mock`. Its bounded implementation is closed. Unsupported-write rejection is automated/code-path proven and was not production-injected. The user approved keeping all four visible retained Spend sources. A post-publish Google Sheets reconnect and one no-click automatic mapped-value update were then user-confirmed on `2026-08-01`. The deterministic/read-only pack passes; current-release timer-fired GA4 scheduler validation and OAuth durability on `2026-08-07` or later remain open.

- The Overview tab is not yet clean-certified or production-ready for live users. Two mandatory tests remain: current-release GA4 scheduled-run validation, and OAuth durability validation on or after `2026-08-07`. Every other supported 30-day Overview requirement has passed. Commit 19's bounded packet is closed, and no cleanup was performed.
...
- GA4 KPI historical certification evidence remains recorded, but the August 1 fresh re-certification withdrew clean certification. Current status is **not production-ready / `UNVERIFIED`** pending the prevention-first Current Commits 1-10 in `GA4/KPIS_PRODUCTION_READINESS.md`.
- GA4 Benchmark historical certification evidence remains recorded, but the August 1 shared-dependency review withdrew current clean certification. Shared current-value/recompute/downstream paths are `UNVERIFIED`; use Benchmark Current Commits 8-11 in `GA4/BENCHMARKS_PRODUCTION_READINESS.md`.
...
- Certification integrity rule: GA4 readiness is valid only for the exact certified SHA/configuration/dependency boundary. A relevant upstream or downstream change automatically invalidates affected tabs. The GA4 KPI record is `GA4/certifications/ga4-kpis.json` and its checker is `npm run check:ga4-kpi-certification`. Current Commits 1-9 implement the fail-closed gate, dynamic cross-consumer guard, metric identity contract, authoritative traffic/rate window contract, bounded financial source/failure contract, alert/notification eligibility, UI/browser-consumer states, exact recompute/report proof, and persistence/destructive safety while the record remains `UNVERIFIED`; Current Commit 10 and external validation remain open. Copied formulas and source-text guards are structural evidence only.
- Shopify Revenue status (2026-07-15): **production-ready and clean-certified for the currently enabled GA4 Overview Admin API token scope**. `GA4/OVERVIEW_REVENUE_SHOPIFY_PRODUCTION_READINESS.md` is the canonical evidence record and supersedes the historical Shopify conclusions in this README and broader GA4 ledgers. Dormant OAuth and non-GA4 Shopify sources are explicitly outside this certification.
- Google Sheets/Upload CSV revenue readiness: Upload CSV Revenue is clean-certified for the validated documented scope after Current Commit 12 plus the final user-confirmed deployed invalid-file no-mutation result. Evidence includes transactional lifecycle coverage, exact `$150` / `$1,250` / `$150` UI values without duplicates, Revenue baseline restoration, unchanged Spend, 44/44 final local tests, runner syntax and TypeScript checks, and refreshed inventory with no active/reconciliation damage. Google Sheets Revenue remains on hold and is not clean-certified. Use `GA4/OVERVIEW_REVENUE_PRODUCTION_READINESS.md` for the exact exclusions.
- HubSpot Revenue readiness: **clean-certified and production-ready for the validated documented GA4 Overview scope** after H10d. `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md` is the canonical source-specific status and supersedes narrower historical Current Commit 4.x summaries. Its certification is aligned to the implemented atomic lifecycle/failure-retention, date/pagination/mapping, Pipeline Proxy, downstream financial/report/KPI/Benchmark/notification, multi-campaign, and damaged-data contracts. Future provider/API changes, unlisted configurations, future failures/deliveries, and non-GA4 contexts require fresh scoped evidence and are not part of the claim.
- Google Sheets Revenue hold: all further Google Sheets Revenue implementation, automation, provider/OAuth, scheduler, polling, damaged-data inventory, downstream propagation, and deployed validation coverage is on hold until the user explicitly resumes it. The separate revenue-source queue's Current Commit 8 proceeds only with Upload CSV Revenue downstream propagation automation; it is not whole-Overview Current Commit 8.

- Current GA4 spend certification focus: Google Ads live spend validation is deferred and must not be included in the current GA4 spend clean-certification claim. The current GA4 spend validation target is `Google Sheets` and `Upload CSV` only, using `GA4/FINANCIAL_SOURCES.md` plus `PRODUCTION_READINESS.md` and the strict no-overclaiming standard.
...
- Google Sheets spend continuity status: the recurring disconnect root cause was confirmed as Google OAuth `External + Testing`; Publishing status is now `In production`. The post-publish GA4 reconnect completed on `2026-07-30`, and the post-publish Google Sheets reconnect plus one deployed no-click automatic mapped-value update were user-confirmed on `2026-08-01`. Automatic renewal and more-than-seven-day durability remain unproven. Broader Google verification and exhaustive polling/failure behavior remain outside the whole-Overview non-scheduler decision. See `GA4/OVERVIEW_SPEND_PRODUCTION_READINESS.md`.
- Whole-Overview Current Commit 5 policy: until Google Sheets Revenue/Spend is certified, new GA4 setup is fail-closed in both UI and process APIs. Existing exact-source continuity is preserved for later inventory/lifecycle reconciliation. New GA4 CSV Spend is dated-only; already-undated saved sources are retained but unproven.
...
9. `GA4/OVERVIEW_PRODUCTION_READINESS_HISTORY.md` for the chronological Current Commit 0–18 and UI-validation record
...
- `GA4/OVERVIEW_PRODUCTION_READINESS.md`
  Concise canonical current-status index. Current status: implemented 30-day gates have passed; formal clean certification remains pending current-release timer-fired GA4 scheduler validation and the `2026-08-07` OAuth durability gate. It also records the exact included scheduler boundary and certification statement allowed after both pass.
...
- `GA4/OVERVIEW_PRODUCTION_READINESS_HISTORY.md`
  Chronological Overview Current Commit 0–18 and UI-validation ledger preserved from the pre-split canonical file.
- `GA4/KPIS.md`
...
- `GA4/KPIS_PRODUCTION_READINESS.md`
  Canonical whole-tab KPIs production-readiness source of truth. Current status: not production-ready / `UNVERIFIED`. It contains the controlling August 1 evidence boundary and prevention-first Current Commits 0-10; June validation is historical only.
...
- `GA4/BENCHMARKS_PRODUCTION_READINESS.md`
  Canonical whole-tab Benchmarks production-readiness source of truth. Current status: not clean-certified / `UNVERIFIED` for shared current-value/recompute/downstream paths. Historical July packets remain bounded; active Benchmark Current Commits are 8-11.
...
- `GA4/REPORTS_PRODUCTION_READINESS.md`
  Canonical whole-tab Reports production-readiness source of truth. Current status: not clean-certified as a whole section for KPI-bearing output; active Reports Current Commits are 7-9. Historical CRUD/delivery/PDF evidence remains bounded.
...
- HubSpot-specific certification is canonical in `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md`. H10d records HubSpot Revenue as clean-certified and production-ready for the validated documented GA4 Overview scope and reconciles the historical 4.13-4.16 packets with the implemented H1-H9 lifecycle and downstream contracts. The explicit future/excluded boundaries in that file remain non-certified and require fresh scoped evidence if later requested.
- Shopify-specific GA4 Overview readiness is tracked canonically in `GA4/OVERVIEW_REVENUE_SHOPIFY_PRODUCTION_READINESS.md`. Its 2026-07-15 conclusion is production-ready and clean-certified for the currently enabled Admin API token scope; historical packets elsewhere remain bounded supporting evidence rather than the basis for that conclusion.
...
- live Overview Summary cards must use scheduler-backed persisted daily facts for the selected property, saved campaign filter, and configured completed-day lookback as one coherent source; campaign-to-date totals remain reserved for the explicitly labeled financial section
...
- GA4 Overview financial cards use one selected scoped GA4 financial source for native revenue and CPA conversions in fixed order: campaign-to-date provider totals, persisted daily totals where available to the caller, then configured-lookback breakdown only when earlier complete candidates are absent. Valid zero and negative values remain authoritative; selection is never based on maximum revenue. Browser, scheduled/server output, validation runner, and Benchmark comparison follow the same contract. Commit 13 deployed evidence proved provider-first selection even when persisted-daily revenue was one cent higher; full live-window, edge-fixture, and final downstream evidence remains open. `GA4/OVERVIEW_PRODUCTION_READINESS.md` is canonical.
...
- GA4 KPI whole-tab status is not production-ready and remains `UNVERIFIED`; `GA4/KPIS_PRODUCTION_READINESS.md` is controlling. Current Commits 4-7 locally align the traffic/rate window, bounded financial source/failure, backend alert/notification eligibility, and UI/browser-consumer state contracts, but Current Commits 8-10 and all listed external gates remain open.
...
- GA4 Benchmark whole-tab status is not currently clean-certified; shared current-value/recompute/downstream paths are `UNVERIFIED`. `GA4/BENCHMARKS_PRODUCTION_READINESS.md` is controlling.
