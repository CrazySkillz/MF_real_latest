...
- **Google Sheets Revenue is not clean-certified.** Current Commit 4 locally closes the tested GA4 deterministic mapping/date/row validation boundary for foreground and scheduler reprocessing. Foreground add/edit remains non-transactional. Whole-Overview Commit 17 makes scheduler replacement and source-freshness update transactional locally, but deployed provider/failure/idempotency evidence is incomplete; automatic updates still rely on the daily external refresh path rather than bounded source-family polling, and open Overview revenue queries refetch only every ten minutes.
