...
| Current Commit 2d Google Sheets spend refresh validation trigger | `npm test -- server/ga4-auto-refresh-regression.test.ts`; `npm run check`. | The route is campaign/source-scoped, requires campaign access, reuses the scheduler Google Sheets spend reprocess function, and avoids a full daily scheduler run. | Does not prove the deployed route until the pushed commit is deployed and run against the changed Sheet with before/after endpoint and UI evidence. |
...
| Current Commit 2d: Google Sheets spend refresh validation trigger | Implemented locally; pending deployed validation. | One active Google Sheets spend source in one authorized campaign. | Add a campaign/source-scoped validation route that reuses the scheduler Google Sheets spend reprocess function for `/api/campaigns/:id/spend-sources/:sourceId/google-sheets-refresh/run-now`; do not run the full daily scheduler, unrelated providers, reports, emails, alerts, or unrelated campaigns. | No for local code-scope certification; yes for deployed Google Sheets spend refresh/reprocess certification until endpoint and UI evidence are captured. |
...
- Certification boundary: Current Commit 2d proves only the local source-scoped validation trigger shape and route wiring. Google Sheets spend refresh/reprocess remains unproven for deployed production readiness until Render deploys the commit, the existing source `8f67b03f-a00b-434f-b81f-db1b2b951595` is refreshed through the run-now route after a controlled sheet amount change, endpoint after-state evidence confirms same source ID/no duplicate/expected spend, and the UI is confirmed.
...
1. Google Sheets spend add/import and edit/update are closed only for source `8f67b03f-a00b-434f-b81f-db1b2b951595` in the validated deployed campaign/source path.
2. Next source family/action after deploy: validate Google Sheets spend refresh/reprocess for source `8f67b03f-a00b-434f-b81f-db1b2b951595` with the new run-now route.
3. Keep evidence packets separate: do not combine Google Sheets spend with revenue latency validation or another source family.
4. Keep Current Commit 2c separate from spend evidence; the user-confirmed latency validation does not prove Google Sheets spend behavior.
5. For the next packet, change only the existing Google Sheet spend value, call `/api/campaigns/:id/spend-sources/:sourceId/google-sheets-refresh/run-now` for the same source ID, then capture endpoint after-state plus UI confirmation before any delete action.
...
- Google Sheets revenue delete/deactivate is closed only for the validated deployed campaign/source path recorded above. Current Commit 2c deployed latency timing is user-confirmed closed without a numeric SLA. Google Sheets spend add/import and edit/update are closed only for the validated deployed campaign/source path recorded above. Current Commit 2d added local Google Sheets spend refresh/reprocess trigger wiring, but deployed refresh/reprocess and delete/deactivate remain unproven until each action has deployed endpoint evidence plus UI parity.
