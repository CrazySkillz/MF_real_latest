...
describe("campaign Executive Summary regression guard", () => {
  ...
  it("renders one live-only Executive Summary section from aggregate availability", () => {
    ...
    expect(page).toContain('Total connected revenue is ${formatAggregateCurrency("revenue")}.');
    expect(page).toContain('Connected web analytics recorded ${formatAggregateInteger("conversions")} conversions.');
    ...
    expect(page).toContain("No connected paid-media source is available. Actions shown below are limited to connected web analytics, outcome metrics, and configured targets.");
