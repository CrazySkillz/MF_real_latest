...
- CSV revenue and spend preview routes require campaign access before uploaded files are parsed or previewed.
...
- Spend source deletion now deletes normalized spend records tied only to the already campaign-confirmed source ID before recalculating campaign spend, matching the revenue-source delete cleanup model and preventing stale normalized records.
...
- Meta ad-platform spend edit/import routes now require campaign access, preserve stable spend source identity when a `sourceId` is supplied, reject wrong source types or wrong ad-platform labels, and replace only that source's normalized spend records instead of appending duplicates.
- Meta connection delete routes require campaign access before deleting a connection, delete only by `campaignId`, and return `404` when no connection row is deleted.
- Google Ads connection delete routes require campaign access before deleting a connection, delete only by `campaignId`, and return `404` when no connection row is deleted.
- Campaign deletion now removes report send-event audit rows proven by campaign-owned report snapshots, while refusing to touch send events for report IDs that still belong to another campaign.
- Client deletion now fails closed before the transaction if the client contains any campaign row outside the caller's owner scope, preventing orphaned campaigns or cross-owner data loss.
- Campaign delete table coverage is regression-guarded against `shared/schema.ts`: every direct `campaignId` table must be either deleted, soft-hidden where appropriate, or handled through the optional-table guard.
- Campaign/client delete runtime validation passed after deploy with the expected campaign/client disappearance and no delete-time Render errors.
- Legacy report route reachability is now regression-inventoried: `/api/linkedin/reports` has no frontend caller but remains scheduler-referenced, while `/api/meta/reports` is active for both Meta and Google Ads and cannot be removed or split without a later schema-backed platform discriminator.
- Ownerless legacy campaign compatibility is now regression-inventoried as claim-based behavior in `ensureCampaignAccess` and `/api/campaigns`; do not broaden ownerless visibility or remove claim behavior without an explicit migration plan.
- Connected data source list and preview routes are active UI dependencies and now require campaign access before exposing source metadata or source preview rows.
- Google Sheets connection list and primary-selection routes are active UI dependencies and now require campaign access. Primary selection also verifies the target active connection belongs to the campaign before clearing existing primary flags.
- Legacy transfer routes for Meta, Google Sheets, and LinkedIn now require access to both source and target campaigns before mutation. Google Sheets transfer deletes the source connection by connection ID instead of campaign ID.
...
- Source edit/delete and normalized-record cleanup: started. Google Sheets campaign source read/update/helper routes are access-guarded and connection mutations verify campaign membership. CSV revenue/spend preview/process/edit/delete paths are code/test complete for the targeted scope: previews require campaign access before parsing uploads, edit mode refuses non-CSV `sourceId` updates, individual revenue source delete verifies campaign/source ownership before deleting revenue records, and spend source delete now removes only normalized spend records tied to the already-confirmed source ID. Google Sheets GA4 revenue/spend add/edit/refresh identity is hardened so add mode creates an additive source and edit/refresh mode updates only by `sourceId`; spend auto-refresh now passes that `sourceId` instead of creating duplicate sources. Google Sheets spend duplicate cleanup is restricted to exact duplicate groups and hard-delete purge is restricted to inactive zero-record duplicates. HubSpot, Salesforce, and Shopify revenue save/refresh endpoints now validate supplied `sourceId` against campaign, platform context, and source type before mutation, their connection delete routes verify the connection belongs to the requested campaign before deactivation, the direct source-delete route proves campaign/source ownership before deleting source records, scheduler stale-source 404s are skipped as stale sources rather than false failed reprocesses, LinkedIn spend/edit/refresh/disconnect/revenue-cleanup source boundaries are code/test complete for the targeted scope, Meta spend/edit/import/disconnect source boundaries are code/test complete for the current test-mode scope, Google Ads disconnect is fail-closed, and Custom Integration UI connect/read/metrics/PDF-upload/transfer routes are campaign-access guarded before source mutation.
- Campaign/client delete cascades: code-audited for the targeted scope. Campaign delete access, child cleanup reuse, report snapshot/send-event orphan boundaries, client-delete foreign-campaign fail-closed behavior, and schema-derived direct `campaignId` table coverage are regression-guarded.
- Legacy route/schema/storage cleanup: started with read-only route reachability inventory and active source/transfer route access hardening. No legacy code has been removed.
...
- Source delete/edit flows: started. Google Sheets source route access, connection ownership checks, CSV preview access/source-type edit/delete guards, GA4 revenue/spend add/edit/refresh source identity, read-only Google Sheets spend duplicate inspection, confirmed Google Sheets spend duplicate cleanup, inactive zero-record hard-delete purge, HubSpot/Salesforce/Shopify revenue save/refresh `sourceId` fail-closed guards, HubSpot/Salesforce/Shopify connection delete membership guards, direct GA4-context revenue source delete campaign/source ownership, stale CRM/ecommerce source scheduler skips, LinkedIn spend edit/refresh `sourceId` preservation, LinkedIn revenue cleanup HubSpot pipeline context guarding, LinkedIn disconnect fail-closed campaign-scope guards, Meta ad-platform spend/connection source-flow guards, Google Ads disconnect fail-closed guards, and Custom Integration UI connect/read/upload/transfer guards are complete for the targeted pass.
- Campaign/client delete cascades: code-audited for the targeted scope. Campaign delete route access, campaign-owned report snapshot/send-event cleanup, client-delete foreign-campaign fail-closed behavior, and schema-derived direct `campaignId` table coverage are covered by regression guards.
- Legacy/stale routes and schema cleanup: started. LinkedIn and Meta/Google Ads report route reachability is regression-inventoried; no code removal is approved yet.

## Authoritative Outstanding Queue

1. Legacy/stale route and schema/storage cleanup.
   ...
   Current progress: first report-route reachability inventory is complete. LinkedIn report routes are not frontend-called but remain scheduler-compatible legacy routes; Meta report routes are active shared Meta/Google Ads routes with no persisted platform discriminator. Ownerless legacy campaign behavior is inventoried as centralized first-access claim behavior, not as permission to expose unrelated data. Connected data source list/preview, Google Sheets connection list/primary-selection, and legacy transfer routes were proven reachable and guarded rather than removed.
   Validation: `server/legacy-route-reachability-regression.test.ts`, `server/campaign-delete-cascade-regression.test.ts`, `npm run check`, and `npm run build` passed before commit `f0ab5337`.

   Remaining substeps:
   - Inventory unused source/platform/report routes by caller, scheduler dependency, storage dependency, and schema support.
   - Guard stale routes that are still reachable but unsafe.
   - Remove code only after proving it is unreachable and not required by production data.
   - Add or update regression coverage before marking any legacy cleanup complete.

## Next Step

The next safest smallest step is to continue legacy/stale cleanup with another route-family inventory slice before removing anything. Prioritize active source/platform routes that expose source data, then guard reachable unsafe routes and remove code only after proving it is unused and not production-data dependent.
...
- `server/campaign-delete-cascade-regression.test.ts`
- `server/legacy-route-reachability-regression.test.ts`
