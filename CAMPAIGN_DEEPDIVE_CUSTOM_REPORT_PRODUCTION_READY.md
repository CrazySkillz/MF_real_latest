...
- [x] Completed locally: Scheduled Reports card `Pause` disables the backend schedule with backend status `paused`, marks the local card as paused, keeps the report visible in Scheduled Reports, and scheduled cards now expose `Download last sent report` backed by the same latest-value PDF regeneration path.
...
- Scheduled Reports card `Pause` must disable the backend schedule where a backend report ID exists, mark the local card as paused, keep the report visible in Scheduled Reports, and show whether each scheduled report is currently `Enabled` or `Paused`
...
- Scheduled Reports card action fix added on 2026-05-29: `Pause` now disables the backend schedule, writes backend status `paused`, keeps paused cards visible with a `Paused` status, shows active cards as `Enabled`, and `Download last sent report` regenerates from latest values.
