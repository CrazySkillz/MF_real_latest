...
6. **Benchmark Current Commit 13 — prove exact deployed Benchmark-consumer parity:** blocked only on authorized controlled scheduled Benchmark/Insights artifacts; every available read-only consumer passed at deployed SHA `f636b3c52cabc608566daa0c55432f6e4e5c4eb8`.
...
8. **Benchmark Current Commit 15 — resolve email delivery and issue the final certification:** blocked by Current Commits 13-14 and the email decision/evidence below.
...
- Minor findings: 5, all fixed: a documentation contradiction in the historical threshold record; the read-only provider-validation route comparing campaign-to-date financial Benchmarks against a 30-day window instead of the real application financial window; one line-ending-sensitive UI source-text guard; one destructive-safety guard that still expected the superseded split-delete implementation instead of the safer transactional helper; and the deployed validator capturing traffic-backed consumers before their browser input reached a terminal state.
...
- Current Commit 13 read-only deployed parity: exact deployed SHA `f636b3c52cabc608566daa0c55432f6e4e5c4eb8` returned `200`. The authenticated validator inspected `2` campaigns and `4` active GA4 Benchmarks with zero application mutation attempts. For the configured property, Conversions `95` and Revenue `51405.93` matched live provider, persisted rows, scheduler candidates, UI candidates, cards, Tracker, Insights conclusions, and the downloaded browser Benchmark PDF; alert decisions also matched exactly (Revenue not breached/no notification, Conversions below `250`/one current notification with matching value, threshold, condition, and action URL). The campaign without a property failed closed to the connection state, and its saved zero rows produced no false notifications. Production has `3` active GA4 reports but only one Benchmark-consuming report, a scheduled Benchmark report with no sent event or snapshot; no deployed Insights report artifact exists. Current Commit 13 therefore remains blocked only on a user-authorized controlled Benchmark/Insights report event and captured artifacts.

Remaining production-only evidence, without adding new local gates: authorize and capture one controlled scheduled Benchmark report artifact and one Insights report artifact (deployed Insights, browser PDF, and alert/notification parity are already proven); observe a successful natural timer-fired daily run on the corrected revision; and capture fresh provider-confirmed/inbox evidence before enabling or claiming Benchmark email delivery. Until then, machine-readable production status stays `UNVERIFIED`.
...
Blocked on August 3, 2026 only by missing production artifacts; all evidence that could be obtained read-only passed on exact deployed SHA `f636b3c52cabc608566daa0c55432f6e4e5c4eb8`:

- `GET /api/health` returned `200`, `nodeEnv = production`, and exact SHA `f636b3c52cabc608566daa0c55432f6e4e5c4eb8`. `GET /health/scheduler` returned `200`; the GA4 daily scheduler was started, timer-scheduled, idle, and configured for `03:00 UTC`. Natural-run evidence remains Current Commit 14 and was not awaited here.
- The validator used a database `READ ONLY` transaction, authenticated owner-scoped GET requests, browser GET navigation, and local browser PDF downloads. Every non-GET application request was blocked. The final run recorded zero attempted application mutations.
- Complete inventory and scope passed for `2` campaigns and `4` active Benchmarks: campaign, property, owner, campaign filter, reporting timezone, current-value window, authenticated Benchmark IDs, and active report counts matched.
- The configured campaign passed exact Conversions `95` and Revenue `51405.93` parity across live provider, persisted rows, scheduler candidate, UI candidate, card current values, targets, threshold statuses, Tracker totals, and Insights conclusions. The initial false UI/PDF mismatch was a Minor validator timing defect: the validator captured the conversion card while its traffic source was still loading. Waiting for a terminal consumer state and switching tabs without reloading fixed the validator; no runtime value changed.
- The deployed browser Benchmark PDF was downloaded without an application mutation and parsed locally. Both Benchmark names, current values, and target values matched the same rows exactly.
- Alert/notification parity passed. The eligible Revenue Benchmark was not breached and had zero visible alert notifications. The eligible Conversions Benchmark was below its alert threshold and had exactly one visible current notification with matching current value, threshold, condition, campaign-scoped action URL, and Benchmark ID. Both no-property saved-zero rows were ineligible and produced zero false notifications.
- The no-property campaign correctly showed the connection state, did not render scored Benchmark/Insights conclusions, and produced no false breach. Valid-zero, unavailable, stale/last-good, blocked, insufficient-data, and failure semantics remain covered by the exact clean local real-path gate; the deployed inventory did not contain every one of those states, so their absence is not claimed as production occurrence evidence.
- Report inventory exposed `3` active GA4 reports and only `1` Benchmark-consuming report. Its browser PDF passed, but the scheduled report had no sent event and no snapshot. There was no active Insights report artifact. The read-only send-event and snapshot inventories were accessible, but there was no existing artifact to compare.
- The deployed `GET /api/report-snapshots/:snapshotId/pdf` path was deliberately not called: it runs `preflightGA4ReportKPIConsumers`, which persists a GA4 KPI/Benchmark recompute. Calling it would violate the production read-only constraint and could not prove a historical sent artifact.
- The final evidence-tool guard `server/ga4-benchmark-regression.test.ts` passed `14` tests. `npm run check` and `npm run build` both passed after the validator changes.
- Findings for Current Commit 13: Critical `0`; Major `0`; Minor `1`, fixed in validation tooling. Evidence blocker: authorize one controlled production Benchmark report and one Insights report event using approved recipients, then capture the generated PDFs/snapshots and rerun the read-only comparison. Until that authorization and evidence exist, Current Commit 13 stays open/blocked and production certification stays `UNVERIFIED`.
