...
- Legacy route/schema/storage cleanup: started with read-only route reachability inventory. No legacy code has been removed.
...
The next safest smallest step is to continue legacy/stale cleanup with another read-only inventory slice: ownerless/legacy storage behavior for campaign-scoped rows. Do not remove schema/storage paths until reachability and production data dependency are proven.
