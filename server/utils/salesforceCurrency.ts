...
export async function detectSalesforceCurrency(args: DetectSalesforceCurrencyArgs): Promise<DetectSalesforceCurrencyResult> {
  ...
  const authBase = (args.authBase || process.env.SALESFORCE_AUTH_BASE_URL || DEFAULT_AUTH_BASE).replace(/\/+$/, '');
  ...
  try {
    const uiResp = await fetchImpl(`${authBase}/services/oauth2/userinfo`, { headers: { Authorization: `Bearer ${args.accessToken}` } });
