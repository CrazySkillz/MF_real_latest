...
describe("campaign Executive Summary regression guard", () => {
  ...
  it("guards the endpoint and builds current values from the shared aggregate contract", () => {
    ...
    expect(route).toContain("const currentVal = parseNum(bm.currentValue);");
    ...
    expect(route).not.toContain("const currentVal = aggregateMetricValue(aggregateBenchmarkMetric);");
  ...
  it("renders Executive Overview current values from aggregate availability", () => {
    ...
    expect(page).not.toContain("const current = aggregateMetricValue(aggregateKpiMetric);");
