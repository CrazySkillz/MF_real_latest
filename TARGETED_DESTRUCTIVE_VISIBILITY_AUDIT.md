...
- Source delete/edit flows: started. Google Sheets source route access, connection ownership checks, CSV source-type edit guards, GA4 revenue/spend add/edit/refresh source identity, read-only Google Sheets spend duplicate inspection, confirmed Google Sheets spend duplicate cleanup, inactive zero-record hard-delete purge, HubSpot/Salesforce/Shopify revenue save/refresh `sourceId` fail-closed guards, HubSpot/Salesforce/Shopify connection delete membership guards, direct GA4-context revenue source delete campaign/source ownership, stale CRM/ecommerce source scheduler skips, LinkedIn spend edit/refresh `sourceId` preservation, and LinkedIn revenue cleanup HubSpot pipeline context guarding are complete for the targeted pass. Remaining LinkedIn disconnect behavior, Meta, Google Ads, Custom Integration, and remaining CSV/source-upload read/delete paths still need the same route-by-route check.
...
1. Ad-platform source-flow pass.
   Scope: finish LinkedIn disconnect behavior, then Meta and Google Ads spend/revenue source read, edit, delete, disconnect, refresh, and scheduler paths.
   ...
   Fast execution order: LinkedIn remaining cleanup/disconnect boundaries first, then Meta, then Google Ads.
...
The next safest smallest step is item 1: finish LinkedIn disconnect behavior, then move to Meta source-flow routes. Commit after this only if code changes are made and targeted tests pass; otherwise keep the result as a documentation-only audit update.
