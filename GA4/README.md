...
- Certification integrity rule: GA4 readiness is valid only for the exact certified SHA/configuration/dependency boundary. A change to a consumed contract requires impact analysis and proportionate revalidation. For KPIs, changes outside the explicit dated dependency manifest do not automatically invalidate certification; semantic changes to a consumed KPI path do. The Overview record is `GA4/certifications/ga4-overview.json` and its checker is `npm run check:ga4-overview-certification`; the KPI record and checker remain separate. Copied formulas and source-text guards are structural evidence only.
- Historical exact-SHA reconciliation note (2026-09-05): KPI and Overview were certified at deployed runtime `a58ff3a4`. This is revision-specific supporting history and does not replace the current KPI certificate.
- Historical exact-SHA revalidation note (2026-09-06): runtime `bf078b03` added race-safe canonical GA4 KPI create uniqueness. This is revision-specific supporting history and does not replace the current KPI certificate.
...
- Campaign DeepDive `Trend Analysis` production-readiness is tracked in `CAMPAIGN_DEEPDIVE_TREND_ANALYSIS_PRODUCTION_READY.md`. The current page is one comprehensive Executive View. For a GA4-only campaign, current traffic totals are cumulative from the saved initial-import boundary through the latest completed reporting day; current and exact historical financial cards require compatible scheduler-written `financial_daily_snapshot_v1` rows and their financial-card requests do not trigger live GA4 aggregate reads. Campaign Performance Trend charts use only complete scheduler-stored daily rows, including scheduler-written zero rows for completed no-activity dates, and do not call the live GA4 coverage route on page load. The browser selector defaults to 7 days, and its `7/14/30/90-day` options change the exact comparison date and chart window, not current totals. The shared Trend PDF uses a separate fixed 30-day calendar window.
...
- `GA4/REPORTS_PRODUCTION_READINESS.md`
  GA4 Reports readiness narrative, lifecycle matrices, and historical evidence. The exact current decision is controlled by `GA4/REPORTS_CERTIFICATION_2026-09-21.md` at runtime `a7271fc18058b6db78a11e88bf79b887abda5f44`; Campaign DeepDive is outside this boundary.
- `GA4/REPORTS_CERTIFICATION_2026-09-21.md`
  Exact-runtime certificate for the five bounded GA4 Reports areas, including immutable GA4 snapshot artifacts and the authorized natural delivery packet. The protected historical machine record and master ledger remain unchanged.
...
- GA4 Overview `Campaign Breakdown`, `Landing Pages`, and `Conversion Events` use the fixed initial-import boundary through the latest completed day; the initial 30-day import must accumulate subsequent completed days rather than roll forward
- GA4 Overview `Landing Pages` can supplement missing row conversions from conversion-prioritized same-scope `pageLocation` UTM rows only by exact landing-page/source/medium match, including when the base rows came from same-scope `pageLocation` traffic fallback because primary campaign attribution dimensions were empty; it may not allocate campaign-level conversions or imported revenue into unmatched rows, and rows that already have conversions/revenue are not overwritten
- GA4 Overview `Conversion Events` uses exact saved-campaign `sessionCampaignName` rows first and tries exact `firstUserCampaignName` and then `firstUserManualCampaignName` only when the preceding complete result has no positive conversion rows. It never uses `pageLocation`, never merges attribution models, preserves fractional GA4 conversion credit, and displays only rows with `Conversions > 0`
- GA4 Overview `Landing Pages` and `Conversion Events` are live row-level GA4 Data API views for numeric property IDs, not reconstructions from scheduler-populated `ga4_daily_metrics`; zero row-level conversions remain valid for Landing Pages when GA4 returns zero for its exact grain, while zero-conversion event rows are intentionally excluded from Conversion Events
- GA4 Overview financial cards use one selected scoped GA4 financial source for native revenue and CPA conversions in fixed order: initial-import-to-latest-completed-day provider totals, compatible persisted daily totals where available to the caller, then configured-lookback breakdown only when earlier complete candidates are absent. Imported Revenue and Spend use all available mapped records. Campaign metadata dates and app creation time do not define these financial boundaries. Valid zero and negative values remain authoritative; selection is never based on maximum revenue. Browser, scheduled/server output, validation runner, and Benchmark comparison follow the same contract. Commit 13 proved provider-first selection even when persisted-daily revenue was one cent higher; the later exact-boundary reconciliation closed the included downstream and negative-state gates. Future source mixes and excluded fixtures remain outside the certification. `GA4/OVERVIEW_PRODUCTION_READINESS.md` is canonical.
- Native GA4 Revenue precision boundary (2026-09-24): the scoped provider aggregate remains authoritative for headline and exact-date comparison values. Because `ga4_daily_metrics.revenue` is persisted at two decimal places, summing daily rows can differ from an aggregate that retains more underlying precision. The current version does not claim exact cent-for-cent cross-granularity equality and must not allocate a residual to an arbitrary day. Higher-precision daily persistence, exact-source backfill, and reconciliation coverage are deferred to the next version; the bounded Campaign3 finding and acceptance criteria are recorded in `CAMPAIGN_DEEPDIVE_TREND_ANALYSIS_PRODUCTION_READY.md`.
- GA4 Overview `Campaign Breakdown` preserves fixed import-to-date Sessions, Users, Conversions, and native Revenue; its native Revenue query must reconcile to the GA4 Revenue card and must not scale or allocate revenue
...
Performance Summary GA4 validation should use the exact value paths documented in `CAMPAIGN_DEEPDIVE_PERFORMANCE_SUMMARY_PRODUCTION_READY.md`. Key Outcomes consume current cumulative GA4 Summary values and campaign financial totals; Revenue and Spend display in the campaign currency. Recent Movement compares Sessions and Conversions by deriving the cumulative value at the exact prior date from covered GA4 daily facts, Spend from dated active-source records through that exact date with source-set/currency/current-aggregate compatibility, and Total Revenue from same-source native/imported totals through that exact date. Missing, incompatible, ambiguous, stale, or failed historical inputs fail closed rather than reusing the current value as verified history. While a comparison selection is loading, the last settled cards remain visible and the requested period is committed only after the Revenue, Spend, and snapshot requests have settled for the requested boundary.

Performance Summary Campaign Health, Top Priority Action, and Recommended Actions use campaign-scoped GA4 KPI and Benchmark records whose current values are refreshed from the authoritative cumulative traffic and connected-source financial inputs. Health is withheld when any configured metric is unverified. Top Priority ranks below-target KPIs by configured priority and then gap severity, with Benchmark fallback only when no eligible KPI lags. Recommendations identify verified target gaps and are guidance to investigate causes, not causal proof; Revenue and CPA recommendation values and targets use the campaign currency in both the live page and scheduled Performance Summary PDF. When no connected-source metrics or no targets exist, the section shows the corresponding setup state instead of claiming all metrics are on track.

Budget & Financial Analysis GA4 financial behavior is tracked in `CAMPAIGN_DEEPDIVE_BUDGET_FINANCIAL_ANALYSIS_PRODUCTION_READY.md` and `GA4/FINANCIAL_SOURCES.md`. GA4 child revenue/spend inputs can feed aggregate financial totals, but Allocation & Sources and paid-media optimization guidance must preserve the documented source-capability and provenance boundaries. Aggregate Financial Position values come from `/outcome-totals.performanceSummary`; Budget Position, burn, pacing, budget guidance, and their Budget report equivalents additionally require the separate compatible `budget_pacing_v1` Spend derivative. Budget dates never filter GA4 Total Spend or its source modal. Budget & Financial current values refetch while visible and on window focus, and retained comparison plumbing accepts only compatible aggregate snapshots rather than legacy top-level snapshot totals. Current GA4 financial aggregate values should align with GA4 Overview selected scoped native GA4 financial totals plus imported revenue/spend provenance, while top-level date-range GA4 fields can remain windowed.
...
Trend Analysis GA4 behavior is tracked in `CAMPAIGN_DEEPDIVE_TREND_ANALYSIS_PRODUCTION_READY.md`. The visible Executive View combines the aggregate-backed performance chart, efficiency trends, cumulative website engagement/conversion summary, and contextual recommendations. GA4-only current Sessions, Users, Conversions, CVR, and Engagement Rate use the authoritative cumulative persisted GA4 window. Campaign Performance Trend and its GA4 efficiency history accept only a complete scheduler-stored daily window and never call the live GA4 coverage route on page load. Revenue and Spend use the compatible scheduler-written financial snapshot for the latest completed day; ROAS, ROI, and CPA derive from those accepted financial inputs. Exact-date historical financial inputs require the matching scheduler snapshot and fail closed when it is absent. Trend's financial-card refetches never substitute today's value, a daily Revenue sum, or a live GA4 aggregate fallback for a missing current or historical snapshot. The five financial card positions stay visible with an unavailable/scheduler-waiting state until the current snapshot exists.
...
Executive Summary shows the top Risk Level badge but no longer renders the former full Risk Assessment page card. Risk Level uses the current ROI/ROAS, compatible seven-day trajectory, GA4 platform KPI/Benchmark exceptions, applicable freshness state, and paid-platform concentration when a paid source exists. The Campaign DeepDive Executive report PDF separately exports an explicit Risk Assessment breakdown from those same inputs; this does not change the page layout. Budget pacing remains in Budget & Financial Analysis.
...
Campaign DeepDive `Custom Report` implementation history is tracked in `CAMPAIGN_DEEPDIVE_CUSTOM_REPORT_PRODUCTION_READY.md`. The controlling current result is `CAMPAIGN_DEEPDIVE_REPORTS_CERTIFICATION_2026-09-21.md` at deployed revision `809227f7aefba97d50d9c7649de8e7d06c022371`. It certifies the GA4-first Campaign DeepDive builder, all six current direct PDF bodies, saved lifecycle, immutable artifacts, scheduler/delivery safety, and the standalone fail-closed Reports surface without re-certifying protected GA4 calculations. Google Ads, Meta, Instagram, TikTok, and other unconfigured source mixes remain uncertified for this scope.

Campaign DeepDive opens `/reports?campaignId=<campaignId>`. In campaign context the page hides the standalone Standard Reports, Scheduled Reports, and All Reports tab/filter shell and displays only the active campaign's backend scheduled report cards. Unscheduled `Download Report` uses `POST /api/campaigns/:campaignId/custom-report-pdf` and creates no report-library or snapshot row. New campaign-scoped reports hide `Tabs to include`; choosing a report type automatically saves its single-page composition, while legacy saved keys normalize when read/rendered. Performance Summary, Budget & Financial Analysis, Trend Analysis, and Executive Summary each render one current source-aligned report body through the shared server renderer used by direct downloads, snapshots, and scheduled attachments. The Executive report includes the report-specific Risk Assessment breakdown described above even though the main page retains only its top Risk Level. Scheduled create/edit persists recipients, schedule, browser time zone, report type, and composition through `/api/platforms/campaign_deepdive/reports`. Saved-report downloads create an immutable server PDF snapshot and download the exact stored artifact. The current Mailgun production path deduplicates scheduled work by report ID and scheduled key, fails closed for missing campaigns/recipients/artifacts or unconfirmed delivery, and updates snapshots and `lastSentAt` only after confirmed delivery. The current certificate reuses unchanged authorized delivery evidence and records current deployed UI, PDF, artifact, and isolation evidence. The standalone `/reports` route retains its tab shell but does not read/render legacy browser-local report rows or expose report creation; Campaign DeepDive report actions require campaign context.
...
Trend Analysis Executive Recommendations are contextual decision guidance derived from the available trend, efficiency, and conversion signals in the same comprehensive view. The browser shows at most three: an adjacent-window conversion action only when both equal-length daily windows are complete, ROAS guidance only when the financial decision context fully reconciles (otherwise budget guidance is withheld), and cumulative conversion-volume context only when Sessions are positive. They do not use Google Trends keyword widgets, infer causes, or present cumulative growth as like-for-like period performance.
...
The controlling Trend status is the bounded `2026-09-20` GA4-first certification in `CAMPAIGN_DEEPDIVE_TREND_ANALYSIS_CERTIFICATION_2026-09-20.md`: application runtime `7dc72dc8dc5ba302146128b62c45f3bf7d86f6eb`, currently deployed within documentation-only successor `595268463c79b111e77b181ad3003e58f912d208`. It covers the audited `Campaign2` configuration, all `7/14/30/90-day` browser options, the single visible Executive View, exact-date/missing-data behavior, bounded browser/PDF parity, and the explicitly documented negative states and exclusions. The earlier `2026-08-26` commit `cd35bba1c4ff4bb0b045c3bc6c176f2847cd80eb` is historical evidence only. This is Campaign DeepDive evidence; it does not modify or broaden protected GA4 Overview, KPI, Benchmark, Ad Comparison, Insights, or Reports certification records.

Trend report composition exposes one saved section, `trend-analysis:overview` labelled `Executive View`. Legacy saved Trend section keys are normalized to that one section so direct, snapshot, and scheduled PDFs cannot repeat retired tab content. The shared PDF renderer uses a fixed 30-day calendar window ending on the latest completed reporting day, independent of the browser selector. GA4-only report rows fail closed when the saved initial-import boundary does not cover that requested window and are filtered by exact calendar dates; sparse row count is never used as a substitute for a date window. Anomaly Detection remains browser-only PDF-excluded content.
