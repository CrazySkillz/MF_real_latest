...
- the connection screen should always show both `OAuth (recommended)` and `Admin API token` radio options; when server OAuth configuration is incomplete, selecting OAuth must show a clear unavailable message and keep the connect action disabled rather than hiding the method or falling through to the token path
- Shopify OAuth configuration, authorization, and callback persistence require `read_orders` (or implied `write_orders`) but do not require `read_all_orders`. The existing provider-window guard remains separate and must fail closed if any revenue read requests a window older than 60 days without `read_all_orders`; OAuth connection success must not be presented as proof that an older historical window is available.
...
- Current deployed validation records `Admin API token (fallback)` only. Shopify OAuth remains unverified until its real callback, stored connection, and an in-scope order read are observed after deployment; Admin API token certification must not be treated as OAuth validation. The greater-than-250-order pagination and bounded-rate-limit branches are regression-proven and fail closed, while live provider conditions absent from the current store remain operational evidence boundaries rather than blockers to the enabled-scope certification.
...
- The historical portability packet for a second Shopify Admin API token campaign remains bounded evidence only. It does not close current certification; OAuth, real >250-order pagination, all-production-campaign health, provider mutation convergence, deployed scheduler/failure behavior, and complete downstream/alert evidence remain open in the canonical Shopify readiness document.

- The authenticated `POST /api/campaigns/:id/revenue-sources/:sourceId/shopify-refresh/run-now` validation route runs the same saved-mapping reprocess function for one exact active GA4 Shopify source. It has no user-facing UI action and does not run the full daily cycle, other sources, other campaigns, emails, or reports; successful deployed response and refreshed-value evidence are still required, and it does not prove natural timer firing.
...
- HubSpot revenue edit mode should open at the first `Source` step with the saved settings prefilled; it must not jump directly to `Review Settings`
...
- active GA4 HubSpot sources with saved selected values are automatically reprocessed every five minutes by default through the saved mapping and stable revenue source ID; a saved stage is required only when Pipeline Proxy is enabled, while revenue-only sources refresh confirmed revenue without a Pipeline card; the shared CRM interval remains configurable from one to sixty minutes with `SALESFORCE_PIPELINE_REFRESH_INTERVAL_MINUTES`, and overlapping financial refreshes are skipped
- when a mapped HubSpot deal leaves the selected open stage for a current Closed Won stage, that atomic refresh removes the deal amount from Pipeline Proxy, materializes it once as confirmed revenue, and records the mapped deal/value in the Revenue Sources breakdown without a wizard resave
- an open GA4 Overview checks the saved HubSpot Pipeline result every minute and refetches Total Revenue, Revenue Sources, and Revenue Breakdown when the provider refresh timestamp changes
...
- in the GA4 Overview `Revenue Sources` modal, `HubSpot (Deals)` has no redundant provider subtitle; saved platform campaign mappings remain attribution metadata and are not repeated there
- HubSpot confirmed values should be itemized directly beneath the provider subtotal without a redundant `Confirmed deals` count heading; each value row keeps its own exact remove control in the same name, amount, and action columns as Salesforce
- removing one HubSpot value must preserve the stable HubSpot source and every other selected value, then atomically rematerialize confirmed Total Revenue and Pipeline Proxy; removing the final value removes that exact source
...
- HubSpot-backed GA4 KPI/Benchmark financial values use the same source-backed financial total as Overview (`selected GA4 native financial revenue + imported revenue`, Pipeline Proxy excluded). The current implementation is unverified only until the final local OAuth expiry-persistence correction is deployed and its renewed expiry is observed; current bounded lifecycle, scheduler, integrity, and downstream evidence is recorded in `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md`.
- The authenticated `POST /api/campaigns/:id/revenue-sources/:sourceId/hubspot-refresh/run-now` validation route runs the same saved-mapping reprocess function for one exact active GA4 HubSpot source. It has no user-facing UI action and does not run the full daily cycle, other sources, other campaigns, emails, or reports. Current natural five-minute firing is separately proven only for the five exact sources recorded in the canonical HubSpot readiness document.
...
- when the source chooser already has an active same-context Salesforce source, its card must be non-actionable and direct users to the provider-level edit control in Revenue Sources; only that exact-source edit flow may update the saved opportunity selection. The chooser must also wait until active sources are resolved before allowing first-time Salesforce setup so a failed or pending lookup cannot enter additive connect mode.
...
- the GA4 Revenue Sources modal should itemize confirmed Salesforce values from `campaignValueRevenueTotals` directly beneath the Salesforce provider subtotal without repeating a `Salesforce` subtitle or a `Confirmed opportunities`/`Confirmed attributed values` group heading. Pipeline Proxy values remain excluded.
...
- in the GA4 Revenue Sources modal, the Shopify edit control and provider subtotal belong on the same header line as the Shopify provider label; the mapped campaign label, repeated breakdown amount, and source remove control must use the same text sizing and name, amount, and action columns as itemized Salesforce and HubSpot entries
...
- `Google Sheets` is the mapped campaign-scoped import workflow with automatic refresh of the same saved source
...
- the deployed Google Ads/Upload CSV chooser evidence after whole-Overview Current Commit 5 is historical; Current Commit 21 restores Google Sheets and requires a new visibility check
- existing LinkedIn and Meta source edit/delete paths remain continuity-only; this policy performs no persisted-data mutation
...
- downstream KPI, benchmark, ad comparison, insights, alert/notification, and report values must use the recomputed financial state; revenue-availability gates and GA4 financial alert visibility must follow the same selected GA4 financial source as `Total Revenue`, not a separate Summary-card, imported-only, or persisted-row revenue fallback
