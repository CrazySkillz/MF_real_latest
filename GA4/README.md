...
- GA4 Overview `Landing Pages` can supplement missing row conversions from same-scope `pageLocation` UTM rows only by exact landing-page/source/medium match; `Conversion Events` can supplement missing row conversions from same-scope `pageLocation` UTM rows only by exact event-name match; neither table may allocate campaign-level conversions or imported revenue into unmatched rows
