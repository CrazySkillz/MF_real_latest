...
Current Overview status: **UNVERIFIED**. Current Commit 20 corrects the escaped use of the 30-day setup value as a rolling Summary window. The supported contract is a fixed initial 30-completed-day import followed by completed-day accumulation. Deployment/API/UI/report parity, a timer-fired scheduled-run no-drop check, and OAuth durability on or after `2026-08-07` remain required. The concise decision is in `GA4/OVERVIEW_PRODUCTION_READINESS.md`; evidence is in its evidence ledger; chronological Current Commits 0-20 are in its history ledger.
