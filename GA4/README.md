...
- Shopify-specific GA4 Overview source-family certification is tracked in `GA4/OVERVIEW_PRODUCTION_READINESS.md` and `GA4/FINANCIAL_SOURCES.md`. Current Commit 6 deployed evidence is closed only for the validated Admin API token Shopify edit/re-import cache/refetch packet: revenue, KPI, Benchmark, Reports, and Notifications fetches were observed after the source action and endpoints passed. Current Commit 6a is locally regression-covered and deployed-closed only for the validated Admin API token campaign/report/email packet: report snapshot/PDF content inspection, KPI/Benchmark row-value parity, notification row value content, and actual delivered report email. Current Commit 8 is locally regression-covered only for portability/provider-boundary route guards. OAuth, deployed second-campaign/mapping evidence, real >250-order provider evidence, future provider changes, future report/email sends, other report variants, and provider-boundary variants remain unproven.
...
- HubSpot rows in the GA4 Overview `Revenue Sources` modal should show the saved mapped platform-campaign name under `HubSpot (Deals)` when `campaignMappings` exist, falling back to `HubSpot` when no mapping is saved
