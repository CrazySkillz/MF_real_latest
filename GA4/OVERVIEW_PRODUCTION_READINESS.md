...
| Current Commit 2b revenue source delete false-failure guard | `npm test -- server/ga4-source-lifecycle-recompute-regression.test.ts`; `npm run check`. | GA4 revenue source mutation routes keep source deletion/update response semantics stable even if downstream GA4 KPI/Benchmark recompute fails after the source mutation. | Does not close Google Sheets revenue delete/deactivate until the fixed route is deployed and a delete action has endpoint response evidence plus after-delete endpoint/UI parity. |
...
| Current Commit 2b: Revenue source delete false-failure response guard | Implemented locally; deployed validation pending. | Google Sheets revenue delete/deactivate false-failure observed during Current Commit 2. | Guard GA4 KPI/Benchmark recompute inside `recomputeCampaignDerivedValues` so completed source mutations do not return false failure responses if downstream recompute fails; keep recompute logged and continue the source endpoint response. | No for local code-scope certification; yes for the Google Sheets revenue delete/deactivate action until fixed-route deployed response evidence and after-delete endpoint/UI parity are captured. |
...
1. Next source family/action: deploy Current Commit 2b, then validate Google Sheets revenue delete/deactivate without treating the prior `Delete failed` response as a pass.
2. If source `dd5dc470-814d-42b9-af19-4b53ac7d08f8` is already gone, capture read-only after-delete endpoint evidence proving the target source is absent, the `$600` CSV revenue source remains, spend remains `$0`, GA4 native revenue remains available, and the Revenue Sources modal no longer shows the Google Sheets row.
3. To prove the corrected deployed response path, create one additional disposable Google Sheets revenue source with a known small amount, capture a baseline snapshot, delete only that disposable source through the deployed Overview Revenue Sources modal after Current Commit 2b is deployed, and capture the DELETE response plus after snapshot.
4. Confirm visible Overview UI parity: Total Revenue, Revenue Sources modal/list, and any spend-dependent derived-card behavior must match the traced endpoint totals.
5. Record the evidence in this file before moving to any Google Sheets spend lifecycle action.
...
- Google Sheets revenue delete/deactivate remains unproven until Current Commit 2b is deployed and fixed-response endpoint evidence plus after-delete endpoint/UI parity are captured; Google Sheets spend lifecycle actions remain unproven until each relevant add/import, edit/update, refresh/reprocess, and delete/deactivate action has deployed before/after endpoint evidence plus UI parity.
