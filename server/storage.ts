...
export class MemStorage {
  ...
  async getSpendBreakdownBySource(campaignId: string, startDate: string, endDate: string): Promise<Array<{ sourceId: string; displayName: string; sourceType: string; spend: number; currency?: string }>> {
    ...
    for (const rec of Array.from(this.spendRecords.values())) {
      ...
      const existing = totals.get(srcId) || { displayName: (src as any).displayName || (src as any).sourceType || 'Unknown', sourceType: (src as any).sourceType || 'unknown', spend: 0, currency: (rec as any).currency };
      existing.spend += v;
      totals.set(srcId, existing);
    }
  ...
  async getRevenueBreakdownBySource(campaignId: string, startDate: string, endDate: string, platformContext: 'ga4' | 'linkedin' | 'meta' = 'ga4'): Promise<Array<{ sourceId: string; displayName: string; sourceType: string; revenue: number; currency?: string }>> {
    ...
    for (const rec of Array.from(this.revenueRecords.values())) {
      ...
      const v = parseFloat(String((rec as any).revenue ?? "0"));
      if (Number.isNaN(v)) continue;
      const existing = totals.get(srcId) || { displayName: (src as any).displayName || (src as any).sourceType || 'Unknown', sourceType: (src as any).sourceType || 'unknown', revenue: 0, currency: (rec as any).currency };
      existing.revenue += v;
      totals.set(srcId, existing);
    }
  ...
  async createGoogleSheetsConnection(connection: InsertGoogleSheetsConnection): Promise<GoogleSheetsConnection> {
    ...
    const sheetsConnection: GoogleSheetsConnection = {
      ...
      isPrimary: isPrimary,
      isActive: true,
      columnMappings: connection.columnMappings || null,
      platforms: connection.platforms || null,
      cachedData: null,
  ...
  async createLinkedInConnection(connection: InsertLinkedInConnection): Promise<LinkedInConnection> {
    ...
    const linkedinConnection: LinkedInConnection = {
      ...
      method: connection.method,
      encryptedTokens: null,
      conversionValue: connection.conversionValue || null,
      campaignUtmMap: null,
  ...
  async createMetaDailyMetric(metric: InsertMetaDailyMetric): Promise<MetaDailyMetric> {
    ...
    const newMetric: MetaDailyMetric = {
      ...
      spend: String(metric.spend ?? "0"),
      conversions: metric.conversions ?? 0,
      videoViews: metric.videoViews ?? 0,
      postEngagement: metric.postEngagement ?? 0,
      linkClicks: metric.linkClicks ?? 0,
      ctr: metric.ctr != null ? String(metric.ctr) : null,
      cpc: metric.cpc != null ? String(metric.cpc) : null,
      cpm: metric.cpm != null ? String(metric.cpm) : null,
      cpp: metric.cpp != null ? String(metric.cpp) : null,
      frequency: metric.frequency != null ? String(metric.frequency) : null,
      costPerConversion: metric.costPerConversion != null ? String(metric.costPerConversion) : null,
      conversionRate: metric.conversionRate != null ? String(metric.conversionRate) : null,
      revenue: metric.revenue != null ? String(metric.revenue) : null,
      roas: metric.roas != null ? String(metric.roas) : null,
  ...
  async createCustomIntegrationMetrics(metricsData: InsertCustomIntegrationMetrics): Promise<CustomIntegrationMetrics> {
    ...
    const metrics: CustomIntegrationMetrics = {
      ...
      avgSessionDuration: metricsData.avgSessionDuration || null,
      pagesPerSession: metricsData.pagesPerSession || "0",
      bounceRate: metricsData.bounceRate || "0",
      organicSearchShare: metricsData.organicSearchShare || "0",
      directBrandedShare: metricsData.directBrandedShare || "0",
      emailShare: metricsData.emailShare || "0",
      referralShare: metricsData.referralShare || "0",
      paidShare: metricsData.paidShare || "0",
      socialShare: metricsData.socialShare || "0",
      emailsDelivered: metricsData.emailsDelivered || 0,
      openRate: metricsData.openRate || "0",
      clickThroughRate: metricsData.clickThroughRate || "0",
      clickToOpenRate: metricsData.clickToOpenRate || "0",
      hardBounces: metricsData.hardBounces || "0",
      spamComplaints: metricsData.spamComplaints || "0",
      listGrowth: metricsData.listGrowth || 0,
      pdfFileName: metricsData.pdfFileName || null,
      emailSubject: metricsData.emailSubject || null,
      emailId: metricsData.emailId || null,
      previousMetrics: metricsData.previousMetrics || null,
      previousUpdateAt: metricsData.previousUpdateAt || null,
      uploadedAt: new Date(),
    };
