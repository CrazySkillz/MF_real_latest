...
**CLEAN-CERTIFIED / PRODUCTION_READY for the Executive Financials cards and their “Sources used” provenance at exact Render revision `f4fe2f3e3b47a3b350dd14c8a8d336bc8ff946ca`, within the campaign, property, sources, and value states below.** All required gates for this bounded consumer result passed. The cards use the shared `financialRevenue`, `financialSpend`, `financialROAS`, and `financialROI` values also rendered by Overview Revenue & Financials. Profit uses those same Revenue and Spend values. Executive Financials withholds Spend and dependent values when the shared Spend input is unavailable or non-finite, or when Overview has no Spend display sources; missing individual contributor names are disclosed in “Sources used.” Different reporting cutoffs are disclosed when all three dates are present; ratios across them are not described as same-period comparisons.
