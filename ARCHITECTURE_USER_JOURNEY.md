...
- Executive Summary Benchmark Comparison should follow the same source-of-truth pattern. Campaign-level Benchmark records define the rows and benchmark targets, while `Yours` current values must come from available connected-source aggregate metrics. Unmapped or unavailable benchmark current values should be excluded from Executive Summary Benchmark Comparison instead of silently using saved Benchmark `currentValue` snapshots.
