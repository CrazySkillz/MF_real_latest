...
- HubSpot imported revenue must enter GA4 financial Benchmark values only through the same Overview financial total (`GA4 native financial revenue + imported revenue`, Pipeline Proxy excluded); HubSpot Revenue is UNVERIFIED on the current revision, with `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md` as the controlling status record
