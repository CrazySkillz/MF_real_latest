    ...
    try {
      ...
      const onMessage = (event: MessageEvent) => {
        ...
        if (data.type === "salesforce_auth_success") {
          ...
          // Open the dedicated Salesforce viewer (separate screen) after connection.
          (async () => {
            try {
              ...
              if (onDataConnected) onDataConnected();
              if (onOpenSalesforceViewer) onOpenSalesforceViewer({ sourceId: sfId });
              setTimeout(() => onClose(), 150);
            } catch (e: any) {
              toast({
                title: "Connected, but viewer failed to open",
