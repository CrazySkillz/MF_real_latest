...
| What to investigate next | stable page header; grouped finding title, description, recommended check, severity, basis, confidence, hidden count | financial integrity rules; eligible KPI/Benchmark values and snapshot analytics; fail-closed saved-target/current-window period matching with affected target identification; saved KPI priority and factual below-target status; consolidated unverified-target readiness finding; factual no-snapshot state; complete 3-day or 7-day calendar comparisons; raw same-window channel context; explicit unavailable/stale/configuration findings; no generic total-ROAS strength inference |
