...
- corresponding GA4 KPI and Benchmark `currentValue` fields are recomputed from those cumulative traffic inputs and the connected-source financial contract: native GA4 financial values use the same fixed initial-import boundary, while imported Revenue and Spend use all available mapped records
...
- Performance Summary currency values must use the campaign currency. This includes Key Outcomes Revenue/Spend, Recent Movement financial values, and Recommended Actions Revenue/CPA current and target values; the scheduled Performance Summary PDF receives the same campaign currency.
...
Trend Analysis renders one comprehensive executive view instead of separate tabs. The view combines the Connected-Source Performance Summary, Campaign Performance Trend, Efficiency Trends, conditional Anomaly Detection, Website Engagement & Conversion Summary, conditional paid/source detail, and evidence-gated Executive Recommendations without duplicating the same values across panels. Its browser selector defaults to `7 days`; the `7/14/30/90-day` options control the chart window and exact cumulative comparison date, but do not replace authoritative cumulative current totals with rolling-window totals. For the GA4-first consumer, current and exact historical financial cards read compatible scheduler-written `financial_daily_snapshot_v1` rows; their current/comparison requests do not perform live GA4 aggregate fallbacks, and missing snapshots fail closed. The Campaign Performance Trend chart likewise reads only complete scheduler-stored `/ga4-daily` rows, including explicit scheduler-written zero rows for completed no-activity dates; opening the page does not run a live GA4 coverage request. The five financial card positions remain visible with an unavailable/scheduler-waiting state so snapshot availability does not change the card count. The shared Trend PDF uses its separate fixed 30-day calendar contract.
...
Budget & Financial Analysis is a downstream consumer of platform-level connected-source financial records. Platform `Total Revenue` and `Total Spend` remain the source of truth and are not changed by budget controls. Budget Position, utilization, burn rate, pacing status, budget guidance, and their Budget report equivalents use a separate verified Spend derivative containing only dated Spend records inside `pacingStartDate` through `pacingEndDate`, capped at the connected financial data-through date. If that derivative cannot verify the complete active Spend source set, those budget outputs fail closed as unavailable. This period filter must never alter source records, aggregate Total Spend, ROI, ROAS, profit, CPA, or source breakdowns.
...
- The same executive opens Executive Summary to get one campaign-wide page containing factual narrative, 7-day trajectory, Risk Level, funnel, key metrics, KPI/Benchmark exceptions, and Recommended Actions. The former Executive Overview and Strategic Recommendations tabs no longer exist.
- Executive Summary shows GA4-only campaigns as web analytics/outcome campaigns. The funnel uses Users -> Sessions -> Conversions -> Revenue when paid-media reach/click inputs are unavailable; unavailable metrics remain unavailable rather than becoming zero.
- Funnel and five key metric cards read `performance_summary_aggregate_v3` from `/outcome-totals`. For the GA4-first boundary, Users, Sessions, Conversions, and Engagement Rate are property-level GA4 campaign-filtered values; Total Revenue combines GA4-native campaign revenue with active imported connected revenue; Spend uses active GA4-context spend records.
...
- The factual narrative uses the same `performanceSummary` ROI/ROAS values as the visible cards, plus the current Risk Level and trajectory state. It must not use hidden score/grade wording.
...
- For GA4-connected campaigns, KPI and Benchmark rows/current values come from `google_analytics` platform records refreshed from the configured GA4 property reporting contracts. These configured records may use a different documented reporting window from the current aggregate cards and must not be silently substituted for one another.
- Recommended Actions is part of the same page. The visible GA4-first action appears only when property-level website/outcome evidence exists and CVR, Conversions, or Revenue has a KPI/Benchmark exception. It gives an investigation step and does not make a paid-media budget claim; insufficient evidence produces `No Evidence-Backed Actions Available`.
- The page refetches `/executive-summary` and `/outcome-totals` on mount, window focus, and every 60 seconds while active. It fetches the trajectory for the authoritative completed reporting date and waits for the applicable current requests before rendering the completed page.
- Executive Summary daily trajectory snapshots use `executive_summary_daily_snapshot_v2`, one row per campaign/reporting date. They store the exact contract plus stable persisted `revenue_source:<id>` and `spend_source:<id>` identities so same-label sources remain distinct and changed source sets fail closed as incompatible history.
- A snapshot is not written until persisted GA4 property rows have refreshed after the completed reporting day closed. Trajectory compares compatible revenue readings exactly seven days apart: more than +10% is accelerating, less than -10% is declining, and otherwise stable; missing or incompatible history remains unavailable.
- Future Connected Platform integrations must feed Executive Summary through the shared aggregate/source-capability contract and prove their current-value, scheduler snapshot, KPI/Benchmark, risk, recommendation, and deployed source-mix paths before their values are considered production-ready here.
- Current implementation, exact production evidence, exclusions, and path classifications are authoritative in `CAMPAIGN_DEEPDIVE_EXECUTIVE_SUMMARY_PRODUCTION_READY.md` under `Current Authoritative Implementation And Certification — 2026-08-27`.
