...
The historical second-campaign Admin API token packet remains bounded portability evidence: Shopify revenue appeared in GA4 Overview, Total Revenue included it, Revenue Sources and Revenue Breakdown matched, source IDs did not overlap, and unrelated spend/revenue stayed unchanged. It is supporting evidence rather than the certification basis. `GA4/OVERVIEW_REVENUE_SHOPIFY_PRODUCTION_READINESS.md` is canonical and records the 2026-07-15 production-ready and clean-certified conclusion for the currently enabled GA4 Overview Admin API token scope, including the completed owner-scoped inventory, cleanup, scheduler/alert, and downstream boundaries. Dormant OAuth remains excluded.
