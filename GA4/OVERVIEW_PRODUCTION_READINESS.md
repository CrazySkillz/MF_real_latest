...
9. Current Commit 4.7b: deployed HubSpot provenance capture. Re-run a HubSpot Total Revenue-only add/import or edit/update on the target campaign, then run `GA4OverviewValidation.hubspotProvenance({ campaignId, expectedPipelineEnabled: false })` before deleting the source. Evidence must record `checkedAt`, `account`, `accountPresent`, active source ID, mapping `campaignProperty`, `selectedValues`, `revenueProperty`, `dateField`, `pipelineEnabled: false`, record count, source revenue, source-modal expected label/value data, findings, and pass/fail. Do not include tokens or secrets.
10. Current Commit 4.7c: HubSpot refresh/reprocess applicability trace. Trace the deployed UI, client handlers, API routes, and scheduler dependency for HubSpot refresh/reprocess. If no user-facing refresh/reprocess action exists for GA4 HubSpot revenue, document it as not applicable with route evidence. If a route/action exists, validate it with before/after endpoint, inventory, and provenance evidence. Do not infer scheduler safety from add/edit/delete.
11. Current Commit 4.7d: HubSpot clean-certification update. Only after 4.7b and 4.7c pass, update this file with the validated provider/provenance/refresh evidence and mark HubSpot clean-certified for the exact proven campaign/provider scope. Do not use Google Sheets, CSV, KPI, Benchmark, Reports, or other source-family evidence as proof.
...
| HubSpot revenue or Pipeline Proxy | Confirmed HubSpot Total Revenue path only for the tested `Total Revenue only (no Pipeline card)` flow; Pipeline Proxy remains early-signal only and was not selected or certified | Endpoint/data-health evidence captured for add/import on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`: active HubSpot source `1`, records `2`, revenue `$10,600`, no findings | Endpoint/data-health evidence captured for edit/update: active HubSpot source stayed `1`, records `4`, revenue `$16,600`, no findings | No deployed UI refresh/sync/reprocess button found in the tested flow; not separately proven and not yet traced as not applicable | Endpoint/data-health evidence captured for delete/deactivate: active HubSpot source `0`, records `0`, revenue returned to `$600`, no findings | Partially validated for this campaign's confirmed Total Revenue add/import, edit/update, and delete/deactivate endpoint/data-health behavior only. Current Commit 4.7a adds local read-only provenance automation, but deployed provenance capture remains pending. Not clean-certified until HubSpot account, selected fields/date field/campaign values, source-modal provenance, and refresh/reprocess applicability are recorded. Pipeline Proxy, other campaigns, and other HubSpot configurations remain unproven. |
...
- Shopify, Salesforce, legacy Manual revenue, LinkedIn Ads, Meta Ads, Google Ads, and legacy Manual spend remain provider/user validation work until each relevant family has before/after evidence. HubSpot has partial deployed Total Revenue add/edit/delete endpoint and data-health evidence for campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`, and Current Commit 4.7a adds local read-only provenance automation, but HubSpot remains not clean-certified until the deployed provenance and refresh/reprocess gaps above are closed.
