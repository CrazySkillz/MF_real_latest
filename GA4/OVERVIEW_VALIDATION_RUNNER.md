...
Recorded `2026-08-01` final-pack evidence for campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`: the endpoint returned `success=true`, `readonly=true`, zero generic damage findings, and passing CSV/HubSpot/Shopify-local checks. The user matched the four visible retained Spend sources totaling $2,698.75 to the deployed modal and explicitly approved keeping all four. `retainedSourceInventoryPass=false` therefore records their presence, not damage. A later post-publish Google Sheets reconnect and user-confirmed no-click mapped-value update closed the stale connection blocker for the exact Overview boundary. `shopifyInventoryScopeComplete=false` preserves broader campaign-local provider/history/cross-campaign exclusions governed by the separate canonical Shopify readiness record. The final non-scheduler pack passed; no cleanup was authorized or performed. The later `2026-08-10` provider-usable connection and exact timer-fired target refresh closed the bounded OAuth durability and scheduled-run gates.
...
Recorded deployed Current Commit 10 UI evidence on `2026-07-12`: the user confirmed the normal fixture lifecycle passed. Adding `Alpha` showed `$150`; editing the same source to `Alpha + Beta` showed `$1,250` without a duplicate; editing it back to `Alpha` showed `$150` without a duplicate; deleting it removed the source and restored the original Total Revenue; Total Spend remained unchanged throughout. The user subsequently confirmed the invalid CSV test showed an error, created no source, and left Total Revenue and Total Spend unchanged. No runner output, campaign/property/source IDs, endpoint-parity packet, or exact Profit/ROAS/ROI/CPA values were supplied; those are not claimed as deployed evidence.

Current Commit 12 certification rerun outcome on `2026-07-12`: the CSV-only bounded local packet passed 44/44, the runner syntax check passed, and TypeScript passed. A refreshed read-only target-database scan covered 4 campaigns, 23 GA4 CSV sources, and 21 linked records; it found only the same five known inactive-source record groups, with no active/reconciliation damage and no cleanup performed. After the user confirmed the deployed invalid-file no-mutation result, Upload CSV Revenue is clean-certified for the validated documented scope. The runner remains optional evidence collection tooling, not an additional UI action required for that bounded claim. Current Commit 21 later re-enabled Google Sheets Revenue; its separate certification remains open.
...
The Revenue Sources modal lists sources contributing to confirmed Total Revenue. A HubSpot source with `$0.00` confirmed revenue is therefore omitted while its configured early-signal amount remains available through Pipeline Proxy; the source must appear automatically once confirmed revenue is materialized.
...
Current HubSpot Revenue Sources and edit-review display guards live in `server/hubspot-revenue-ga4-overview-regression.test.ts`. The source row does not repeat provider or mapped-campaign subtitles, zero-confirmed-revenue HubSpot sources are omitted from the contributing-source list, confirmed values are shown without a redundant count heading, and an unchanged zero Pipeline Proxy amount is suppressed in edit review. Deployed UI validation passed for this bounded presentation; the canonical HubSpot readiness document controls certification status.
...
Historical 4.13 mismatch before the persisted-job fix: the earlier `actualCurrentValue: 35317.57` versus `expectedCurrentValue: 44864.15` packet exposed incompatible native-source selection. That historical diagnosis must not be treated as the current selection rule. Current Overview, persisted KPI/Benchmark jobs, report validation, portability validation, and the Benchmark provider-validation comparison use the first complete provider → persisted-daily → breakdown candidate, with valid zero/negative authoritative and Pipeline Proxy excluded.

Whole-Overview Current Commit 13 deployed evidence on `2026-07-30`: commit `d353383e` served runner `2026-07-30.10`. On existing campaign `GA4 single` / `ga4_mock`, the authenticated read-only Benchmark comparison returned `success: true`, `provider: live_provider_success`, provider revenue `$34,705.93`, persisted-daily revenue `$34,705.94`, and selected revenue `$34,705.93`. The lower provider value winning proves that the deployed route follows ordered provider-first selection rather than maximum revenue. This closes only Commit 13's validation-path parity packet; it does not prove missing-source, valid-zero/negative production fixtures, other campaigns, or complete Overview readiness.
...
The historical second-campaign Admin API token packet remains bounded portability evidence: Shopify revenue appeared in GA4 Overview, Total Revenue included it, Revenue Sources and Revenue Breakdown matched, source IDs did not overlap, and unrelated spend/revenue stayed unchanged. A later same-UTC-day order contradicted the untested default window path, so `GA4/OVERVIEW_REVENUE_SHOPIFY_PRODUCTION_READINESS.md` now records current readiness as unverified pending deployed parity after the local correction. Dormant OAuth remains excluded.

The previous lifecycle, startup scheduler, downstream content, delivered-email, and mapped-campaign packets remain historical leads only. Future readiness answers must cite the canonical Shopify readiness document and its final certification gate, not restate any one historical packet as independent certification.
