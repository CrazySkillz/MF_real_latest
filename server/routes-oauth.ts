...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  /**
   * TEST-ONLY: Seed Salesforce Opportunities using the stored Salesforce connection for a campaign.
   *
   * Guarded by env vars:
   * - ALLOW_SALESFORCE_SEED=true
   * - SALESFORCE_SEED_KEY=<shared_secret>
   *
   * Header required:
   * - x-mm-seed-key: <shared_secret>
   */
  app.post("/api/salesforce/:campaignId/opportunities/seed", async (req, res) => {
    try {
      const allow = String(process.env.ALLOW_SALESFORCE_SEED || "").trim().toLowerCase() === "true";
      const seedKey = String(process.env.SALESFORCE_SEED_KEY || "").trim();
      const headerKey = String(req.header("x-mm-seed-key") || "").trim();
      if (!allow || !seedKey || headerKey !== seedKey) {
        return res.status(403).json({ error: "Salesforce seeding is disabled." });
      }
      ...
      const {
        ...
        amountMax,
        closeDays,
        campaignFieldApiName,
        campaignFieldValues,
      } = req.body || {};

      const n = Math.min(Math.max(parseInt(String(count ?? "25"), 10) || 25, 1), 500);
      const acctId = String(accountId || "").trim();
      const acctName = String(accountName || "MetricMind Test Account").trim();
      const oppPrefix = String(namePrefix || "MetricMind Test Opportunity").trim();
      const stage = String(stageName || "Closed Won").trim();
      const minAmt = Number.isFinite(Number(amountMin)) ? Number(amountMin) : 100;
      const maxAmt = Number.isFinite(Number(amountMax)) ? Number(amountMax) : 5000;
      const closeInDays = Math.min(Math.max(parseInt(String(closeDays ?? "14"), 10) || 14, 0), 3650);

      const fieldApi = String(campaignFieldApiName || "").trim();
      const fieldVals: string[] = Array.isArray(campaignFieldValues)
        ? campaignFieldValues.map((v: any) => String(v).trim()).filter(Boolean)
        : String(campaignFieldValues || "")
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean);
      ...
      const sfFetch = async (path: string, init?: RequestInit) => {
        const url = `${instanceUrl}/services/data/${version}${path.startsWith("/") ? "" : "/"}${path}`;
        ...
        const json: any = await resp.json().catch(() => ({}));
        if (!resp.ok) {
          const msg = (Array.isArray(json) ? json?.[0]?.message : null) || json?.message || json?.error || "Salesforce API error";
          throw new Error(String(msg));
        }
        return json;
      };

      // Optional: validate custom campaign field exists on Opportunity (better error UX than silent failures)
      if (fieldApi) {
        const desc = await sfFetch("/sobjects/Opportunity/describe");
        const fields = Array.isArray(desc?.fields) ? desc.fields : [];
        const ok = fields.some((f: any) => String(f?.name || "") === fieldApi);
        if (!ok) {
          return res.status(400).json({
            error: `Opportunity field "${fieldApi}" not found. Ensure the API name is correct (custom fields usually end with __c).`,
          });
        }
      }

      // Ensure Account
      let resolvedAccountId = acctId;
      if (!resolvedAccountId) {
        const created = await sfFetch("/sobjects/Account", { method: "POST", body: JSON.stringify({ Name: acctName }) });
        resolvedAccountId = String(created?.id || "").trim();
        if (!resolvedAccountId) throw new Error("Failed to create Account");
      }

      // Create Opportunities
      const closeDate = (() => {
        const d = new Date();
        d.setDate(d.getDate() + closeInDays);
      ...
      for (let i = 0; i < n; i++) {
        const amt = minAmt + Math.random() * Math.max(0, maxAmt - minAmt);
        const amount = Math.round(amt * 100) / 100;
        const payload: any = {
          Name: `${oppPrefix} #${i + 1} (${Date.now()})`,
          AccountId: resolvedAccountId,
          StageName: stage,
          CloseDate: closeDate,
          Amount: amount,
        };
        if (fieldApi && fieldVals.length > 0) {
          payload[fieldApi] = fieldVals[i % fieldVals.length];
        }
        const created = await sfFetch("/sobjects/Opportunity", { method: "POST", body: JSON.stringify(payload) });
      ...
      res.json({
        ...
        opportunityIds: createdIds.slice(0, 50),
      });
    } catch (error: any) {
      console.error("[Salesforce Seed] Error:", error);
      res.status(500).json({ error: error.message || "Failed to seed Salesforce Opportunities" });
    }
  });
