...
The historical second-campaign Admin API token packet remains bounded portability evidence: Shopify revenue appeared in GA4 Overview, Total Revenue included it, Revenue Sources and Revenue Breakdown matched, source IDs did not overlap, and unrelated spend/revenue stayed unchanged. It does not close current Shopify certification. `GA4/OVERVIEW_REVENUE_SHOPIFY_PRODUCTION_READINESS.md` is canonical and records the unresolved OAuth, real-provider, all-campaign inventory, scheduler, alert, and downstream boundaries.

The previous lifecycle, startup scheduler, downstream content, delivered-email, and mapped-campaign packets remain historical leads only and must not be restated as a current Shopify clean certification.
