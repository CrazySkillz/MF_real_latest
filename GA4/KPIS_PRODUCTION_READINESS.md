...
3. **Timer scheduler Gate 3 — pending until the already-configured natural run.** Deployed health at `2026-08-02T10:23:06Z` showed `runOnStartup=false`, `totalRuns=0`, `lastRunStatus=idle`, and next run `2026-08-03T03:00:00Z`. No manual trigger was used. The scheduler logs exact processed/skipped/failed campaign and KPI IDs. The smallest observability closure now also retains SHA-256-prefixed ID arrays in public scheduler health without exposing raw IDs or changing scheduler behavior; 34/34 focused scheduler/auto-refresh/certification tests, the checker, and TypeScript pass locally. The field still requires deployment and cannot prove a timer run before the configured time.
