...
- HubSpot Revenue status: **UNVERIFIED for the current implementation** only until the final local OAuth expiry-persistence correction is deployed and its renewed expiry is observed. Runtime `ce3eaeaf` passed the bounded provider transition, Pipeline-enabled and revenue-only five-minute refresh, open-Overview update, item/source deletion, edit, expired-token provider use, and current source integrity checks. The process-wide daily run still failed unrelated obsolete/test/provider jobs, so no global scheduler-health claim is made. Use `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md` for the exact evidence and exclusions.
...
- HubSpot-specific readiness is canonical in `GA4/OVERVIEW_REVENUE_HUBSPOT_PRODUCTION_READINESS.md`; current deployed lifecycle and scheduler evidence is complete, while the final local OAuth expiry-persistence correction remains deployment-unverified. Use `GA4/CRM_REVENUE_SOURCE_PATTERN.md` for the Salesforce reference and preserved CRM contracts.
