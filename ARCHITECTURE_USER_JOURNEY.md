...
GA4 Reports production-readiness status is tracked in GA4/REPORTS_PRODUCTION_READINESS.md; the Reports section is currently UNVERIFIED, and historical report evidence must not be treated as current whole-section certification.
...
For an active Google Sheets spend source, edits to mapped sheet values must propagate automatically into the same campaign-scoped spend source, `Total Spend`, and downstream financial consumers without requiring the user to reopen or resave the source. Automatic propagation is a near-real-time bounded polling contract: the default runtime target is a provider pull within 1 minute and an open GA4 Overview display refetch within 15 additional seconds, approximately 75 seconds under normal provider/runtime conditions. Literal zero-latency delivery is not guaranteed.
...
- Executive Summary unavailable clicks and impressions should use concise executive-facing copy such as `Unavailable from connected sources`; source-specific diagnostic reasons should stay in the aggregate/API rather than the compact cards.
...
- Executive Summary Risk Assessment must show what it checked. Its configured risk factors are available ROI below 0%, available ROAS below 1x, paid-platform concentration, compatible 7-day revenue decline, aggregate-backed KPI rows below 70% of target, Benchmark rows below 70% of benchmark, and connected-source data freshness warnings. Budget pacing remains in Budget & Financial Analysis until Executive Summary has a shared pacing input.
- Executive Summary Risk Assessment visible risk-input values and KPI/Benchmark risk counts must use the same page-level `performanceSummary` aggregate as the visible metric cards, KPI Progress, and Benchmark Comparison so the risk explanation cannot drift from the numbers executives see.
- Executive Summary Risk Assessment should show a fixed executive-facing `Risk inputs` list: KPI Risk, Benchmark Risk, Data Freshness, ROI / ROAS Risk, 7-Day Trend Risk, and Paid Platform Concentration Risk. Inputs that cannot currently be assessed should remain visible with `Not Applicable` or `Not Enough History` status.
- Executive Summary Risk Assessment must refetch both the Executive Summary endpoint and the page-level outcome totals aggregate when the page mounts or regains browser focus so updated KPI/Benchmark records, source freshness, trajectory, paid-source mix, ROI, ROAS, and aggregate values are reflected without relying on stale cached data.
- Executive Summary should render once campaign data and `/executive-summary` are available. The secondary `/outcome-totals` aggregate should continue loading/refetching in the background and update page-level aggregate values when it arrives, instead of blocking the full subsection behind a skeleton.
- Executive Summary tab selection should be reflected in the URL hash so refreshing on Strategic Recommendations preserves that tab instead of resetting to Executive Overview.
- Executive Summary's corrected sections should be understood as follows: funnel and key metric cards come from page-level `performanceSummary.totals`; KPI Progress and Benchmark Comparison use campaign records for rows/targets and page-level `performanceSummary.totals` for current values; Risk Assessment uses the fixed six risk inputs; Strategic Recommendations use source-capability gating so paid-media guidance requires paid-media main sources plus available spend, revenue, ROI, and ROAS inputs. GA4-only web/outcome recommendations should state live revenue, conversion, and CVR outcomes plainly, compare mapped KPI/Benchmark targets when available, and give a next action without making paid-media claims. Website Outcomes expected impact should be scannable as separate bullets for available data, interpretation, target check, and next action. Strategic Recommendations should update after the Executive Summary page refetches its endpoint and page-level outcome totals on mount, window focus, or the active-tab refresh interval; this is refetch-based synchronization, not real-time push.
- Future Connected Platform integrations must feed Executive Summary through the same `performanceSummary` aggregate source contract used by `/outcome-totals`. Any new or refined main source must be wired into `/executive-summary`, `/outcome-totals`, scheduler snapshots, source freshness, KPI/Benchmark metric mapping, risk inputs, and Strategic Recommendation eligibility before it can be considered production-ready in Executive Summary. Google Ads-specific refinement remains a Connected Platforms concern; Executive Summary should consume it through the aggregate contract once the source is refined.
- The Executive Summary status map lives in `CAMPAIGN_DEEPDIVE_EXECUTIVE_SUMMARY_PRODUCTION_READY.md` under `Executive Summary Status Map`; completed aggregate future-proofing work lives under `Completed Executive Summary future-proofing checklist`.
- The Executive Summary future-platform acceptance gate lives in `CAMPAIGN_DEEPDIVE_EXECUTIVE_SUMMARY_PRODUCTION_READY.md` under `Future Connected Platform acceptance gate`; it is a standing rule for future or refined sources, not an open Executive Summary implementation task. New or refined main sources must pass that gate before they are called production-ready as Executive Summary sources.
- The deployed validation checklist lives in `CAMPAIGN_DEEPDIVE_EXECUTIVE_SUMMARY_PRODUCTION_READY.md` under `Deployed validation checklist and evidence log`; it records acceptance evidence for deployed source mixes after the implementation path is complete. Local regression coverage proves the shared aggregate contract, while deployed or production-like evidence validates specific GA4-only, GA4 plus refined Google Ads, and GA4 plus multiple-paid-source campaigns.
- `/executive-summary` and `/outcome-totals` should use the same aggregate wrapper before calling the shared `performanceSummary` builder so future main source lists are composed in one place.
- Scheduler snapshots that feed Executive Summary trajectory must store `metrics.performanceSummary` with the same normalized main source set used by live Executive Summary aggregates; each future platform must wire its scheduler snapshot source before Executive Summary trajectory is production-ready for that platform.
...
- Executive Summary KPI Progress should list only campaign-level KPI records whose current value can be mapped to an available connected-source aggregate metric. Campaign-level KPI create, update, and delete actions must feed this Executive Summary section by causing the campaign Executive Summary query to refetch. Targets come from the campaign-level KPI records, but current values, progress percentages, and statuses must render from the same page-level `performanceSummary.totals` values as the visible metric cards. Executive Summary must not silently fall back to saved KPI progress/current values for unmapped or unavailable current values.
- Executive Summary Benchmark Comparison should follow the same source-of-truth pattern. Campaign-level Benchmark records define the rows and benchmark targets, and campaign-level Benchmark create, update, and delete actions must cause the campaign Executive Summary query to refetch. `Yours` current values must come from available connected-source aggregate metrics. Unmapped or unavailable benchmark current values should be excluded from Executive Summary Benchmark Comparison instead of silently using saved Benchmark `currentValue` snapshots.
...
Campaign-scoped Custom Report must consume the same `/outcome-totals` `performanceSummary` aggregate used by the other Campaign DeepDive sections. Metric selection should expose only metrics marked available by connected main sources; GA4-only campaigns should show web analytics and outcome metrics only, and paid-media metrics should remain hidden until a connected main paid-media source supplies them. KPI and Benchmark sections use campaign records for rows and targets, but current values come from available aggregate metrics. Campaign-scoped Reports tabs, report type filters, and result counts should include only reports whose saved `campaignId` matches the active Campaign DeepDive campaign, and campaign-scoped pages should not seed global/demo reports. All Reports cards should remain summary-only and should not show connected-source values, KPI/Benchmark row details, generated status pills, or `Includes` configuration details inline. The card edit icon should reopen the report dialog with saved values prefilled, show `Update Report`, and keep update disabled until a value changes. The top-level `Create Report` action should always open a blank create form instead of reusing values from the last edited report; unscheduled create mode should show `Download Report` and save the report under `Standard Reports`, while scheduled create mode should use `Schedule Automated Report`, show `Schedule Report`, and save the report under `Scheduled Reports`. When opened from Campaign DeepDive Custom Report, the report type dropdown should show Campaign DeepDive subsection report types and allow users to select which tabs from the selected subsection to include; selected tabs are saved as report composition in `selectedSections`. Scheduled Reports cards should show those selected tab labels in `Data Included`. Downloaded Campaign DeepDive subsection PDFs should render body content for each selected tab from `performanceSummary.totals` and `performanceSummary.sources`, not only the selected tab names. Scheduled Campaign DeepDive Custom Reports must write backend scheduled report records through `/api/platforms/campaign_deepdive/reports`, persist recipients, schedule time, browser time zone, and saved composition, and be processed by `server/report-scheduler.ts` like platform-level scheduled reports. Deployed scheduled-email evidence can be completed with a GA4-only campaign or after a later source mix such as GA4 plus LinkedIn, but the evidence applies to the source mix tested; each added main Connected Platform still needs its own source-readiness validation before its values are trusted in Custom Report. The standalone `/reports` route is not represented as a sidebar Reports item.
