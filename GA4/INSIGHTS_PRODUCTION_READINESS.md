...
| What to investigate next | grouped finding title, description, recommended check, severity, basis, confidence, hidden count | financial integrity rules; eligible KPI/Benchmark values and snapshot analytics; fail-closed saved-target/current-window period matching; consolidated unverified-target readiness finding; complete 3-day or 7-day calendar comparisons; raw channel context; explicit unavailable/stale/configuration findings |
