...
Current queue status on `2026-07-31`: Commit 16 deployed as `747192ff`; the authenticated connection response for existing campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458` / property `542352127` returned the saved `lookbackDays: 30`, closing the bounded saved-window correction and explaining the change from the prior erroneous 90-day fallback. Commit 17 is locally implemented: Google Sheets Revenue and Google Ads/Meta Spend scheduler replacement plus individual Spend deletion now use exact campaign/source/context-scoped transactions. No production data was changed. Deployment/provider-cycle evidence and the separately authorized disposition of the eight retained sources remain open; OAuth seven-day durability is explicitly unproven.
