...
- Campaign/client delete cascades: partially hardened; campaign delete access, child cleanup reuse, report snapshot/send-event orphan boundaries, and client-delete foreign-campaign fail-closed behavior are regression-guarded. Final table-by-table pass for remaining child tables is still required.
...
- Campaign/client delete cascades: partially hardened. Campaign delete route access, campaign-owned report snapshot/send-event cleanup, and client-delete foreign-campaign fail-closed behavior are covered by regression guards; final table-by-table destructive-scope pass remains outstanding.
- Legacy/stale routes and schema cleanup: not started and must remain last.

## Authoritative Outstanding Queue

1. Campaign/client delete cascade pass.
   ...
   Current progress: campaign-owned report snapshot/send-event cleanup and client-delete foreign-campaign fail-closed behavior are hardened. Remaining: final child-table coverage proof.

2. Legacy/stale route and schema/storage cleanup.
...
The next safest smallest step is to finish item 1 with the remaining table-by-table child coverage check, then move to legacy/stale cleanup only after that passes.
