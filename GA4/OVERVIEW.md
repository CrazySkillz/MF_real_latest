...
- Shopify imported revenue follows the same GA4 source-backed financial model. Current Commit 6 is locally guarded for scheduled/server report PDF totals, GA4 KPI/Benchmark financial jobs, notification enrichment, and GA4 client cache freshness after Shopify-backed revenue source add/edit/delete. Deployed cache-refresh evidence is closed only for the validated Admin API token Shopify edit/re-import packet where revenue, KPI, Benchmark, Reports, and Notifications fetches were observed and endpoints passed. Current Commit 6a is locally regression-covered for Shopify downstream value/content and deployed-closed only for the validated Admin API token campaign/report/email packet: report/PDF inspection, KPI/Benchmark row evidence, notification UI row evidence, and actual delivered report email. Current Commit 8 is local-code validation only for portability/provider-boundary route guards. OAuth, deployed second-campaign/mapping evidence, real >250-order provider evidence, future provider changes, future report/email sends, other report variants, and provider-boundary variants remain unproven.
