...
| Current Commit 2i: GA4 Overview automated validation pack | Implemented locally as validation automation; pending deploy/browser execution before it can be recorded as deployed evidence. | Repeatable GA4 Overview endpoint health, native GA4 daily/to-date/breakdown/table endpoints, source-backed revenue/spend endpoints, stale daily-row status, and optional saved-report snapshot/PDF smoke validation. | Extend `/ga4-overview-validation-runner.js` with `overviewPack(...)` and `reportPack(...)`; add `GA4/OVERVIEW_AUTOMATED_VALIDATION.md`; add focused Playwright wrapper `e2e/ga4-overview-readiness.spec.ts` for logged-in storage-state runs. | Does not mutate analytics data, source records, scheduler state, or calculations. `reportPack({ createSnapshot: true })` creates a report snapshot only when explicitly requested, and `sendTest: true` sends a real test email only when explicitly requested. Still does not prove UI pixel parity, PDF text/value parity, inbox delivery, or future provider behavior by itself. |
| Current Commit 2g: Google Sheets spend tab/mapping variants | Deferred optional breadth gate, not a confirmed code bug and not a repeat of the basic Google Sheets spend lifecycle already closed in 2f. | Google Sheets spend tabs and mapping shapes beyond the validated deployed source paths. | Run only if the certification target requires tab/mapping breadth beyond the already proven Google Sheets spend lifecycle. Validate controlled packets for the specific unproven mapping shape, not another generic spend import. | No for local code-scope certification; yes only for tab/mapping breadth certification. Leave as an explicit caveat if speed is prioritized. |
...
5. Run Current Commit 2h separately for report/email propagation; do not treat live Overview UI evidence or endpoint automation as PDF text parity or inbox-delivery proof.
6. Run Current Commit 3 as a read-only production source-damage inventory before any cleanup claim; do not delete or rewrite rows until the exact damaged-data boundary is documented.
