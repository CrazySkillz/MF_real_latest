...
- Deployed boundary: deployed timing/UI proof is not yet captured. After push/deploy, validate one spend import/update click in the deployed UI and confirm the source/totals still reconcile while the success response is no longer blocked by the heavy GA4 KPI/Benchmark recompute.
