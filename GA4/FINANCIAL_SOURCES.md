...
- in the GA4 Revenue Sources modal, the Shopify edit control belongs beside the Shopify provider label, the source remove control belongs beside its mapped campaign label in the same right-hand action column used by itemized CRM entries, and the Shopify subtotal must share the same right edge as GA4 and CRM provider subtotals
