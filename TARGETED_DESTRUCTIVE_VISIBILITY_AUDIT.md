...
- Source delete/edit flows: started. Google Sheets source route access, connection ownership checks, CSV source-type edit guards, and GA4 revenue/spend add/edit/refresh source identity are complete for the targeted pass; HubSpot, Salesforce, Shopify, LinkedIn, Meta, Google Ads, and Custom Integration still need the same route-by-route check.
...
- Source delete/edit flows: Google Sheets source routes are guarded for campaign access and connection ownership, CSV revenue/spend edit routes refuse non-CSV source IDs, and Google Sheets GA4 revenue/spend add/edit/refresh paths now preserve additive source behavior by updating existing records only through `sourceId`. Continue review for HubSpot, Salesforce, Shopify, LinkedIn, Meta, Google Ads, and Custom Integration delete/edit routes. Confirm each route only mutates the intended campaign/source/platform records.
