...
Whole-Overview status: **PRODUCTION_READY** for certified runtime boundary `12789c1ebb92dd6a905a9f2f0f877f0bc6a90627` and the exact campaign/property/source boundary in `GA4/OVERVIEW_PRODUCTION_READINESS.md`. Evidence-only deployment `e175ac5c` does not change production runtime code. That decision includes the four reviewed configured Spend sources totaling `$2,699.75` and their contribution to visible Overview financial formulas. This does not convert the broader future Google Sheets provider/scheduler lifecycle into an independent blanket certification.

Current clean-certification status:

- **Google Sheets spend has bounded deployed continuity evidence but is not independently clean-certified for its full future provider/scheduler lifecycle.** On `2026-07-11`, the repaired manual run-now path returned `200/success:true` and visibly ingested a new value for source `618e5e12-0f3f-44a2-837a-d2677ad95f64`. On `2026-08-01`, the user completed the post-publish reconnect, changed the mapped sheet, left Overview open without manually refreshing, and confirmed the displayed value updated automatically. This closes stale reconnect and basic no-click propagation for that exact whole-Overview boundary. The chat did not record the exact changed amount, source-count parity, generalized cadence, or a failure-injection result, so those broader claims remain excluded.
...
- On `2026-08-01`, the authenticated read-only inventory and deployed Spend Sources modal matched exactly: three CSV sources totaled $2,000.00 and one Google Sheets source totaled $698.75. The user explicitly approved keeping all four. The later user-controlled sheet change supplied bounded automatic-propagation evidence. No cleanup was authorized or performed.
...
- Current Commit 12's historical chooser packet is committed as `23a4fdc0`. Whole-Overview Commit 5 (`5da5f41c`) later hid Google Sheets temporarily. Current Commit 21 restored Google Sheets beside Google Ads and Upload CSV through the existing campaign-scoped atomic process path; that chooser code was proven at historical runtime `8ba694060411a2a05663a4915652767e4e3ba713` and remains inside certified runtime boundary `12789c1e`. Retained-source reconnect and one automatic-update observation remain bounded evidence.
- The durable reconnect guard and all-mapped-record spend-window fixes are deployed. The successful repaired run provides partial provider evidence, but the repaired token was issued while the Google OAuth app was still `External + Testing` and therefore is not durable production-token evidence.
- On `2026-07-30`, reconnect classifier `efaa6f60` and picker fix `6d32514a` were deployed and the post-publish GA4 reconnect completed. The post-publish Google Sheets reconnect and one automatic mapped-value update were user-confirmed on `2026-08-01`. Automatic renewal and more-than-seven-day durability remain unproven; broader Google verification remains outside the whole-Overview non-scheduler decision.
...
Do not call the complete future Google Sheets spend provider/scheduler lifecycle clean-certified until the exact remaining source-family evidence below is closed. This narrower limitation does not revoke the whole-Overview certification for its exact configured-source boundary.
...
The remaining OAuth verification work is:
...
These Google Sheets/Google OAuth tasks will be completed later when the user resumes this work. No clean-certification claim may treat the hold as completed evidence.
...
- The shared spend recompute incorrectly started at `campaign.startDate`, while the documented spend-to-date and Overview breakdown contracts include every mapped imported record. This produced the observed split: `spend-to-date = 0` and `spend-breakdown = 498.75`.
...
- The recurring disconnect cause is confirmed for this OAuth project: Google Auth Platform showed `External + Testing`, which imposed seven-day authorization/refresh-token expiry for the requested scopes. Publishing status was changed to `In production`; post-publish GA4 reconnect completed on `2026-07-30`, and post-publish Google Sheets reconnect completed on `2026-08-01`. Automatic renewal and more-than-seven-day proof remain open.
- Before Current Commit 8b, the only automatic Google Sheets spend pull was inside the once-daily all-provider scheduler while Overview polled persisted spend every 10 minutes. Current Commit 8b added source-family-only one-minute polling and 15-second Overview refetch. On `2026-08-01`, the user changed the mapped sheet, left Overview open without manual refresh, and confirmed the displayed value updated automatically. Exact timing/delta and failure-injection evidence were not recorded and are not inferred.
...
- Before Current Commit 10, dated imports counted all selected positive rows in the response but omitted blank/invalid-date rows from persisted daily records. Current Commit 10 incorrectly repaired parity by dropping those selected rows, changing the established total-spend contract.
- Current Commit 10a then preserved those rows by assigning blank/invalid-date spend to the import date. That reconciled all-mapped-record totals but falsely represented when the spend occurred and could distort date-range financial metrics.
...
- At Current Commit 10b, snapshot behavior was available by selecting None for Date column, but that alternative was not part of the deployed validation. Whole-Overview Current Commit 5 later superseded that behavior by requiring a Date column for every new GA4 CSV spend source; only already-undated saved sources retain continuity-only snapshot behavior.
- Current Commit 10c removed the current Spend and Campaign identifier selections from CSV Date choices, cleared a stale collision, and added a server fail-closed guard before aggregation. Valid Date and None mappings were unchanged at that historical revision; Whole-Overview Current Commit 5 later removed None from new GA4 CSV creation.
...
| Latest-day/daily spend materialization | New GA4 CSV processing requires mapped dates and materializes daily records. Google Sheets and already-undated saved CSV continuity sources may materialize a current-date aggregate row when no date column exists. | Code trace through CSV and Google Sheets process routes and storage active-source joins. | Not certified as a separate visible card beyond its use in current spend records and breakdowns. |
...
Current Commit 10a local regression validation on 2026-07-11:

- Exact Alpha aggregation passed locally: four selected positive rows total $1,250 and Beta is excluded. The later review determined that assigning the $1,100 with unknown dates to the import date was analytically unsafe.
- Focused CSV packet passed: 10 tests.
- Adjacent GA4 Overview spend packet passed: 6 files, 65 tests.
- npm run check passed.

Current Commit 10b local regression validation on 2026-07-11:

- Focused CSV packet passed: 10 tests.
- Current adjacent GA4 spend packet passed: 7 files, 65 tests.
- A broader eight-file run passed 71 of 72 tests; the excluded failure is a pre-existing static assertion in campaign-financial-analysis-regression.test.ts that expects the platform-spend formula without its existing custom-integration term. It is not CSV evidence and was not changed.
- npm run check passed.
- git diff --check passed.

Current Commit 10b deployed UI validation on 2026-07-11:

- The mixed-date Alpha CSV displayed Selected spend rows contain blank or invalid dates and did not proceed as a successful import.
- After the blank-date and invalid-date rows were removed, the remaining valid dated rows imported as a CSV source and the user confirmed GA4 Overview Total Spend updated correctly.
- This is bounded evidence for deployed rejection and valid-row propagation. It does not prove an exact rejected-state before/after total, a corrected all-row $1,250 dated import, or the optional no-date snapshot path.

Current Commit 10c local regression validation on 2026-07-11:

- Focused CSV packet passed: 11 tests.
- Current adjacent GA4 spend packet passed: 7 files, 66 tests.
- GA4 UI regression packet passed: 32 tests.
- npm run check passed.
- git diff --check passed for the Current Commit 10c files.

Current Commit 10d local regression validation on 2026-07-11:

- Focused CSV packet passed: 12 tests.
- Current adjacent GA4 spend packet passed: 7 files, 67 tests.
- GA4 UI regression packet passed: 32 tests.
- npm run check passed.
- git diff --check passed for the Current Commit 10d files.

Current Commit 10d deployed UI validation on 2026-07-12:

- The user confirmed the deployed CSV Date dropdown no longer shows the non-date revenue and conversion-value columns.
- This proves the visible Date-option filter for the tested CSV only. The server-side forged numeric-date rejection was not manually invoked and remains automated evidence.

Current Commit 10e deployed reconciliation validation on 2026-07-12:

- Baseline: Total Spend $1,598.75; 4 sources.
- Mixed blank/invalid-date Alpha import returned the expected error and left Total Spend at $1,598.75 with 4 sources.
...
Current Commit 12 local and deployed validation on 2026-07-12:
...
- Deployed UI confirmation: the new-source Spend chooser showed Google Ads, Google Sheets, and Upload CSV, and did not show LinkedIn Ads or Meta / Facebook.
...
- One deployed no-click source-family update was user-confirmed on `2026-08-01`. The exact elapsed time, changed amount, same-source/source-count parity, and failure behavior were not recorded, so the evidence is not generalized into exhaustive timer certification.

## Lifecycle Matrix

| Source family | Add/import | Edit/update | Refresh/reprocess | Delete/deactivate | Boundary |
| --- | --- | --- | --- | --- | --- |
| Google Sheets spend | Historical packets closed for sources `8f67b03f-a00b-434f-b81f-db1b2b951595` and `62772549-88dc-4cc5-bfe6-2e991d518ef5`. | Historical packets closed for those exact sources and runs. | **Deployed continuity observed:** source `618e5e12-0f3f-44a2-837a-d2677ad95f64` previously passed repaired manual reprocess; after the `2026-08-01` post-publish reconnect, a mapped sheet change became visible automatically with no manual refresh. Exact delta/source-count/failure evidence and durable-token operation remain unproven. | Historical delete packets remain closed for their exact source IDs. | Whole-Overview non-scheduler scope passes; full source-family certification still requires the explicitly listed exact and durability evidence. |
| Upload CSV spend | Current Commit 10e proves invalid-date no-mutation plus exact $150 add from a $1,598.75 baseline. Historical deployed add/import also remains bounded at $2,020. | Current Commit 10e proves same-source replacement from $150 to $1,250 with a $1,100 Total Spend delta and no source-count increase. Historical edit evidence remains bounded at $3,120. | Not applicable as a scheduler/provider action. | Current Commit 10e proves the temporary $150 source deletion restored the exact $1,598.75 / 4-source baseline; historical delete remains bounded to source c3611c0f-4bbf-47b9-8615-93e4b140385e. | Dated Alpha fixture lifecycle is closed; existing legacy no-date snapshot continuity and unlisted CSV shapes remain outside this claim. |
...
| Current Commit 5: Fresh reconnect token and all-mapped-record spend parity | **Code deployed; partial runtime validation passed.** | Prevent explicit reconnect from reusing a rejected refresh token; make persisted spend-to-date use the same all-mapped-record window as spend breakdown. | The repaired source returned `200/success:true`; still capture `spend-to-date === spend-breakdown` after an exact mutation, and do not treat the pre-publish Testing token as durable evidence. | Yes until parity and production-token durability are validated. |
...
| Current Commit 7: Successful automatic timer mutation | **Bounded deployed update observed; exact packet incomplete.** | Automatic update after a user changes the sheet. | On `2026-08-01`, a post-publish reconnect was followed by a mapped sheet change that appeared automatically in an open Overview without manual refresh. Exact changed amount, elapsed time, source-count and downstream endpoint parity were not recorded. | No for whole-Overview non-scheduler readiness; yes only for a broader exact timer claim. |
| Current Commit 8: Durable GA4 OAuth reconnect | **Post-publish reconnect and immediate deployed UI validation passed; durability remains open.** | Require a newly issued GA4 refresh token, preserve existing GA4 rows until a durable replacement is created and promoted, and show reconnect only for a missing or Google-rejected durable credential. | `efaa6f60` and `6d32514a` deployed; GA4 connected, selected campaign metrics loaded, and refresh persistence passed on `2026-07-30`. Still prove automatic token renewal and more-than-seven-day durability. | Yes for durable GA4-derived downstream spend-consumer validation; does not block source-backed Total Spend itself. |
...
| Current Commit 8b: Automatic Google Sheets spend propagation | **Committed, pushed, deployed; bounded no-click update observed.** | Replace the once-daily-only Google Sheets spend freshness gap without increasing any other source-family cadence. | Commit `e4c3de5a` adds a 1-minute source-only timer and 15-second Overview polling. One deployed no-click mapped-value update was user-confirmed on `2026-08-01`; exact timing/delta and failure evidence remain outside that observation. | Closed for the bounded configured-source Overview claim; broader timing/failure claims remain excluded. |
| Current Commit 8c: Production Google OAuth publishing and verification | **Publishing and both post-publish reconnects complete; GA4 durability observed; broader Sheets durability open.** | Eliminate Testing-mode seven-day token expiry for Google Sheets and GA4. | GA4 reconnect completed `2026-07-30` and the certified GA4 connection remained provider-usable on `2026-08-10`; Google Sheets reconnect completed `2026-08-01`. Broader Google verification remains separate. | No open gate for the exact Overview certification; yes only for broader future Google Sheets credential claims. |
| Current Commit 9: Additional Google Sheets tab/mapping variants | **Optional; not required for chooser restoration.** | Unlisted tabs, headers, dates, filters, and mappings. | Run one bounded packet per mapping family before broadening mapping claims. | Only blocks broader mapping claims. |
| Current Commit 10: Upload CSV automated validation packet | **Committed and pushed as efef0493; failed first deployed validation and retained as negative evidence.** | CSV parser, aggregation, filtering, edit, negative, and atomic materialization paths. | Unsupported spend platformContext and the incorrect $150 expectation are preserved as regression evidence; Current Commits 10a-10e supersede and reconcile the affected behavior. | No; superseded by completed 10b-10e evidence. |
| Current Commit 10a: Restore GA4 CSV compatibility and all-selected-row totals | **Committed and pushed as 20db9ff8; superseded before clean validation.** | GA4 context normalization plus the Alpha $1,250 filter case. | GA4 normalization and campaign summation remain valid; the import-date fallback is analytically unsafe and is removed by Current Commit 10b. | Superseded by Current Commit 10b. |
| Current Commit 10b: Reject unknown dates without inventing spend timing | **Committed and pushed as e6149110; deployed exact no-mutation validation passed under 10e.** | Dated CSV inputs containing selected positive rows with blank/invalid dates. | Invalid import preserved the exact $1,598.75 / 4-source baseline; corrected rows subsequently reconciled through add/delete and edit. | Closed for the tested dated fixture. |
| Current Commit 10c: Prevent duplicate CSV mapping roles | **Committed and pushed as ab54104e; deployed UI behavior is covered by the stricter 10d filter.** | CSV Date mapped to the selected Spend or Campaign identifier column. | Omit conflicting Date choices only in the CSV wizard, clear stale collisions, and reject duplicate-role requests server-side before aggregation. | Closed for visible role filtering; forged server request remains automated evidence. |
| Current Commit 10d: Filter CSV Date choices by sampled values | **Committed and pushed as 7b8ee0f0; deployed bounded UI validation passed.** | Non-date revenue/conversion columns shown as Date choices and numeric values coerced into years. | At that revision, tested CSV no longer exposed revenue/conversion as Date choices and retained None plus the date column; Whole-Overview Current Commit 5 later removed None from new GA4 CSV creation. | Closed for the reported dropdown defect; forged numeric-date rejection remains automated evidence. |
...
| Current Commit 11: Deferred broader CSV/source validation | **Optional, non-blocking, and deferred for later.** | Existing legacy no-date CSV snapshot continuity, unusual/unlisted CSV shapes, deployed forged-request checks, and additional campaign/source inventories. | Run bounded packets only when the user later broadens the certification claim. | No for the existing bounded dated-CSV claim; only blocks broader claims. |
| Current Commit 12: Limit v1 Spend chooser source families | **Committed and pushed as `23a4fdc0`; deployed chooser UI confirmed.** | Remove LinkedIn Ads and Meta / Facebook from new spend-source setup without damaging existing-source continuity. | User confirmed only Google Ads, Google Sheets, and Upload CSV are shown; LinkedIn/Meta are absent. Existing-source continuity remains local evidence only. | Closed for the v1 new-source chooser scope. |
...
- CSV campaign filtering is exact. New GA4 CSV imports require dates and fail before mutation if any selected positive row has a blank/invalid date, so the app never invents temporal provenance. Only already-undated saved continuity sources retain no-date snapshot aggregation.
...
- Whole-Overview Current Commit 5 (`5da5f41c`) narrows the GA4 new-source Spend chooser to Google Ads and Upload CSV. The user confirmed the deployed chooser no longer shows Google Sheets. Google Sheets, LinkedIn Ads, and Meta / Facebook remain available only through retained exact-source continuity paths.
- New GA4 CSV Spend requires a Date column in UI and API. Already-undated saved sources remain continuity-only; a dated saved source cannot be converted to undated.
...
- Local code gives Google Sheets spend its own bounded polling path without adding CSV or another provider family to that timer; one deployed no-click update was observed, without proving every timing/failure case.

Proven by recorded deployed evidence:

- Historical lifecycle packets remain bounded to their recorded source IDs. For the current source, one repaired manual run-now returned `200/success:true`; after post-publish reconnect, one mapped sheet change appeared automatically in the open Overview without manual refresh. This does not prove exact reconciliation, every timer/failure case, or durable production OAuth.
...
- Current Commit 10b deployed UI evidence confirms mixed blank/invalid-date input is rejected and that the remaining valid dated rows update Total Spend through a CSV source after those invalid rows are removed.
- Current Commit 10d deployed UI evidence confirms non-date revenue and conversion-value columns are removed from Date choices for the tested CSV.
- Current Commit 10e deployed evidence confirms exact invalid-import no-mutation, $150 add, exact delete restoration, and same-source edit replacement to $1,250 with Beta excluded and no source-count increase.
- Current Commit 12's earlier three-option chooser evidence is superseded; whole-Overview Current Commit 5's deployed two-option chooser was user-confirmed.

## Unproven

- Durable GA4 and Google Sheets production-mode operation after more-than-seven-day evidence.
...
- Exact proof that the Google Sheets-spend-only timer met the configured interval, preserved source identity/source count, propagated the exact changed amount across all downstream endpoints, and retained last-good values under a deployed provider failure. One successful visible no-click update was observed, but those exact details were not recorded.
...
- New optional no-date GA4 CSV snapshot import is locally disabled. Existing undated sources remain unproven continuity rows for later inventory/lifecycle reconciliation; this does not expand the bounded dated-CSV claim.
- Deployed editing of an existing LinkedIn/Meta spend source remains unproven; retained continuity is locally regression-covered and outside the v1 new-source chooser claim.
- Deployed manual proof that forged duplicate-role and numeric-date requests fail without source or Total Spend mutation remains unproven and is explicitly deferred for later; these server guards currently have automated evidence.
- Duplicate-header files, locale-specific decimal formats, ambiguous non-ISO valid dates, row/file limit boundaries, and other unlisted mappings remain unproven and are explicitly deferred for later.
- Other campaigns/properties outside the recorded packets remain unproven; additional read-only inventories are explicitly deferred for later.
...
- GA4/Google Sheets more-than-seven-day durability, exact deployed mutation reconciliation, and exhaustive one-minute timer/failure behavior beyond the bounded successful update already observed.
...
GA4 Overview Google Sheets spend has deployed current-value, retained-source, post-publish reconnect, and one no-click automatic-update observation. Its full future provider/scheduler source-family certification still excludes generalized timing/delta/failure and more-than-seven-day durability claims. The bounded Google Sheets automatic-update gate and the separate whole-Overview target GA4 daily scheduled-run gate are closed for the exact certified boundary. Upload CSV retains its bounded deployed dated-Alpha lifecycle packet; new GA4 CSV sources are dated-only by automated UI/API guards. Existing undated sources, unusual/unlisted CSV shapes, deployed forged-request behavior, and additional inventories remain unproven. Google Ads and all other source families remain excluded from this component certification.
