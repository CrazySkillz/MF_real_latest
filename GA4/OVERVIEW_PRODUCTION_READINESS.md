...
3. Current Commit 3: Shopify edit/update source identity and rematerialization. Local root cause confirmed: GA4 Overview exposes Shopify edit through the Revenue Sources modal, but Shopify edit identity was less explicit than HubSpot/Salesforce because the selected source ID was carried only through `initialMappingConfig`, and the save route could still fall back instead of failing closed if a requested source was not resolved before mutation. Smallest safe local fix: pass the selected Shopify `sourceId` as an explicit wizard prop, use that stable edit source ID in both preview and save payloads, and make the save route return `404` instead of updating/creating another Shopify source when `requestedSourceId` is present but no active Shopify source is resolved before Shopify connection/source mutation. Local validation: `npm test -- server/shopify-revenue-regression.test.ts`, `npm test -- server/endpoint-auth-audit.test.ts server/revenue-additivity.test.ts`, and `npm run check`. Deployed edit/update evidence is pending; do not close edit/update until a real UI edit proves the same source ID is updated, old records for that source are replaced, totals/source modal match, no duplicate active Shopify source is created, and unrelated revenue/spend sources are unchanged.
4. Current Commit 4: Shopify delete/deactivate exact source boundary. Pending. Validate only if the current UI/API exposes Shopify source deletion; prove only the selected Shopify source/records are removed or deactivated, total revenue decreases by the exact Shopify amount, unrelated revenue/spend sources are preserved, and source inventory remains clean.
5. Current Commit 5: Shopify refresh/reprocess applicability trace. Pending. First prove whether GA4 Overview exposes a Shopify run-now refresh/reprocess action or scheduler path. Current local trace found no GA4 Overview Shopify run-now route; do not implement refresh behavior unless a real reachable route/job is confirmed.
6. Current Commit 6: Shopify downstream propagation. Pending after the core source lifecycle is closed. Reports, KPI/Benchmark, notifications, and emails are not part of Current Commit 2; they are separate downstream evidence packets if Shopify clean certification is expanded beyond GA4 Overview source values.
...
- Current Commit 3 is local-code validated only. Deployed edit/update validation remains pending and must prove same-source update/rematerialization with no duplicate active Shopify source and no unrelated source changes.
...
| Shopify revenue | `Total Revenue`, revenue sources modal, Profit, ROAS, ROI, CPA, report payload values after the change | Closed only for deployed Admin API token normal save/materialization on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458` after UI validation plus read-only endpoint packet: Shopify `$99.99` and total revenue `$16,799.99` matched source/breakdown endpoints with clean inventory | Current Commit 3 local source-identity/rematerialization fix and regression coverage are complete; deployed UI edit/update evidence is pending | Required | Required | Partially closed only for the validated Admin API token add/import path. OAuth, >250-order provider pagination, deployed edit/update, refresh/reprocess, delete/deactivate, other campaigns, other mappings, Reports, KPI/Benchmark, emails, and future provider changes remain unproven. |
