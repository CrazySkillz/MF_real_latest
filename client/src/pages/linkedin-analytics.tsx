...
function LinkedInAnalyticsCampaign({ campaignId }: { campaignId: string }) {
  ...
  const { data: benchmarks = [], refetch: refetchBenchmarks } = useQuery<any[]>({
    ...
    queryFn: async () => {
      const url = `/api/campaigns/${encodeURIComponent(String(campaignId))}/benchmarks/evaluated${sessionId ? `?session=${encodeURIComponent(String(sessionId))}` : ""
        }`;
  ...
  const deleteLinkedInRevenueSourceMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["/api/salesforce", campaignId, "pipeline-proxy"] });
              ...
              <div className="max-w-7xl mx-auto space-y-6">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  ...
                  <TabsContent value="overview" className="space-y-6 fade-in" data-testid="content-overview">
                    ...
                    {false && linkedinCampaignBreakdown.length > 0 && (
                      <Card>
                        <CardHeader>
