...
- HubSpot and Salesforce `Review Settings` show the selected deal/opportunity labels together with the amount that will be imported for each selected record, and HubSpot also shows the selected CRM value to saved platform-campaign mapping before save; confirmed `Total Revenue (to date)` remains the sum of included confirmed records
