...
| Current Commit 2b revenue source delete false-failure guard | `npm test -- server/ga4-source-lifecycle-recompute-regression.test.ts`; `npm run check`; deployed DELETE evidence captured on `2026-07-01T18:28:32.608Z`. | GA4 revenue source mutation routes keep source deletion/update response semantics stable even if downstream GA4 KPI/Benchmark recompute fails after the source mutation; deployed evidence proves the fixed response for disposable source `32661325-d2a5-404f-a898-2c84e4275809`. | Visible UI parity is still required before closing Google Sheets revenue delete/deactivate. |
| Current Commit 2c revenue import/update response latency guard | `npm test -- server/ga4-source-lifecycle-recompute-regression.test.ts`; `npm run check`. | GA4 revenue source process routes no longer block the user-facing success response on the heavyweight GA4 KPI/Benchmark daily/history/alert job after source records are durable; source-backed campaign current values still refresh synchronously before the response. | Does not prove deployed latency improvement until the pushed commit is deployed and timed during a real revenue import/update; downstream GA4 KPI/Benchmark history and alerts may reconcile shortly after the source save response. |
...
| Current Commit 2b: Revenue source delete false-failure response guard | Deployed fixed-response endpoint evidence captured; visible UI parity pending. | Google Sheets revenue delete/deactivate false-failure observed during Current Commit 2. | Guard GA4 KPI/Benchmark recompute inside `recomputeCampaignDerivedValues` so completed source mutations do not return false failure responses if downstream recompute fails; keep recompute logged and continue the source endpoint response. | No for local code-scope certification; yes for full Google Sheets revenue delete/deactivate closure until visible UI parity is confirmed and recorded. |
| Current Commit 2c: Revenue import/update response latency guard | Implemented in this commit; deployed latency validation pending. | GA4 revenue source add/import and edit/update paths for source-backed revenue. | Keep source writes and source-backed campaign current-value refresh synchronous, then schedule the heavier GA4 KPI/Benchmark daily/history/alert reconciliation after the response so imports/updates are not blocked by live GA4/Data API or global alert work. | No for local code-scope certification; yes for deployed latency certification until a real revenue import/update is timed after deploy. |
...
- Certification boundary after this evidence: the deployed fixed DELETE response path is proven for this disposable Google Sheets revenue source. Full Google Sheets revenue delete/deactivate closure still requires visible UI parity confirmation that Total Revenue and the Revenue Sources modal match the endpoint totals and no longer show the deleted Google Sheets row.
...
- Certification boundary: local code and tests prove the response no longer awaits the heavyweight GA4 job. Deployed performance remains external until a real revenue import/update is timed after Render deploys the commit. A short delay can still exist before downstream GA4 KPI/Benchmark history and alerts finish reconciling; that is now explicit background work rather than a blocking import/update step.
...
1. Next source family/action: confirm visible UI parity for the Google Sheets revenue delete/deactivate evidence captured on `2026-07-01T18:28:32.608Z`.
2. Confirm the deployed Overview UI shows imported revenue back to `$600` plus native GA4 revenue as applicable, the Revenue Sources modal no longer shows Google Sheets source `32661325-d2a5-404f-a898-2c84e4275809`, and the remaining CSV revenue source still appears at `$600`.
3. Record the UI confirmation in this file before moving to any Google Sheets spend lifecycle action.
4. Do not create or delete another Google Sheets revenue source for Current Commit 2b unless this endpoint evidence is contradicted.
5. After UI confirmation is recorded, move to the next source family/action: Google Sheets spend lifecycle validation.
6. If any action changes an unrelated source ID, duplicates an active source, loses provenance, changes totals unexpectedly, broadens source scope, or still shows a false failed response after a successful delete, stop Current Commit 2 and add the exact runtime fix as the next Current Commit.

What remains unproven externally:

- Google Sheets revenue delete/deactivate has deployed fixed-response endpoint evidence and after-delete endpoint reconciliation; visible UI parity remains unproven until the user confirms Total Revenue and the Revenue Sources modal match the endpoint totals. Google Sheets spend lifecycle actions remain unproven until each relevant add/import, edit/update, refresh/reprocess, and delete/deactivate action has deployed before/after endpoint evidence plus UI parity.
