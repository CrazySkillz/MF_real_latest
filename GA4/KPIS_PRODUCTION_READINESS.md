...
- the Current Commit 10 dry-run found 0 cleanup candidates and applied 0 changes. Gate 1 later proved that its 17 skips were 10 financial KPI rows plus 7 immutable duplicate-email audit rows, not 17 financial KPIs. Three simulated-property financial rows are now deterministically verified; 4 live-provider and 3 missing-property rows keep the target-data gate pending
...
- target inventory: dry-run mode, 0 candidates, 17 skipped items, 0 applied. The exact split is 7 access-token financial KPIs, 3 missing-property financial KPIs, and 7 immutable duplicate-email audit rows. Gate 1 proves three of the access-token rows use the deterministic `yesop` property and exactly match their current values, leaving 7 financial rows unresolved. No `--apply` command or destructive production action was run
...
- proven: Current Commit 10 evidence revision `6755e6f369e98212f99a23d479aa88db92d8f8e5` is deployed in production; the three widened columns and current row bounds are present on the target database; all 10 financial KPIs have an owner and valid reporting timezone; 3 deterministic simulated-property rows exactly match their resolved formula/source values; the 7 duplicate-email audit skips are immutable evidence retained by design; the user-confirmed Commit 7 browser PDF downloads, opens, and looks correct
- unproven: 4 live-provider financial rows and 3 missing-property financial rows; current deployed valid-zero/unavailable/stale/failure/source-mix/timezone UI matrix; timer-fired exact updated/skipped/failed KPI IDs; live provider and token-refresh behavior bound to the deployed SHA; production notification retry after an injected failure; current direct snapshot/test-send/manual-send/scheduled-report freshness parity; and SHA-bound alert/report inbox delivery. Historical audit rows or provider acceptance are not inferred as current proof

Chronological smallest-safe validation-closure queue (no implementation authorized by this commit):

1. **Target-data Gate 1 — attempted and partially closed; gate remains pending.** The 17 skips are fully classified. Close the remaining 4 live-provider rows using current read-only totals with refresh disabled, and close the 3 missing-property rows only after the user separately reconnects/selects a property. Keep any configuration or cleanup action outside this read-only audit.
...
| `dd14fc00c0b8` | ROAS | owner present; no active connection/property; UTC; no campaign filter or financial sources | same captured windows | no provider or persisted inputs; zero cannot be classified as authoritative; pending property configuration |
| `3d0f126ef97f` | CPA | same disconnected campaign boundary | same captured windows | no provider or persisted inputs; zero cannot be classified as authoritative; pending property configuration |
| `befd1e90d209` | Revenue | owner present; primary live property; UTC; campaign filter; HubSpot revenue and manual spend source | same captured windows | no persisted native rows; `$12,000` imported revenue; spend source has no records; provider precedence cannot be verified read-only; pending |
| `7ed1d5c4d10f` | Revenue | owner present; primary live property; Europe/Amsterdam; campaign filter; CSV/HubSpot/Shopify revenue and CSV/Sheets spend | same captured windows | 16 persisted native rows through `2026-07-12`, `$33,400` imported revenue, and `$2,699.75` spend exist; live provider remains the first-precedence candidate and is unavailable to this read-only audit; pending |
| `ff24c717b842` | Revenue | same live campaign/property/source boundary; its only calculation metadata is alert-email schedule | same captured windows | same persisted/source inputs; live provider remains unverified; pending |
| `85cf158ed54d` | Revenue | owner present; active primary connection has an empty Property ID; UTC; campaign filter; HubSpot revenue and ad-platform spend | same captured windows | no native rows; `$10,000` imported revenue and `$13,858.31` spend exist, but native GA4 boundary is absent; pending property selection |
...
All 10 rows are standard recognized metrics with `applyTo=all`; there is no specific-campaign override or hidden source-ID calculation mapping. The 4 live-property rows have encrypted access/refresh material but their recorded access-token expiry is in the past, and this local environment does not have the production token-encryption key. Calling deployed validation or refresh paths could rotate and persist tokens, so current provider totals were not inferred. The shortest safe closure is a refresh-disabled read-only provider capture using current production credentials, or evidence from a naturally occurring production refresh followed by an exact read-only totals capture. The other 3 rows require separate user-authorized property configuration before they can be re-inventoried.
