...
describe("GA4 Benchmark provider validation guard", () => {
  it("keeps the provider validation route campaign-scoped and Benchmark-read-only", () => {
    ...
    expect(route).toContain('const financialWindow = { startDate: "1900-01-01", endDate: currentValueEndDate };');
    expect(route).toContain('const spendSourceWindow = { startDate: "1900-01-01", endDate: currentValueEndDate };');
    expect(route).toContain("spend: spendSourceWindow");
    expect(route).not.toContain("getGA4KPIFinancialSourceWindow");
    ...
    expect(route).toContain("const schedulerInputsForMetric = currentValueWindowInputs;");
    expect(route).not.toContain("? requestedWindowInputs");
    expect(route).toContain("const uiFinancialProviderCandidate = currentValueProviderInput ? {");
    expect(route).toContain("value: currentValueProviderInput");
    expect(route).toContain("value: currentValueDailyInput");
