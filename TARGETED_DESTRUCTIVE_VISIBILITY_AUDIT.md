...
1. Ad-platform source-flow pass.
   Scope: Google Ads spend/revenue source read, edit, delete, disconnect, refresh, and scheduler paths. Meta is code/test complete for current test-mode spend scope; no Meta revenue source is currently configured.
   ...
   Fast execution order: Google Ads next.
...
The next safest smallest step is item 1: audit Google Ads source-flow routes using the same ad-platform guard pattern just applied to Meta.
