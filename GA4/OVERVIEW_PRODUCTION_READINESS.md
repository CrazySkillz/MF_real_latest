...
Next active HubSpot step: capture deployed Current Commit 4.15 output for at least one additional campaign/property packet using `hubspotOtherCampaignPortabilityPack(...)`. Current Commit 4.14 email/PDF evidence is user-confirmed only for the configured GA4 Overview Report packet; future sends and report variants still require their own evidence.
