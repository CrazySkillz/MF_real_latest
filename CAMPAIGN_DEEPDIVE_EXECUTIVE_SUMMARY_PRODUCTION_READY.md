...
- KPI Progress UI correction: Executive Summary KPI progress bars and status labels now follow the campaign-level KPI progress pattern (`Target Achieved`, `On Track`, `Needs Attention`, `Below Target`) and no longer show priority badges such as `medium` beside KPI names.
...
- KPI Progress UI regression now asserts Executive Summary uses campaign-level status labels/default progress bar styling and does not render KPI priority badges beside metric names.
