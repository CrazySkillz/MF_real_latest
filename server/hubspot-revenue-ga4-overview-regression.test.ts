...
describe("HubSpot revenue GA4 Overview regression guard", () => {
  ...
  it("keeps HubSpot Pipeline Proxy scoped and separate from confirmed GA4 financial revenue", () => {
    ...
    const salesforcePipelineRoute = sliceBetween(
      routes,
  ...
  it("removes individual HubSpot revenue items without changing the Salesforce path", () => {
    ...
    expect(revenueSourcesDialog).toContain('!hasSingleSourceBreakdown && sourceType !== "hubspot" && sourceType !== "salesforce" && <p');
    expect(revenueSourcesDialog).not.toContain('sourceType !== "hubspot" && <p');
