...
- [x] Completed locally: Shopify revenue OAuth now builds its callback URL from `SHOPIFY_REDIRECT_URI`, `SHOPIFY_APP_BASE_URL`, or the canonical app base URL before falling back to the current request host, because Shopify requires the redirect URL to exactly match its whitelisted app callback.
