...
- On `2026-08-01`, the authenticated read-only inventory and deployed Spend Sources modal matched exactly: three CSV sources totaled $2,000.00 and one Google Sheets source totaled $698.75. The user explicitly approved keeping all four. This closes only their keep/remove disposition and confirms they are not hidden damage; it does not close the active Google Sheets refresh/credential lifecycle. No cleanup or production mutation was authorized or performed.
...
- On `2026-07-30`, reconnect classifier `efaa6f60` and picker fix `6d32514a` were deployed and the post-publish GA4 reconnect completed. The user confirmed GA4 connected, the intended campaign selection/count and metrics loaded, and the state remained correct after refresh. Automatic renewal and more-than-seven-day durability remain unproven; the final Google Sheets reconnect and broader Google verification work remain open.
...
- perform one final Google Sheets reconnect; the post-publish GA4 reconnect completed on `2026-07-30`
...
- The recurring disconnect cause is confirmed for this OAuth project: Google Auth Platform showed `External + Testing`, which imposed seven-day authorization/refresh-token expiry for the requested scopes. Publishing status was changed to `In production`; the post-publish GA4 reconnect completed on `2026-07-30`, while the final Google Sheets reconnect, Google verification, automatic renewal, and more-than-seven-day proof remain open. The token used for the earlier successful Google Sheets repaired run was issued before the publishing change.
...
| Current Commit 8c: Production Google OAuth publishing and verification | **Publishing changed; post-publish GA4 reconnect completed; durability and Google Sheets remain open.** | Eliminate Testing-mode seven-day token expiry for Google Sheets and GA4. | Capture GA4 automatic renewal and >7-day durability; complete the final Google Sheets reconnect plus public-page/domain and broader Google verification work when resumed. | Yes for stable-connection clean certification. |
...
- Durable GA4 production-mode operation after automatic renewal and more-than-seven-day evidence; durable Google Sheets credentials after its final post-publish reconnect and remaining Google OAuth work.
...
- Final post-publish Google Sheets reconnect, GA4 automatic renewal/more-than-seven-day evidence, exact deployed mutation reconciliation, and successful one-minute automatic timer behavior after Current Commits 5, 7, 8b, and 8c resume.
...
GA4 Overview Google Sheets spend is not yet clean-certified, so whole-Overview Current Commit 5 blocks new GA4 setup while preserving existing exact-source continuity. The deployed chooser removal was user-confirmed. Upload CSV retains its bounded deployed dated-Alpha lifecycle packet; new GA4 CSV sources are dated-only by automated UI/API guards, but no separate deployed CSV mapping check was performed for this packet. Existing undated sources, unusual/unlisted CSV shapes, deployed forged-request behavior, and additional inventories remain unproven. Google Ads and all other source families remain excluded from this component certification.
