...
- The concise current Overview decision and active gates live in `GA4/OVERVIEW_PRODUCTION_READINESS.md`; detailed evidence lives in `GA4/OVERVIEW_PRODUCTION_READINESS_EVIDENCE.md`; chronological Current Commits 0–21 live in `GA4/OVERVIEW_PRODUCTION_READINESS_HISTORY.md`. Current status is **UNVERIFIED**: Current Commit 20's corrected legacy import boundary and Current Commit 21's restored Google Sheets financial-source choosers are local and await deployment validation; OAuth durability remains due on or after `2026-08-07`.

- The Overview tab is not clean-certified or production-ready for live users. Current Commit 20's corrective follow-up must be deployed and validated, then the current-release scheduled-run and OAuth durability gates must pass. No production cleanup is authorized.
...
- GA4 KPI status remains **UNVERIFIED** for exact deployed SHA `249fc18787409fb5d3700a7f64e091822a99c2d3`. Commit 14A's corrected Engagement Rate parity is complete. Commit 14B is the single combined final task; its local release checks pass, while exact-SHA browser-PDF/report execution and provider-confirmed delivery remain blocked by the explicit no-recompute boundary and Mailgun's daily limit until `2026-08-05T19:58:54Z`. Exact evidence is in `GA4/KPIS_PRODUCTION_READINESS.md`.
- GA4 Benchmark re-certification has completed all local, deployed parity, lifecycle, security, and deterministic scheduler gates. Production status remains `UNVERIFIED` only until Current Commit 14 records one successful natural timer-fired GA4 daily run on the unchanged reviewed revision. Reports delivery and inbox receipt belong to the separate Reports audit and do not gate Benchmarks.
...
- Google Sheets/Upload CSV revenue readiness: Upload CSV Revenue is clean-certified for its validated documented scope. Current Commit 21 re-enables Google Sheets Revenue through the existing scoped workflow, but Google Sheets Revenue is not independently clean-certified; deployment and the remaining source-family provider/lifecycle evidence are still required. Use `GA4/OVERVIEW_REVENUE_PRODUCTION_READINESS.md` for the exact boundary.
...
- Google Sheets Revenue is no longer on hold. Current Commit 21 restores its GA4 chooser/API entry point without changing production data; this re-enablement is not certification.

- Current GA4 Insights spend boundary: Google Ads, Google Sheets, and Upload CSV are enabled and must all pass the revision-specific Insights certification. LinkedIn, Meta/Facebook, and Instagram are not enabled for this release. Separate Overview source-certification records do not certify or exclude the live Insights Google Ads path.
...
- `GA4/OVERVIEW_PRODUCTION_READINESS_HISTORY.md`
  Chronological Overview Current Commit 0–21 and UI-validation ledger preserved from the pre-split canonical file.
- `GA4/KPIS.md`
...
- `GA4/KPIS_PRODUCTION_READINESS.md`
  Canonical whole-tab KPIs production-readiness source of truth. Current status: UNVERIFIED for exact reviewed/deployed SHA 4287871f82a84afa70b50cd6cc0f1810040bb016 with two precise blockers recorded in the controlling status.
...
- `GA4/INSIGHTS_PRODUCTION_READINESS.md`
  Canonical live-tab Insights production-readiness source of truth. Current status: `UNVERIFIED` after invalidation of the earlier revision-specific certification. Reports-owned behavior is outside this certification.
- `GA4/REPORTS.md`
...
- GA4 Insights live-tab status is `UNVERIFIED`; `GA4/INSIGHTS_PRODUCTION_READINESS.md` is the sole controlling source for the revision-specific boundary and evidence.
- The current GA4 Insights new-spend chooser includes Google Ads, Google Sheets, and Upload CSV. LinkedIn, Meta/Facebook, and Instagram are outside this release and absent from that chooser; their platform contexts must not feed GA4 Insights totals.
