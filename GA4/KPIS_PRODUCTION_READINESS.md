...
3. **Timer scheduler Gate 3 — pending until the already-configured natural run.** Deployed health at `2026-08-02T09:55:18Z` showed `runOnStartup=false`, `totalRuns=0`, `lastRunStatus=idle`, and next run `2026-08-03T03:00:00Z`. No manual trigger was used. The deployed scheduler now logs exact processed/skipped/failed campaign and KPI IDs, but current evidence cannot prove a timer run that has not occurred.
4. **Provider Gate 4 — partially passed and pending for two inaccessible campaigns.** Refresh-disabled, non-persisting provider success and simulated-failure checks passed for KPI hashes `7ed1d5c4d10f` and `ff24c717b842`: exact property/filter/Europe-Amsterdam scope, campaign-to-date provider window `2026-06-24..2026-08-01`, imported-revenue and spend source IDs, and resolved Revenue `$51,405.93` matched stored values; failure returned no provider totals and attempted no token refresh. KPI hashes `2f27f064cd32` and `befd1e90d209` remain inaccessible because their campaign owners have no active authenticated session. No token or connection was changed.
5. **Report Gate 5 — pending because no qualifying existing fixture exists.** Read-only inventory found configured-recipient scheduled GA4 reports, but none selects a KPI section or KPI IDs. The only existing GA4 custom report with `sections.kpis=true` has no recipients and no schedule. Therefore direct snapshot/test-send/manual-send/scheduled-send parity against the same selected KPI IDs and confirmed delivery cannot be proven without changing configuration, which this task forbids. No report was generated or sent merely to produce irrelevant evidence.
