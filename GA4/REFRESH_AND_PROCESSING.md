...
- GA4 Overview is not production-ready or clean-certified. Current Commits 1-7 are closed for their documented bounded packets, including the exact `Summer splash` Manual Spend cleanup and post-delete inventory on `2026-07-30`. Whole-Overview Current Commit 8 is deployed; its 30-day coverage/activity and UI-stability checks passed, while live 90-day and current timer-fired evidence remain unproven. Current Commit 9's read-only inventory found redundant LinkedIn/Meta scheduler writes into orphan generic spend rows; commit `57036ebc` deployed the forward-only fix. The immediate post-deploy read-only inventory found no growth from the final pre-deploy 325,538-row boundary. First four-hour scheduled-cycle proof remains required, and existing rows remain unchanged pending separate cleanup authorization. Current Commit 10 deployed as `ec265895` and is closed for its bounded code/browser packet after `GA4 single` / `ga4_mock` Total Spend and Total Revenue matched the corresponding GA4 Overview values on the actual existing Campaign DeepDive screens. The fix keeps existing 90-day engagement/platform windows, aligns campaign-to-date GA4 financial values, restricts persisted financial/Trend rows to GA4 context, preserves valid-zero/negative ROAS/ROI, and separates corrected snapshots as `performance_summary_aggregate_v2`. Scheduled artifact and historical/multi-source evidence remain open. See `GA4/OVERVIEW_PRODUCTION_READINESS.md`.

Whole-Overview Commit 15's bounded deployed parser/parity packet closed at `e0f8baf2`: runner `2026-07-31.12` passed all 14 endpoints without reauthorization and proved Revenue `16700 = 16700` and Spend `2698.75 = 2698.75`. Commit 16 is local: authenticated connection responses add sanitized saved-window metadata, the connection-status response adds non-secret token-state metadata, the existing Overview uses the persisted 30/60/90-day value, and runner `2026-07-31.13` detects mismatches and directly observed expiry advancement. No scheduler cadence, provider query, formula, storage/schema, campaign scope, or production data changes. Deployment, live window variants, timer-fired evidence, automatic-renewal observation, and post-publish seven-day durability remain unproven.
