...
| --- | --- | --- | --- |
| Sessions | GA4 aggregate totals from the same selected-property/campaign-filter report used by the configured-lookback breakdown | selected connection's 30/60/90 completed-day lookback | Commit 18 follow-up removes the persisted-daily Summary preference; deployed provider parity remains open. |
| Users | GA4 aggregate `totalUsers` for the full selected property/filter/window; daily or dimension-row users are not summed | configured completed-day lookback | Commit 18 follow-up corrects the non-additive daily-user boundary locally; deployed provider parity remains open. |
...
### B3. Users provenance and aggregation are misleading — Current Commit 18 follow-up implemented locally

Current Commit 2 corrected the copy but preserved an underlying aggregation defect: browser and scheduled Summary totals preferred sums of persisted daily rows, and the dimensioned acquisition response rebuilt totals by summing detail rows. `totalUsers` is non-additive across dates and dimensions, so that contract could overcount users and change when completed-day rows were refreshed. The Current Commit 18 follow-up requests GA4's aggregate `TOTAL` row in the existing property/window/filter report and uses that one aggregate object for all Summary cards. Daily facts remain limited to charts/history and the existing ordered financial fallback. Deployment and a same-campaign provider/UI comparison remain required.
...
- Post-deploy Summary discrepancy follow-up root cause (2026-08-01): the browser and scheduled Overview preferred persisted daily sums for Summary, while the GA4 property response was dimensioned and its totals were reconstructed by summing rows. That is invalid for non-additive `totalUsers` and allows completed-day revisions/refresh timing to change visible values independently from GA4's aggregate property total. Commit `aa9050e7` did not change successful Summary values; it exposed this pre-existing source-contract defect during validation.
- Follow-up smallest safe boundary: request GA4 `TOTAL` aggregation in the existing acquisition query, use its same-window/property/filter aggregate totals for browser and scheduled Summary, and retain persisted daily rows only for history/charts and the existing ordered financial fallback. No property, filter, saved window, provider metric, financial formula, API field, source record, scheduler cadence, or production data is changed.
- Follow-up local evidence: the regression-first packet failed at the aggregate-users, browser-source, and tooltip boundaries before the runtime edit, then passed 52/52. Six adjacent financial/report/downstream files passed 46/46. TypeScript and the production build passed. Two pre-existing stale guards outside this diff remain red: one expects the older reporting-timezone import string, and one omits the already-required `ga4` argument from a Spend range assertion.
- Follow-up status: the bounded implementation is **proven locally**. It remains **unproven in deployment** and requires one same-campaign GA4 aggregate/UI comparison before the discrepancy is closed. The exact aggregate can legitimately differ from the former displayed value because the former value summed non-additive rows.
