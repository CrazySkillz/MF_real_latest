...
- Certification integrity rule: GA4 readiness is valid only for the exact certified SHA/configuration/dependency boundary. A relevant upstream or downstream change automatically invalidates affected tabs. The GA4 KPI record is `GA4/certifications/ga4-kpis.json` and its checker is `npm run check:ga4-kpi-certification`. Current Commit 1 implements the fail-closed gate and Current Commit 2 adds a bounded dynamic nine-metric cross-consumer guard while the record remains `UNVERIFIED`; functional fixes and external validation remain open. Copied formulas and source-text guards are structural evidence only.
...
- GA4 KPI whole-tab status is not production-ready and remains `UNVERIFIED`; `GA4/KPIS_PRODUCTION_READINESS.md` is controlling. Current Commit 3 locally normalizes the supported standard and legacy metric inventory across the exercised consumers, but Current Commits 4-10 and all listed external gates remain open.
