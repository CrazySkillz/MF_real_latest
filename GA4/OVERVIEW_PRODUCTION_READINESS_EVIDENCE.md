...
The numerical observations below are retained as historical evidence, but their boundary terminology is updated to avoid presenting retired behavior as current. They must not be used as current-value certification after the V1 boundary change: native GA4 financial values start at the saved initial-import date, imported Revenue/Spend include every mapped record, and campaign metadata dates are not financial boundaries.
...
- all three cumulative Campaign Breakdown rows reconcile exactly to Summary traffic/conversions before exact campaign-matched all-mapped-record imported revenue is added
- Landing Pages fails closed without relabeling `pageLocation`, and Conversion Events excludes zero-conversion traffic
- the exact-current focused Overview/downstream packet passed 34 files / 433 tests; the full current-version suite had zero blocking failures
- TypeScript, production build, authenticated ownership denial, production-data integrity, and parsed browser-PDF parity pass
- the deployed scheduler is started and scheduled for `22:00 UTC`; GA4 daily and automatic-refresh startup runs are disabled

The exact-current natural timer observation remains open. This is not production-ready status.
...
| Visible value or state | Source and transform under the current contract | Window/scope | Historical evidence status |
| --- | --- | --- | --- |
| Sessions | Sum of persisted GA4 daily rows for the exact selected property and saved campaign filter | fixed initial 30 completed days plus later completed-day accumulation | Persisted/API/UI parity passed at 1,505 Sessions over `2026-07-02` through `2026-09-06`; dates are unique and production rows are not simulated. The exact-current natural timer remains pending. |
| Users | Sum of persisted daily `totalUsers` rows for the exact selected property/filter | same fixed initial boundary plus accumulation | Current authenticated result is 1,507. This is explicitly a sum of daily users, not a cross-day deduplicated user count. |
| Conversions | Same coherent persisted Summary source object | same fixed initial boundary plus accumulation | Current authenticated result is 193; Campaign Breakdown and Conversion Events reconcile exactly. Missing data remains unavailable rather than zero. |
| Engagement Rate | engaged sessions divided by sessions from the selected Summary source | same persisted Summary boundary | Current exact result is `1,027 / 1,505 = 68.2392%` (rendered `68.2%`). |
| Conversion Rate | Summary conversions divided by Summary sessions | same persisted Summary boundary | Current exact result is `193 / 1,505 = 12.8239%` (rendered `12.8%`). |
| GA4 native financial revenue | ordered complete-source selection: saved-import-window provider, compatible persisted daily values where available to the caller, then configured-lookback breakdown only when earlier candidates are absent | saved initial-import date through the latest completed reporting day | Historical runtime value was `$65,362.20 USD` under a now-retired metadata-derived boundary. It is not current-boundary evidence. |
| Financial conversions | Taken from the same object selected for native financial revenue | same ordered financial candidate | Historical runtime value was 292 under the same retired boundary. It is not current-boundary evidence. |
...
| Total Revenue | selected saved-import-window GA4 native revenue + all available mapped imported revenue | native saved import window plus all mapped imported records | Historical runtime result was `$82,162.19 USD`; because its native component used the retired boundary, it is not a current Total Revenue certification. |
...
| Total Spend | scoped materialized spend breakdown; scoped all-mapped-record source total only when breakdown is absent | active `ga4` plus reviewed legacy null-context spend sources for the selected campaign | The four configured sources reconciled to the current `$2,699.75` Spend Sources modal and were explicitly retained; valid zero does not fall through to cached campaign spend. Foreign contexts are excluded. |
| Profit | Total Revenue - Total Spend | same available financial inputs | Historical runtime result was `$79,462.44`; the formula remains current, but the old amount is not current-boundary evidence. |
| ROAS | Total Revenue / Total Spend | same available financial inputs | Historical runtime result was `30.43x`. The formula and zero handling remain current; the old amount is not current-boundary evidence. |
| ROI | (Total Revenue - Total Spend) / Total Spend | same available financial inputs | Historical runtime result was `2943.3%`. The formula and zero handling remain current; the old amount is not current-boundary evidence. |
| CPA | Total Spend / financial conversions | spend plus conversions from the selected native financial candidate | Historical runtime result was `$9.25`; the formula remains current, but the old amount is not current-boundary evidence. |
...
| Campaign Breakdown | Fixed saved-import-window traffic and native GA4 Revenue rows, followed by exact mapped imported campaign revenue | observed traffic/native window: `2026-07-02` through `2026-09-06`; all exact mapped imports included | Deployed runtime `08d7abe5` passed authenticated API/UI/browser-PDF parity. Rows are `yesop_paid_social`: 626 Sessions, 627 Users, 57 Conversions, `$20,315.40` native + `$99.99` imported = `$20,415.39`; `yesop_retargeting`: 471, 473, 79, `$26,935.20` + `$16,100.00` = `$43,035.20`; `yesop_email_nurture`: 408, 407, 57, `$18,111.60` + `$600.00` = `$18,711.60`. Totals are 1,505 Sessions, 1,507 Users, 193 Conversions, `$65,362.20` native revenue, `$16,799.99` exact mapped imports, and `$82,162.19` displayed revenue. |
| Landing Pages | GA4 session-scoped `landingPagePlusQueryString` rows only | selected property/filter and fixed initial-import-to-latest-completed-day window; API limit 50, UI renders 20 | Deployed provider response exposed no session-scoped landing attribution, so the route returned zero rows with `sessionScopedAttributionAvailable=false` and the UI rendered the explicit unavailable state. `pageLocation` was not relabeled. |
| Conversion Events | GA4 rows with nonzero conversions only | selected property/filter and fixed initial-import-to-latest-completed-day window; API limit 50, UI renders 25 | Deployed API/UI/PDF parity passed with only `purchase`: 193 Conversions, 193 events, and 193 Users. Zero-conversion `page_view` traffic was excluded. |
...
| GA4 daily | `/ga4-daily` -> reporting-timezone window -> stored rows -> due-day provider backfill -> upsert -> response freshness | Access/property scope and persisted UI/API parity pass; the exact-current natural timer observation remains pending. |
| GA4 native financial aggregate | `/ga4-to-date` -> selected connection -> saved GA4 initial-import date through the latest completed reporting day -> live provider | Used for the connected-source financial contract, not as a configured-lookback Summary fallback. |
| Breakdown | `/ga4-breakdown?window=import-to-date` -> saved `importStartDate`/reporting-timezone end -> `getAcquisitionBreakdown` -> client aggregation/render | Selected property/filter, cumulative window, Summary totals, exact imported allocations, rendered rows, and PDF parity pass in production. |
| Landing Pages | `/ga4-landing-pages?window=import-to-date` -> saved `importStartDate`/reporting-timezone end -> session-scoped provider report -> client first 20 rows | Exact cumulative bounds pass; current provider attribution is unavailable and the deployed UI fails closed with zero rows and explicit copy. |
| Conversion Events | `/ga4-conversion-events?window=import-to-date` -> saved `importStartDate`/reporting-timezone end -> nonzero-conversion provider rows -> client first 25 rows | Exact cumulative bounds and API/UI/PDF parity pass with only `purchase` at 193 Conversions. |
...
| Ad Comparison | GA4-native campaign rows from the fixed initial-import boundary through the latest completed day, plus separate all-mapped-record imported provenance | live client props and guarded GA4/revenue APIs | Separately revision-certified; `GA4/AD_COMPARISON_PRODUCTION_READINESS.md` exclusively controls its current exact SHA and status. Reports-owned output is outside the tab boundary. |
...
The root cause was a page-level hard-coded `90days` request combined with connection-specific daily lookback and a maximum-revenue selector spanning native provider, daily, and breakdown candidates. Current Commit 2 originally derived every Overview live-table request from the selected connection's validated 30/60/90-day setting, excluded intraday `today` from Landing Pages and Conversion Events, separated Summary/table and connected-source financial boundaries, and replaced maximum selection with a fixed complete-source order. Commit 19 later narrowed the supported production contract to 30 days only. Browser and scheduled report builders use that 30-day contract. Commit `5cff21ad` deployed and the bounded UI smoke passed for one configured campaign/window; the later Current Commit 8 and 19 packets passed deployed 30-day evidence. Future 60/90-day options are excluded.

### B1a. Overview detail tables incorrectly remained rolling — fixed and deployed

The initial 30-day setting is an import seed, not a permanent display window. Campaign Breakdown, Landing Pages, and Conversion Events had continued recalculating their start date from a rolling lookback while Summary already used the fixed saved `importStartDate`. Deployed runtime `340da6d5` routes those three browser and report table queries from the saved initial-import boundary through the latest completed campaign-reporting-timezone day. Authenticated production API/UI/PDF parity and the exact `2026-07-02` through `2026-09-06` bounds pass; the 34-file / 433-test packet, TypeScript, and production build pass.

### B1b. Campaign Breakdown revenue used the traffic window — fixed and deployed

Runtime `08d7abe535c9ea4f57f57d8cbdbb072fba64052e` recorded Campaign Breakdown traffic and native Revenue without allocating or scaling values. The observed range was `2026-07-02` through `2026-09-06`; the native row sum reconciled to `$65,362.20`; exact saved mappings added `$16,799.99`; and the displayed rows reconciled to `$82,162.19`. The authenticated read-only audit confirmed the API, rendered table, and downloaded browser PDF, plus unauthenticated/cross-owner denial and zero orphan or duplicate external-revenue keys. This older runtime record does not define the current boundary; current native Revenue always starts at the saved GA4 initial-import date.
...
| Incompatible windows | reject, normalize, or clearly label | Commit 2 fixed completed-day Summary/tables and separated connected-source financial values; Commit 19 now rejects unsupported missing/60/90-day writes and fails closed on retained non-30 rows. The deployed 30-day smoke passed. |
