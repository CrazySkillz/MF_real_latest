...
| Current Commit 2a: Google Sheets revenue refresh validation trigger | Implemented locally; deployed run evidence still pending. | One active Google Sheets revenue source in one authorized campaign. | Add a campaign/source-scoped validation route that reuses the scheduler Google Sheets revenue reprocess function for `/api/campaigns/:id/revenue-sources/:sourceId/google-sheets-refresh/run-now`; do not run the full daily scheduler, unrelated providers, reports, emails, alerts, or unrelated campaigns. | No for local code-scope certification; yes for deployed Google Sheets revenue refresh/reprocess certification until the route is deployed, run, and endpoint/UI evidence is recorded. |
...
- Remaining gate for this Google Sheets revenue refresh/reprocess action: visible Overview UI confirmation after refresh/reprocess was not included in the endpoint packet, so final UI parity remains pending until the user confirms Total Revenue and Revenue Sources modal/list display the refreshed `$84,500` Google Sheets source, no duplicate Google Sheets source, and preserved `$600` CSV revenue source.
- Certification boundary: this deployed run-now evidence does not prove the daily timer fired by itself, and it does not certify Google Sheets revenue delete/deactivate, other campaigns/properties, other Google Sheets tabs/mappings, scheduler timer execution, or report/email propagation.
...
| --- | --- | --- | --- | --- | --- | --- |
| Google Sheets revenue | `Total Revenue`, revenue sources modal, Profit, ROAS, ROI, CPA, report payload values after the change | Closed for deployed campaign/source add/import of `dd5dc470-814d-42b9-af19-4b53ac7d08f8` after endpoint plus UI evidence (`$30,300`) | Closed for deployed campaign/source edit/update of `dd5dc470-814d-42b9-af19-4b53ac7d08f8` after endpoint plus UI evidence (`$54,200`) | Endpoint-proven for deployed campaign/source run-now reprocess of `dd5dc470-814d-42b9-af19-4b53ac7d08f8` after corrected source reconciliation (`$84,500`); visible UI parity pending | Required | Partially proven for Google Sheets revenue add/import, edit/update, and refresh/reprocess endpoint identity only on this campaign/source. Refresh/reprocess UI parity and delete/deactivate remain open. |
...
1. Next source family/action: finish Google Sheets revenue refresh/reprocess by capturing visible Overview UI parity for source `dd5dc470-814d-42b9-af19-4b53ac7d08f8` and refreshed amount `$84,500`. Do not proceed to Google Sheets revenue delete/deactivate until the UI parity gate is recorded.
2. Confirm visible Overview UI parity: Total Revenue, Revenue Sources modal/list, and any spend-dependent derived-card behavior must match the traced endpoint totals.
3. Record the UI evidence in this file before moving to Google Sheets revenue delete/deactivate or any Google Sheets spend lifecycle action.
4. If any action changes an unrelated source ID, duplicates an active source, loses provenance, changes totals unexpectedly, or broadens source scope, stop Current Commit 2 and add the exact runtime fix as the next Current Commit.
