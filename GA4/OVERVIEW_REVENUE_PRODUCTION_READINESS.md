...
Current controlling decision (2026-08-08): **Google Sheets Revenue and Upload CSV Revenue are UNVERIFIED on the current revision.** Historical CSV and Sheets packets remain bounded history only. The current local forward-path fix rejects caller-supplied GA4 revenue currency that differs from campaign currency and persists the normalized campaign currency, but it is not deployed and existing persisted rows have not been inventoried under the expanded currency boundary.
...
- **Google Sheets Revenue is UNVERIFIED.** Foreground GA4 add/edit and scheduler refresh remain campaign/source-scoped and transactional; the current revision adds campaign-currency parity, but deployment, provider-failure, idempotency, persisted-row inventory, and full downstream evidence are not current.
- **Upload CSV Revenue is UNVERIFIED.** The historical Current Commit 12 certification is invalidated by the current CSV import-boundary change. The current revision rejects a supplied currency that differs from the campaign and uses the normalized campaign currency; no existing row was rewritten.
...
- Google Sheets foreground add/edit performs source create/update, old-record deletion, and new-record insertion as separate operations. Whole-Overview Commit 17 changes only the scheduler helper to update source freshness and replace records in the existing exact transaction.
- In Google Sheets Revenue foreground add/edit and other still-non-transactional paths, a failure after old-record deletion can remove the last valid stored source value. The scheduler path and GA4 CSV now have local rollback protection; deployed PostgreSQL/provider-cycle rollback remains unproven.
...
- GA4 CSV Revenue add/edit source and record replacement is transactional. Google Sheets foreground add/edit remains non-transactional; whole-Overview Commit 17 makes its daily scheduler replacement transactional locally.
...
Whole-Overview Current Commit 5 (`5da5f41c`) historically hid new GA4 Google Sheets Revenue setup. Current Commit 21 supersedes that temporary policy and restores the chooser/API path without changing saved production data. Deployment visibility and the Google Sheets gates below remain required evidence.
...
Current Commit 12 completed the CSV-only rerun, and the later user-confirmed invalid-file no-mutation result closes the applicable Current Commit 10 deployed UI gate for the bounded CSV claim. Google Sheets Current Commit 11 is resumed by whole-Overview Current Commit 21 and remains open.
