...
**Current section status: UNVERIFIED.** Campaign Breakdown is validated for deployed runtime `08d7abe535c9ea4f57f57d8cbdbb072fba64052e` and the recorded property/configuration. Its authenticated production API, rendered UI, and downloaded browser PDF preserve fixed import-to-date traffic, reconcile exact campaign-to-date native revenue to the GA4 Revenue card, and add only exact campaign-mapped imported revenue. The overall Overview status remains fail-closed while Landing Pages, Conversion Events, Revenue, Spend, and the natural scheduler gate retain their separate validation status.
...
| Remaining Overview revalidation | **Pending** | Landing Pages, Conversion Events, Revenue, Spend, and the natural `22:00 UTC` scheduler run retain separate validation gates; Campaign Breakdown evidence does not certify them. |
