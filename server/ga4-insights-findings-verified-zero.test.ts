...
describe("GA4 Insights findings use scheduler-synchronized campaign daily history", () => {
  it("uses the chart's campaign-start zero-filled rows without scheduler-freshness gates", () => {
    ...
    expect(findingsRows).toBe("trendsDailyRows");
    ...
    expect(findingMemo).not.toContain("trendsRefreshIsStale");
    expect(findingMemo).not.toContain("findingsHistoryMismatch");
    ...
    const ready = new Function("findingRollups", `return { seven: ${sevenDayReady}, three: ${threeDayReady} };`) as
      (rows: ReturnType<typeof buildGA4InsightsRollups>) => { seven: boolean; three: boolean };
    expect(ready(sparse)).toEqual({ seven: false, three: true });
    expect(ready(complete)).toEqual({ seven: true, three: true }); // The 7-day branch takes precedence.
    const validZero = buildGA4InsightsRollups(dates.map(date => row(date, 0)), "2026-09-17");
    expect(ready(validZero).seven).toBe(true);

    const historyBeforeCreation = dates.filter(date => date >= "2026-09-08").map(date => row(date, 10));
    expect(ready(buildGA4InsightsRollups(historyBeforeCreation, "2026-09-17"))).toEqual({ seven: false, three: true });
    ...
    expect(ready(imported)).toEqual({ seven: false, three: true });
    ...
    expect(findingMemo).not.toContain('id: "integrity:daily_history_outdated"');
