...
- a later local KPI-create latency correction removes only duplicated work: the create route reuses a successful KPI alert sweep already completed by the authoritative campaign recompute, retains the original synchronous fallback when that sweep was not attempted or failed, preserves Benchmark reconciliation and immediate email ordering, and runs independent KPI/Notifications browser refetches concurrently. The focused affected packet passed 7 files / 90 tests; the current-version suite executed 1,375 tests with 1,346 passing, exactly 29 visible deferred failures, and zero blocking failures; TypeScript, certification regression 9/9, and the production build passed. This runtime correction is not deployed and therefore does not restore certification.
