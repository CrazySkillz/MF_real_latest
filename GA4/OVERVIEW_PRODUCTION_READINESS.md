...
9. Current Commit 8: Shopify portability and provider-boundary variants. Local root cause/evidence gap confirmed: all deployed Shopify evidence so far is for one Admin API token campaign, and the validated store has fewer than 250 matching orders, so other campaigns/mappings and the real >250-order provider boundary remain externally unproven. No runtime bug was found in the route trace. Smallest safe local action: add regression guards to `server/shopify-revenue-regression.test.ts` proving every Shopify order-reading path uses the paginated reader and the save route remains campaign-access guarded, campaign-connection scoped, source-ID scoped, platform-context scoped, exact mapping scoped, record-source scoped, and campaign recompute scoped. Local validation passed: `npm test -- server/shopify-revenue-regression.test.ts` (`21` tests). Deployed second-campaign/mapping evidence, real >250 matching-order provider evidence, OAuth, revenue-changing provider mutation, normal wall-clock scheduler timing, future provider changes, and other mappings remain unproven.
...
- Current Commit 8 is local-code validation only: Shopify order reads are guarded to use the paginated reader across order endpoints, and Shopify save portability is guarded for campaign access, campaign connection, source ID, platform context, exact mapping, revenue records, and recompute scope. Deployed second-campaign/mapping evidence and real >250 matching-order provider evidence remain unproven until their own Shopify Admin API token packets are captured.
...
| Shopify revenue | `Total Revenue`, revenue sources modal, Profit, ROAS, ROI, CPA, report payload values after the change | Closed only for deployed Admin API token normal save/materialization on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458` after UI validation plus read-only endpoint packet: Shopify `$99.99` and total revenue `$16,799.99` matched source/breakdown endpoints with clean inventory | Closed only for deployed Admin API token UI edit/rematerialization on the same campaign after browser packet `overallPass: true`: one Shopify source kept the same ID, Shopify revenue changed from `$99.99` to `$199.98`, source/breakdown totals matched, and inventory stayed clean; the shortened packet did not separately archive every unrelated source row | No user-facing Shopify run-now route/UI action found; startup-fired daily auto-refresh scheduler path is closed only for the validated Admin API token campaign after Render startup logs and before/after browser packet proved same source ID, no duplicate, Shopify `$99.99`, Total Revenue `$16,799.99`, spend `$498.75`, zero deltas, and clean inventory | Closed only for deployed Admin API token UI delete on the validated campaign after browser packet `overallPass: true`: Shopify source count `1 -> 0`, Shopify revenue `$199.98 -> $0`, Total Revenue `$16,899.98 -> $16,700`, spend unchanged at `$498.75`, non-Shopify revenue source totals unchanged, and inventory clean | Partially closed only for the validated Admin API token add/import, edit/update, startup-fired scheduler refresh/reprocess, delete/deactivate, local Current Commit 6 source-backed consumer/cache-freshness guards, deployed Current Commit 6 cache/refetch packet, local Current Commit 6a downstream value/content regression coverage, deployed Current Commit 6a value/content plus delivered report-email packet, and local Current Commit 8 portability/provider-boundary route guards. OAuth, real >250-order provider pagination, revenue-changing provider mutation, normal wall-clock scheduler timing, deployed second-campaign/mapping evidence, other mappings, future provider changes, future report/email sends, other report variants, and full unrelated-source row immutability remain unproven. |
...
- Shopify is partially closed only for the validated Admin API token campaign lifecycle packets now recorded above, including startup-fired scheduler execution with no source/total drift, local Current Commit 6 downstream source-backed consumer/cache-freshness guards, the deployed Current Commit 6 cache/refetch packet, local Current Commit 6a downstream value/content regression coverage, the deployed Current Commit 6a value/content plus delivered report-email packet, and local Current Commit 8 portability/provider-boundary route guards. Shopify OAuth, real >250-order provider pagination, revenue-changing provider mutation, normal wall-clock scheduler timing, deployed second-campaign/mapping evidence, other mappings, future report/email sends, other report variants, and future provider changes remain unproven. Salesforce, legacy Manual revenue, LinkedIn Ads, Meta Ads, Google Ads, and legacy Manual spend remain provider/user validation work until each relevant family has before/after evidence. HubSpot is clean-certified only for the exact proven GA4 Overview `Total Revenue only` scope on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`, including the controlled 4.8b scheduler/provider propagation packet. HubSpot Pipeline Proxy is closed only for the recorded deployed 4.9b selected-source packet and the recorded deployed 4.10b proxy-to-confirmed transition packet on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458` / source `d4ad51ef-85fe-4b67-bbd5-854900be3dee`; HubSpot Review Settings campaign-mapping visibility is locally regression-covered in 4.10c, HubSpot Revenue Sources mapped-campaign subtitles plus zero-proxy edit Review Settings display are locally regression-covered in 4.10d, and the read-only 4.11 Campaign Breakdown exact mapped-row checker is locally implemented/regression-covered with deployed evidence closed only for target row `yesop_retargeting` changing by `+$100` while named row `yesop_email_nurture` stayed unchanged; deployed UI confirmation for the display-only paths remains pending; Reports are closed only for the configured 4.12 packet, KPI/Benchmark propagation is closed only for the configured 4.13 Revenue KPI/Benchmark packet, and HubSpot report email/PDF evidence is closed only for the user-confirmed configured 4.14 GA4 Overview Report packet; Current Commit 4.15 deployed other-campaign portability evidence is closed only for the supplied two-campaign packet; Current Commit 4.16 local alternate-mapping automation is implemented/regression-covered but deployed variant packets are pending; alternate mappings beyond the recorded `yesop_brand_search` and `yesop_retargeting` packets, other Campaign Breakdown rows, other KPI/Benchmark metrics, future sends/report variants, future provider changes, and other HubSpot configurations remain unproven.
