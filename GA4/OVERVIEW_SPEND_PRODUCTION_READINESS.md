...
- On `2026-07-29`, read-only target metadata confirmed the affected GA4 connection for campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458` was created before the publishing-status change and therefore still requires one post-publish reconnect. The current change set prevents GA4 permission, transient refresh, server/storage, and post-refresh provider failures from being mislabeled as `Reconnect required`; Google `invalid_grant` remains a legitimate reconnect signal. Render deployment, final consent, automatic renewal, and more-than-seven-day proof remain pending.
...
| Current Commit 8: Durable GA4 OAuth reconnect | **Durable replacement code deployed; reconnect classifier included in the current change set; production-mode reconnect still pending.** | Require a newly issued GA4 refresh token, preserve existing GA4 rows until a durable replacement is created and promoted, and show reconnect only for a missing or Google-rejected durable credential. | Deploy the classifier hardening, reconnect GA4 once under the production configuration, then prove automatic token refresh without another consent prompt. | Yes for GA4-derived downstream spend-consumer validation; does not block source-backed Total Spend itself. |
...
| Current Commit 8c: Production Google OAuth publishing and verification | **Publishing changed; the `2026-07-29` incident proves the pre-publish GA4 connection remains in use and the final reconnect is still required.** | Eliminate Testing-mode seven-day token expiry for Google Sheets and GA4. | Deploy exact error classification, complete one post-publish GA4 reconnect, then capture automatic renewal and >7-day durability. Public-page/domain and broader Google verification work remains deferred unless separately resumed. | Yes for stable-connection clean certification. |
...
- Current Commit 12 deployed UI evidence confirms the v1 new-source Spend chooser shows Google Ads, Google Sheets, and Upload CSV and hides LinkedIn Ads and Meta / Facebook.
...
- The optional no-date snapshot import remains unproven and is explicitly deferred for later; it does not block the bounded dated-CSV claim.
...
GA4 Overview Google Sheets spend is not yet clean-certified. Google Sheets OAuth and automatic-update evidence remain on hold and will be completed later when the user resumes that work. Upload CSV has a complete bounded deployed dated-Alpha lifecycle packet: exact invalid rejection, add, delete restoration, same-source edit to $1,250, Beta exclusion, source-count stability, and Total Spend propagation. Optional no-date snapshot UI behavior, forged-request deployment behavior, unusual/unlisted CSV shapes, and additional campaign/source inventories remain unproven and are deferred for later; they do not block the existing bounded dated-CSV claim. Current Commit 12 deployed UI evidence closes the v1 new-source chooser scope only. Google Ads and all other source families remain excluded.
