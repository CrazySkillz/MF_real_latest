...
13. Current Commit 4.8b: deployed HubSpot scheduler/provider propagation proof. Change exactly one controlled HubSpot deal value/date in the connected account, let the existing scheduler/provider path complete through an approved operational path, then run `hubspotPropagationAfter(...)`. Required pass criteria: same active HubSpot source ID, same active revenue/spend source IDs, no duplicate HubSpot source, expected HubSpot source revenue delta or total, overall revenue delta equals HubSpot source delta, spend unchanged, GA4 context preserved, inventory/provenance pass, and Pipeline Proxy remains outside the evidence unless explicitly selected.
...
17. Current Commit 4.12: HubSpot Reports value propagation. Add local report payload/snapshot tests and deployed report API/PDF evidence only for Overview values sourced from HubSpot. This proves report value propagation, not provider delivery, report scheduler behavior, or unrelated report variants.
18. Current Commit 4.13: HubSpot KPI/Benchmark value propagation. Add local tests where HubSpot revenue changes the Overview-originated inputs used by ROI, ROAS, CPA, KPI, and Benchmark paths. This proves narrow Overview value propagation only; it does not certify whole KPI or Benchmark readiness.
19. Current Commit 4.14: HubSpot email evidence. Add local mocked-email assertions for HubSpot-backed report payload attachments. Deployed proof requires provider acceptance plus actual inbox or delivery-event evidence; provider API acceptance alone is not delivery proof.
...
| HubSpot revenue or Pipeline Proxy | Confirmed HubSpot Total Revenue path only for the tested `Total Revenue only (no Pipeline card)` flow; Pipeline Proxy remains early-signal only and was not selected or certified | Endpoint/data-health evidence captured for add/import on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`: active HubSpot source `1`, records `2`, revenue `$10,600`, no findings | Endpoint/data-health evidence captured for edit/update: active HubSpot source stayed `1`, records `4`, revenue `$16,600`, no findings | Not applicable as a deployed user-facing source action: UI exposes add/edit/save/delete only, source-scoped run-now routes are Google Sheets-only, and HubSpot reprocess is scheduler-only. Optional scheduler/provider propagation remains separate. | Endpoint/data-health evidence captured for delete/deactivate: active HubSpot source `0`, records `0`, revenue returned to `$600`, no findings | Clean-certified only for this campaign's confirmed GA4 Overview `Total Revenue only` interactive revenue path: add/import, edit/update, delete/deactivate, before/after inventory, deployed provenance capture for active source `65867434-cbed-4792-9496-8072f63a9c82`, and no user-facing refresh/reprocess applicability. Pipeline Proxy, other campaigns, scheduler/provider propagation, Reports, KPI/Benchmark propagation, emails, provider mutation propagation, and other HubSpot configurations remain unproven. |
...
- Shopify, Salesforce, legacy Manual revenue, LinkedIn Ads, Meta Ads, Google Ads, and legacy Manual spend remain provider/user validation work until each relevant family has before/after evidence. HubSpot is clean-certified only for the exact proven GA4 Overview interactive `Total Revenue only` scope on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`; Pipeline Proxy, other campaigns, alternate mappings, scheduler/provider propagation, Reports, KPI/Benchmark propagation, emails, provider mutation propagation, and other HubSpot configurations remain unproven. Current Commit 4.8a adds read-only automation for scheduler/provider propagation packets, but it is not deployed provider evidence until 4.8b passes.
