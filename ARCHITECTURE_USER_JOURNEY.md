...
- Future Connected Platform integrations must feed Executive Summary through the same `performanceSummary` aggregate source contract used by `/outcome-totals`. Any new main source must be wired into `/executive-summary`, `/outcome-totals`, scheduler snapshots, source freshness, KPI/Benchmark metric mapping, risk inputs, and Strategic Recommendation eligibility before it can be considered production-ready in Executive Summary.
