...
Budget & Financial Analysis is a downstream consumer of platform-level connected-source financial records. Platform `Total Revenue` and `Total Spend` remain the source of truth and are not changed by budget controls. Budget Position, utilization, burn rate, pacing status, and budget guidance use a separate verified Spend derivative containing only dated Spend records inside `pacingStartDate` through `pacingEndDate`; this period filter must never alter source records, aggregate Total Spend, ROI, ROAS, profit, CPA, or source breakdowns.
