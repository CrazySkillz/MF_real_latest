...
- HubSpot-backed GA4 KPI/Benchmark financial values should use the same source-backed financial total as Overview (`GA4 native financial revenue + imported revenue`, Pipeline Proxy excluded); Current Commit 4.13 records local guards and a read-only deployed runner, with deployed evidence still pending
