...
- GA4 Overview is not production-ready or clean-certified. Current Commits 1-5 are closed for their bounded packets. Current Commit 5 (`5da5f41c`) deployed, and the user confirmed Google Sheets is absent from both new GA4 Revenue and Spend source choosers. Current Commit 6 has one successful campaign-scoped deployed inventory/UI packet, but all-campaign closure still requires the locally added owner-scoped read-only batch endpoint to deploy and pass. The durable source of truth is `GA4/OVERVIEW_PRODUCTION_READINESS.md`.
