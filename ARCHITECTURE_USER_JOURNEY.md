...
- Executive Summary Risk Assessment visible checked-input values and KPI/Benchmark risk counts must use the same page-level `performanceSummary` aggregate as the visible metric cards, KPI Progress, and Benchmark Comparison so the risk explanation cannot drift from the numbers executives see.
- Executive Summary should hide non-actionable Risk Assessment boundary rows from the visible checked-input list; for example, paid-platform concentration should not show for GA4-only campaigns, and budget pacing should remain in Budget & Financial Analysis rather than appearing as a separate-section card.
