...
- Post-`bf078b03` changes modified save-time alert processing, source-read reuse, immediate email breach-episode behavior, and email subject copy. Those changed paths are **UNVERIFIED** pending exact-current regression and deployed/provider validation.
- The earlier `ga4_mock`, Mailgun, and inbox evidence remains revision-bounded supporting history and does not close the current GA4 KPI provider or delivery gates.
