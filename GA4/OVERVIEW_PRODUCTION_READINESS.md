...
23. Current Commit 4.12: HubSpot Reports value propagation. Add local report payload/snapshot tests and deployed report API/PDF evidence only for Overview values sourced from HubSpot. This proves report value propagation, not provider delivery, report scheduler behavior, email delivery, or unrelated report variants.
...
Next active HubSpot step: Current Commit 4.12, HubSpot Reports value propagation. Sandbox provider mutation automation is optional future hardening only if sandbox HubSpot write credentials are available; without write access, keep provider mutation validation as manual HubSpot changes plus automated before/after runner packets.
