...
Whole-Overview Commits 11 and 12 are closed for their bounded deployed fail-closed/configured-value packets at `9ac3fea9` and `152a7dd3`. Commit 13 is closed at deployed `d353383e`: runner `2026-07-30.10` is served and the read-only configured-campaign comparison selected live provider revenue `$34,705.93` over the higher persisted-daily `$34,705.94`, proving ordered provider-first validation parity. Commit 14 regression/document alignment is implemented and locally validated with no runtime-code change; commit is pending. It does not change scheduler cadence, refresh behavior, provider calls, persistence, or token handling. Live 90-day, timer-fired, automatic-renewal, and more-than-seven-day OAuth evidence remain unproven.
