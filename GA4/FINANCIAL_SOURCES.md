...
- active GA4 HubSpot sources with Pipeline Proxy enabled are automatically reprocessed every five minutes by default through the saved mapping and stable revenue source ID; the shared CRM interval remains configurable from one to sixty minutes with `SALESFORCE_PIPELINE_REFRESH_INTERVAL_MINUTES`, and overlapping financial refreshes are skipped
...
- in the GA4 Overview `Revenue Sources` modal, the compact line under `HubSpot (Deals)` should show only the source type label `HubSpot`; saved platform campaign mappings remain attribution metadata and are not repeated there
