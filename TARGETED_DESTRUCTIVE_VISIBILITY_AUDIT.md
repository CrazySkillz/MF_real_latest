...
- Source edit/delete and normalized-record cleanup: started. Google Sheets campaign source read/update/helper routes are access-guarded and connection mutations verify campaign membership. Google Sheets GA4 revenue add/edit/refresh identity is hardened so add mode creates an additive source and edit/refresh mode updates only by `sourceId`. Other source families still require review.
...
- Source delete/edit flows: started. Google Sheets source route access, connection ownership checks, and GA4 revenue add/edit source identity are complete for the targeted pass; CSV, HubSpot, Salesforce, Shopify, LinkedIn, Meta, Google Ads, and Custom Integration still need the same route-by-route check.
