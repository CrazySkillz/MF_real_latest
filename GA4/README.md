...
- GA4 Overview `Campaign Breakdown` preserves fixed import-to-date Sessions, Users, and Conversions, while native Revenue uses an exact per-campaign campaign-start-to-latest-completed-day query that must reconcile to the GA4 Revenue card; it must not scale or allocate native revenue
