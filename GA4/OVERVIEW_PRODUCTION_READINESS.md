...
- Certification boundary: this code change does not close Google Sheets revenue refresh/reprocess until the route is deployed, run against source `dd5dc470-814d-42b9-af19-4b53ac7d08f8`, and followed by before/after endpoint evidence plus visible Overview UI parity. It also does not prove the daily timer fired by itself.
...
1. Next source family/action: Google Sheets revenue refresh/reprocess for source `dd5dc470-814d-42b9-af19-4b53ac7d08f8`, proving the same source ID updates in place from the connected sheet without creating a duplicate. Do not delete in the same evidence packet.
2. Capture an authenticated Current Commit 2 baseline snapshot for the deployed campaign and target Google Sheets revenue source before changing or reprocessing the sheet data.
3. Change only the connected disposable Google Sheet/tab to one known expected revenue amount, then, after Current Commit 2a is deployed, trigger `/api/campaigns/:id/revenue-sources/:sourceId/google-sheets-refresh/run-now` for the existing Google Sheets revenue source. This route is validation-only and does not prove the daily timer fired by itself.
4. Run the after snapshot and confirm the same Google Sheets source ID remains, no duplicate Google Sheets source appears, imported revenue changes to the expected refreshed amount, CSV revenue remains unchanged, spend remains unchanged, GA4 native totals remain available, and source-modal provenance reconciles to Total Revenue.
...
6. Record the evidence in this file before moving to Google Sheets revenue delete/deactivate or any Google Sheets spend lifecycle action.
