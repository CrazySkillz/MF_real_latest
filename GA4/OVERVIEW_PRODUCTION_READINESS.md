...
## Future Reference: Shopify Revenue Clean Certification

Status: Shopify Admin API token GA4 Overview revenue is production-ready and clean-certified under the strict no-overclaiming standard for the validated v1 source-family scope recorded in this file. This status is based on Shopify-specific code traces, regression guards, deployed UI/API packets, downstream value/content validation, delivered report-email evidence, and second-campaign/mapping portability evidence. It is not based on HubSpot, Google Sheets, CSV, KPI, Benchmark, Reports, or any other source-family evidence.
...
- this Shopify certification does not certify Google Ads spend, LinkedIn Ads spend, Meta spend, Salesforce, Manual/legacy, or any other source family

Stable future answer: Shopify Admin API token GA4 Overview revenue can be treated as closed and production-ready for the validated v1 scope above unless later code changes, provider/config changes, failed validation, contradictory deployed evidence, or changed requirements affect a Shopify revenue path. If a new Shopify bug is found, lower only the affected path to unproven until root cause, tests, deployed/provider evidence where needed, and documentation are updated.
...
9. Current Commit 8: Shopify portability and provider-boundary variants. Local root cause/evidence gap confirmed: earlier deployed Shopify evidence was for one Admin API token campaign, and the validated store had fewer than 250 matching orders. No runtime bug was found in the route trace. Smallest safe local action: add regression guards to `server/shopify-revenue-regression.test.ts` proving every Shopify order-reading path uses the paginated reader and the save route remains campaign-access guarded, campaign-connection scoped, source-ID scoped, platform-context scoped, exact mapping scoped, record-source scoped, and campaign recompute scoped. Local validation passed: `npm test -- server/shopify-revenue-regression.test.ts` (`21` tests). Deployed Current Commit 8 is now user-validated for a second Shopify Admin API token campaign/mapping packet: Shopify revenue appeared in GA4 Overview, Total Revenue included Shopify revenue, Revenue Sources and Revenue Breakdown matched, inventory stayed clean, Shopify source IDs did not overlap with the first campaign, and unrelated spend/revenue stayed unchanged. This closes Shopify Admin API token portability/mapping evidence for the validated GA4 Overview revenue scope. OAuth remains deferred; real >250 matching-order provider pagination remains excluded until a matching fixture exists; future Shopify/API/provider changes require future revalidation and are not current blockers; normal wall-clock daily scheduler timing remains optional operational proof beyond the already validated startup-fired scheduler path.
...
- Shopify Admin API token GA4 Overview revenue is clean-certified for the validated scope recorded above: source lifecycle, startup-fired scheduler refresh/reprocess, downstream value propagation, delivered report email evidence, deployed second-campaign/mapping portability, and the display-only mapped-campaign subtitle fix. Shopify OAuth is deferred and excluded. Real >250 matching-order provider pagination is excluded until a matching Shopify fixture exists. Future Shopify/API/provider changes and untested future report/email variants require future revalidation; normal wall-clock daily scheduler timing remains optional operational proof beyond the already validated startup-fired scheduler path. Salesforce revenue is deferred for v1 and hidden from the Add revenue source chooser; legacy Manual revenue, LinkedIn Ads, Meta Ads, Google Ads, and legacy Manual spend remain provider/user validation work until each relevant family has before/after evidence. HubSpot is clean-certified only for the exact proven GA4 Overview `Total Revenue only` scope on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458`, including the controlled 4.8b scheduler/provider propagation packet. HubSpot Pipeline Proxy is closed only for the recorded deployed 4.9b selected-source packet and the recorded deployed 4.10b proxy-to-confirmed transition packet on campaign `8aa735ee-c02f-41e2-bb1f-7c3f43bb9458` / source `d4ad51ef-85fe-4b67-bbd5-854900be3dee`; HubSpot Review Settings campaign-mapping visibility is locally regression-covered in 4.10c, HubSpot Revenue Sources mapped-campaign subtitles plus zero-proxy edit Review Settings display are locally regression-covered in 4.10d, and the read-only 4.11 Campaign Breakdown exact mapped-row checker is locally implemented/regression-covered with deployed evidence closed only for target row `yesop_retargeting` changing by `+$100` while named row `yesop_email_nurture` stayed unchanged; deployed UI confirmation for the display-only paths remains pending; Reports are closed only for the configured 4.12 packet, KPI/Benchmark propagation is closed only for the configured 4.13 Revenue KPI/Benchmark packet, and HubSpot report email/PDF evidence is closed only for the user-confirmed configured 4.14 GA4 Overview Report packet; Current Commit 4.15 deployed other-campaign portability evidence is closed only for the supplied two-campaign packet; Current Commit 4.16 local alternate-mapping automation is implemented/regression-covered but deployed variant packets are pending; alternate mappings beyond the recorded `yesop_brand_search` and `yesop_retargeting` packets, other Campaign Breakdown rows, other KPI/Benchmark metrics, future sends/report variants, future provider changes, and other HubSpot configurations remain unproven.
