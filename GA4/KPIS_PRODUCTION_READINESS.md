...
- proven: revision `797cc0411fb5cfa5b77f1269432cec1a613bac55` is deployed in production; the three widened columns and current row bounds are present on the target database; all 10 financial KPIs have an owner and valid reporting timezone; all 10 rows have exact campaign/property-or-absence/filter/window/source/persisted-input/provider-credential/stored-state classification; 3 deterministic simulated-property rows exactly match their resolved formula/source values; 3 propertyless rows resolve unavailable and their stored values are not treated as current; the 7 duplicate-email audit skips are immutable evidence retained by design; the user-confirmed Commit 7 browser PDF downloads, opens, and looks correct
- unproven: current totals for 4 live-provider financial rows; complete deployed valid-zero/unavailable/stale/blocked/insufficient-data/failure/source-mix/timezone UI matrix; bounded cleanup of 4 legacy orphan KPI parents; timer-fired exact updated/skipped/failed KPI IDs; live provider and token-refresh behavior bound to the deployed SHA; production notification retry after an injected failure; current direct snapshot/test-send/manual-send/scheduled-report freshness parity; and SHA-bound alert/report inbox delivery. Historical audit rows, expired credential metadata, persisted last-good values, or provider acceptance are not inferred as current provider proof
...
2. **Deployed state-matrix Gate 2 — attempted in Commit 10B; partially proven and pending.** Read-only production inventory plus the exact deployed code and 227/227 browser-consumer regressions cover the 19 campaign-valid rows and current consumer contract. Four additional legacy KPI parents reference a deleted campaign; current access/list paths fail closed, all have zero progress/alert/period children, and the only linked notification history is resolved with zero visible candidates, but persisted orphan-parent cleanup is not authorized here. The target also has no authoritative valid-zero, blocked, insufficient-data, or natural failure fixture and no authenticated browser capture was available. Smallest safe queue: (a) add the four-row orphan-parent boundary to the existing damaged-data dry-run and prove the current campaign-delete regression remains forward-safe; (b) in a separately authorized transaction, remove only those proven parent rows while preserving resolved notification audit history; (c) close the state matrix without production fixture changes by running authenticated browser automation against the exact deployed bundle with client-side intercepted state responses and every mutation/token-capable request blocked, capturing cards, tracker, Insights, bell/Notifications, and browser PDF. Do not mark Gate 2 passed until all three are evidenced.
...
Gate 2 status: **pending**. Proven portions are the exact deployed SHA, read-only row/scope/source/window/threshold/notification inventory, current fail-closed campaign access for the four legacy orphan parents, and the local consumer contract. Unproven portions are authoritative valid-zero, blocked, insufficient-data, and natural-failure deployed fixtures; authenticated deployed rendering for card/tracker/Insights/bell/Notifications/PDF across every state; and separately authorized cleanup proof for the four orphan parents. The chronological smallest-safe queue is step 2 above. Overall KPI status remains **UNVERIFIED**.
