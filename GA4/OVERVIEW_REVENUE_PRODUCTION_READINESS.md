...
Current Commit 12 completed the final CSV-only rerun and documentation audit. After that commit, the user confirmed the deployed invalid-file test also passed: the upload showed an error, created no source, and left Total Revenue and Total Spend unchanged. Upload CSV Revenue is clean-certified for the validated documented scope. Current Commit 21 re-enables Google Sheets Revenue, but it is not clean-certified.
...
- **Google Sheets Revenue is re-enabled but not clean-certified.** Current Commit 21 restores its chooser/API entry point. Foreground GA4 add/edit and scheduler refresh use campaign-scoped atomic source/record replacement, but deployed new-source, provider-failure, idempotency, and full downstream evidence remains incomplete; automatic updates still rely on the daily external refresh path rather than bounded source-family polling, and open Overview revenue queries refetch only every ten minutes.
