...
**Current section status: RELEASE_CANDIDATE_READY.** Exact current runtime `c6487555c55726427afed8342312b8393498303b` is user-confirmed deployed. Authenticated UI/API/source parity and the read-only production-data inventory passed for the recorded campaign, live property `542352127`, USD source set, and persisted records. Campaign Breakdown reconciled all three visible rows exactly. A natural timer-fired run on scheduler-fix runtime `85f5233ebfc298afc35f4c24e0930c1a66fbd07c` persisted the exact target data without later app repair. Reviewed later changes through `c6487555` are KPI/Benchmark-only and leave Overview values and the GA4 daily-data producer unchanged. Machine status remains `UNVERIFIED` because this session intentionally does not perform the final combined GA4 certification.
...
- Current reviewed and user-confirmed deployed runtime: `c6487555c55726427afed8342312b8393498303b`
...
| --- | --- | --- |
| Current evidence candidate | **RELEASE_CANDIDATE_READY** | Runtime `c6487555c55726427afed8342312b8393498303b` is user-confirmed deployed. Authenticated Summary/financial/source parity, exact Campaign Breakdown reconciliation, and the read-only zero-finding production inventory passed. The scheduler fix passed its regression-first packet, TypeScript, production build, and natural target-persistence observation at `85f5233e`; later changes through `c6487555` were reviewed as KPI/Benchmark-only and do not alter Overview values or its daily-data producer. Final combined GA4 certification is not included. |
