...
- Certification integrity rule: GA4 readiness is valid only for the exact certified SHA/configuration/dependency boundary. A relevant upstream or downstream change automatically invalidates affected tabs. The GA4 KPI record is `GA4/certifications/ga4-kpis.json` and its checker is `npm run check:ga4-kpi-certification`. Current Commits 1-5 implement the fail-closed gate, dynamic cross-consumer guard, metric identity contract, authoritative traffic/rate window contract, and bounded financial source/failure contract while the record remains `UNVERIFIED`; Current Commits 6-10 and external validation remain open. Copied formulas and source-text guards are structural evidence only.
...
- GA4 KPI whole-tab status is not production-ready and remains `UNVERIFIED`; `GA4/KPIS_PRODUCTION_READINESS.md` is controlling. Current Commits 4 and 5 locally align the traffic/rate window and bounded financial source/failure producer contracts, but Current Commits 6-10 and all listed external gates remain open.
