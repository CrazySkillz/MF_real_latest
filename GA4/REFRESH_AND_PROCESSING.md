...
- the authoritative Google Sheets source-family evidence and broader exclusions are in `GA4/OVERVIEW_SPEND_PRODUCTION_READINESS.md`; no whole-Overview gate remains open inside the exact certified campaign/property boundary
...
- GA4 Overview is **CLEAN-CERTIFIED AND PRODUCTION-READY** for the exact runtime/campaign/property/source boundary in `GA4/OVERVIEW_PRODUCTION_READINESS.md`. The corrected initial-import boundary, timer-fired target refresh, native/imported USD currency parity, and observed OAuth durability are closed at deployed runtime `8ba694060411a2a05663a4915652767e4e3ba713`.
...
KPI release-candidate scheduler evidence (`2026-08-14`): under the temporary `20:35 UTC` schedule on deployed SHA `85f5233ebfc298afc35f4c24e0930c1a66fbd07c`, the natural timer fired at `2026-08-14T20:35:00.001Z` and the target recompute updated all 12 KPIs with zero KPI skips and zero KPI failures. The process-wide run failed because of 17 excluded obsolete campaigns and is not described as globally successful. Later KPI release-candidate commits `950307b6` and `1a93d8d8` changed browser KPI/PDF freshness state and KPI-only Notification visibility, not the scheduler or KPI recompute pipeline. Production on `1a93d8d8` reported the `20:35 UTC` timer armed with zero runs for that new process; no exact-current natural firing is claimed.
