...
| 2 | Revenue Breakdown | `COMPLETE` | `CERTIFIED` | `CLEAN-CERTIFIED / PRODUCTION_READY` only for the exact documented Campaign2/property/source boundary at runtime `dd9cd51ea755e896838c78be8cab020fc0249eba`; controlling certificate: `GA4/AD_COMPARISON_REVENUE_BREAKDOWN_CERTIFICATION_2026-09-16.md`. Native comparison-window revenue and imported all-mapped-record amounts remain separate; no cross-window Total Revenue or report-delivery claim. |

#### GA4 Insights fresh subsection validation

The exact-runtime Insights certificates below remain preserved as historical evidence. They predate the scheduler-only daily writer, completed-date zero materialization, campaign-start zero-filled Trends/findings calendar, one-row Daily rendering, stale-finding removal, and corrected financial KPI/Benchmark import boundary introduced through `4372efadc96523c9df2abe2dd7915f9e1a0443a7`. The current application implementation is therefore unverified pending a new exact-revision deployed pass. Treat GA4 Overview, KPIs, Benchmarks, and financial-source certificates as read-only dependencies. Reports and delivery remain outside this Insights pass.

| Order | Insights subsection | Work state | Fresh certification status | Required validation boundary |
| ---: | --- | --- | --- | --- |
| - | Insights (current combined-page pass) | `REVALIDATION_REQUIRED` | `UNVERIFIED` | Current code behavior is documented in `GA4/INSIGHTS.md` and controlled by `GA4/INSIGHTS_PRODUCTION_READINESS.md` plus `GA4/certifications/ga4-insights.json`. The earlier combined on-screen certificate at deployed runtime `f74167597d8f9aad09a43466f3a94ce3b7c7cedd` is historical and does not cover the current scheduler, zero-date, UI, finding, or financial-window behavior. Required current gates: exact deployment, authenticated owner UI/API parity, tenant isolation, provider failure/last-good validation, dependency freeze, and a natural scheduler cycle. |
...
The latest preserved Performance Summary certificate is separate from the older historical certificate for runtime `12789c1e`. Current implementation `ca1fc5a873ab69fb747937768c31fcd53e16fd64` includes later campaign-currency and Recent Movement transition changes that passed the focused 5-file / 70-test packet, TypeScript check, and production build locally but have not received authenticated exact-revision deployed recertification. The table below records the preserved `ee6e11eb` certificate boundary, not a certificate for current runtime `ca1fc5a8`.
...
| Whole GA4 roll-up | `RECONCILE` | Section rows above override `GA4_PRODUCTION_READY_TRACKER.md` and `GA4_PRODUCTION_READINESS_OUTSTANDING.md` | The broad historical tracker does not establish current whole-GA4 readiness; use the section rows and current machine records. |
...
| --- | --- | --- |
| GA4 daily facts and KPI/Benchmark recompute | `GA4/REFRESH_AND_PROCESSING.md` records earlier bounded natural runs; `GA4/INSIGHTS_COMBINED_ONSCREEN_CERTIFICATION_2026-09-18.md` records a historical Campaign2 manual run on product revision `f7416759`. Current manual/run-now writes are disabled. | `UNVERIFIED` for a current natural timer firing and reconciled effects; the historical manual run is not counted as one. |
