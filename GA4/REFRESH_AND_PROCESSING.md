...
- Campaign DeepDive Trend Analysis rereads persisted values while visible and on window focus. Its mounted GA4-first financial cards use only compatible `financial_daily_snapshot_v1` rows written by the daily scheduler after the GA4 daily and financial-source refresh evidence succeeds for the same reporting date. The Trend financial-card outcome/comparison requests are persisted-only, so page load and selector changes do not perform a live GA4 aggregate fallback or reconstruct missing financial history for those cards. Missing exact snapshots remain unavailable, while the five financial card positions remain stable and display an unavailable/scheduler-waiting state. The separate provider coverage request may still contact GA4 to verify traffic history and zero-date handling; any financial fields in that verification response are not financial-card inputs.
