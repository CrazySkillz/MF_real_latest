...
- GA4 Overview is not production-ready or clean-certified. Current Commits 1-5 are closed. Current Commit 6's inventory is complete, but its closeout is reopened because the user confirmed the `Summer splash` `$400` Manual Spend source is unwanted legacy data; exact deletion and post-delete inventory remain required. Current Commit 7's transaction/cache work is pushed, and a local follow-up blocks GA4 Manual Revenue and Manual Spend create/edit while preserving exact deletion; deployment and cleanup validation remain required. The durable source of truth is `GA4/OVERVIEW_PRODUCTION_READINESS.md`.
- Current Commit 7 also includes a local fail-closed follow-up for active OAuth placeholder rows with an empty GA4 Property ID: they are not usable connections, GA4 metrics remain unavailable, and existing campaign-scoped financial sources remain reachable only for review or exact removal. No reconnect or production-data mutation is part of the fix.
...
- Google Sheets Revenue hold: all further Google Sheets Revenue implementation, automation, provider/OAuth, scheduler, polling, damaged-data inventory, downstream propagation, and deployed validation coverage is on hold until the user explicitly resumes it. Current Commit 8 proceeds only with Upload CSV Revenue downstream propagation automation.
