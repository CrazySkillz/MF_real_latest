...
Next active HubSpot step: deploy the Current Commit 4.13 persisted GA4 KPI/Benchmark financial-source fix, run an existing campaign-scoped GA4 recompute path such as `/api/campaigns/:id/ga4/refresh` or wait for scheduler/source recompute, then rerun `hubspotKpiBenchmarkValuePack(...)` against the configured campaign/property and existing KPI/Benchmark rows. If that passes, proceed to Current Commit 4.14 HubSpot email evidence.
