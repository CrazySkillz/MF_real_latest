...
3. Change only the connected disposable Google Sheet/tab to one known expected revenue amount, then trigger the product's exposed refresh/reprocess action for the existing Google Sheets revenue source. If no separate refresh control exists after UI/API trace, document refresh/reprocess as not applicable only after proving edit/update is the only reprocess path.
