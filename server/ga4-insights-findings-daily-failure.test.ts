...
describe("GA4 Insights finding eligibility on daily-history failure", () => {
  it("withholds the insufficient-history finding until a daily response exists", () => {
    ...
    const eligible = new Function("trendsRefreshIsStale", "findingDailyHistoryMismatch", "ga4InsightsDailyResp", `return (${condition});`) as
    ...
    expect(eligible(false, true, { data: [] })).toBe(false); // Provider mismatch is not insufficient history.
